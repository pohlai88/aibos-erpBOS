...
async function fanoutNewOutboxRows() {
  ...
  for (const ev of out.rows) {
    ...
    const targets = hooks.rows.filter(
      h => Array.isArray(h.topics) && h.topics.includes(ev.topic)
    );
...
async function deliverPending() {
  const due =
    await pool.query(`select a.id, a.webhook_id, a.event_id, a.topic, a.payload::text as payload
