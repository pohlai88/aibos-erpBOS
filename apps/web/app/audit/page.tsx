...
export default function Audit() {
  ...
  async function load(reset = false) {
    ...
    const res = await fetch(
      `http://localhost:3000/api/audit/${base}?` + qs.toString(),
      {
  ...
  async function openLines(j: Journal) {
    ...
    setLines(null);
    const res = await fetch(
      `http://localhost:3000/api/audit/journals/${j.id}/lines`,
      {
        headers: { 'X-API-Key': apiKey },
      }
    );
      ...
      {tab === 'journals' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            ...
            <button onClick={() => load(false)} disabled={busy || !next}>
              Load more
            </button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <button
              onClick={() => {
                ...
                const from = (
                  document.getElementById('efrom') as HTMLInputElement
                ).value;
                const to = (document.getElementById('eto') as HTMLInputElement)
                  .value;
                setRows([]);
                setNext(null);
                setBusy(true);
                ...
                if (to) qs.set('to', to);
                fetch(
                  `http://localhost:3000/api/audit/events?` + qs.toString(),
                  { headers: { 'X-API-Key': apiKey } }
                )
                  .then(r => r.json())
                  .then(j => {
