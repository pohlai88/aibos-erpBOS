...
export async function unpostAssets(
  ...
  const distinctPlans = Array.from(
    new Set(entries.map(entry => entry.plan_id))
  );
  const journalsToReverse = entries
    .map(entry => entry.booked_journal_id)
    .filter(Boolean);
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.error(
        `Failed to reverse journal ${entry.booked_journal_id}:`,
        error
      );
...
export async function validateUnpostSafety(
  ...
  if (
    ...
    periodResult.rows[0].status === 'closed'
  ) {
    warnings.push(
      `Period ${year}-${month.toString().padStart(2, '0')} is closed`
    );
  }
  ...
  if (Number(entriesResult.rows[0].count) === 0) {
    warnings.push(
      `No posted entries found for ${year}-${month.toString().padStart(2, '0')}`
    );
  }
  ...
  if (Number(recentUnpostResult.rows[0].count) > 0) {
    warnings.push(
      `Recent unposting activity detected for ${year}-${month.toString().padStart(2, '0')}`
    );
  }
