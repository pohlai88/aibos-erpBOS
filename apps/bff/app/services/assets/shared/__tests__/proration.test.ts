...
import { prorate, validateProrationParams, getProrationSummary } from "../proration";
...
describe("proration", () => {
  ...
  it("validates proration parameters", () => {
    expect(validateProrationParams(100, "2025-11-15", "days_in_month").valid).toBe(true);
    expect(validateProrationParams(-100, "2025-11-15", "days_in_month").valid).toBe(false);
    expect(validateProrationParams(100, "invalid-date", "days_in_month").valid).toBe(false);
  });
