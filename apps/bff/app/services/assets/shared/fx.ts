...
export async function resolveFxRate(params: FxRateParams): Promise<number> {
  ...
  const rate = await fetchFxRate(
    srcCcy,
    presentCcy,
    policy === 'post_month' && year && month ? { year, month } : undefined
  );
 ...
 */
export function validateFxParams(params: FxRateParams): {
  valid: boolean;
  error?: string;
} {
  ...
  if (params.policy === 'post_month' && (!params.year || !params.month)) {
    return {
      valid: false,
      error: 'Year and month required for post_month policy',
    };
  }
