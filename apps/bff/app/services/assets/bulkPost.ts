...
export async function bulkPostAssets(
  ...
  const baseFilter = (rows: any[]) => rows.filter((r: any) => 
    Number(r.year) === year && 
...
export async function validatePostingSafety(
  ...
  if (targetDate > now) {
    warnings.push(`Posting for future period ${year}-${month.toString().padStart(2, '0')}`);
  }

  // Check if period is too far in the past (more than 2 years)
  const twoYearsAgo = new Date(now.getFullYear() - 2, now.getMonth(), 1);
  if (targetDate < twoYearsAgo) {
    warnings.push(`Posting for old period ${year}-${month.toString().padStart(2, '0')} (more than 2 years ago)`);
  }
  ...
  if (Number(existingRows.rows[0].count) > 0) {
    warnings.push(`Some entries for ${year}-${month.toString().padStart(2, '0')} are already posted`);
  }
