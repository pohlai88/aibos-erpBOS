...
describe('Tax Return Templates Service', () => {
  ...
  beforeEach(async () => {
    // Clean up test data
    await pool.query('DELETE FROM tax_return_box_map WHERE company_id = $1', [
      companyId,
    ]);
  ...
  it('should run tax return dry-run', async () => {
    ...
    // Run dry-run
    const result = await runTaxReturn(
      companyId,
      {
        partner_code: 'MY-SST',
        version: '2025-01',
