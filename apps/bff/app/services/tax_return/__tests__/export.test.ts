...
describe('Tax Return Export Service', () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      'DELETE FROM tax_return_export WHERE run_id IN (SELECT id FROM tax_return_run WHERE company_id = $1)',
      [companyId]
    );
    await pool.query(
      'DELETE FROM tax_return_line WHERE run_id IN (SELECT id FROM tax_return_run WHERE company_id = $1)',
      [companyId]
    );
    await pool.query('DELETE FROM tax_return_run WHERE company_id = $1', [
      companyId,
    ]);
    await pool.query('DELETE FROM tax_return_template WHERE company_id = $1', [
      companyId,
    ]);
    await pool.query('DELETE FROM tax_partner WHERE company_id = $1', [
      companyId,
    ]);
  });
  ...
  it('should throw error for non-existent run', async () => {
    await expect(
      exportTaxReturn('non-existent-run', {
