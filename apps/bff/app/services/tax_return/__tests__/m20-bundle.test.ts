...
import { TaxExportRequest, TaxCarryForwardScanRequest, TaxCarryForwardProposeRequest, TaxCarryForwardAcceptRequest } from '@aibos/contracts';
...
describe('M20.1 - Authority-Specific Exporters', () => {
    describe('MY-SST-02-CSV Profile', () => {
        it('should validate required boxes', () => {
            ...
            const lines = [
                { box_id: 'OUTPUT_TAX', amount: 100.00, box_label: 'Output Tax', ordinal: 1 },
                { box_id: 'INPUT_TAX', amount: 50.00, box_label: 'Input Tax', ordinal: 2 }
            ];
        ...
        it('should reject missing OUTPUT_TAX', () => {
            ...
            const run = { id: 'test', partner_code: 'MY-SST', period_key: '2025-01' };
            const lines = [
                { box_id: 'INPUT_TAX', amount: 50.00, box_label: 'Input Tax', ordinal: 1 }
            ];

            expect(() => exporter.validate(run, lines)).toThrow('Required box OUTPUT_TAX is missing');
        });
        ...
        it('should validate rounding to 2dp', () => {
            ...
            const run = { id: 'test', partner_code: 'MY-SST', period_key: '2025-01' };
            const lines = [
                { box_id: 'OUTPUT_TAX', amount: 100.123, box_label: 'Output Tax', ordinal: 1 },
                { box_id: 'INPUT_TAX', amount: 50.00, box_label: 'Input Tax', ordinal: 2 }
            ];

            expect(() => exporter.validate(run, lines)).toThrow('must be rounded to 2 decimal places');
        });
        ...
        it('should generate deterministic filename', () => {
            const exporter = getExporter('MY-SST-02-CSV');
            const run = { id: 'run123', partner_code: 'MY-SST', period_key: '2025-01' };
    ...
    describe('UK-VAT100-XML Profile', () => {
        ...
        it('should reject missing box 5', () => {
            ...
            expect(() => exporter.validate(run, lines)).toThrow('Required box 9 is missing');
        });
        ...
        it('should validate integer pennies', () => {
            ...
            const lines = Array.from({ length: 9 }, (_, i) => ({
                box_id: (i + 1).toString(),
                amount: 100.50, // Not integer pennies
                box_label: `Box ${i + 1}`,
                ordinal: i + 1
            }));

            expect(() => exporter.validate(run, lines)).toThrow('must be in whole pennies');
        });
        ...
        it('should validate totals reconciliation', () => {
            ...
            expect(() => exporter.validate(run, lines)).toThrow('Box 5 total 500 does not match sum of boxes 1+2+3 (600)');
        });

        it('should generate proper XML format', () => {
            const exporter = getExporter('UK-VAT100-XML');
            const run = { id: 'test', partner_code: 'UK-VAT', period_key: '2025-01', version: '2025', created_at: '2025-01-01' };
            const lines = [
                { box_id: '1', amount: 100, box_label: 'Box 1', ordinal: 1 }
            ];
    ...
    describe('Registry', () => {
        ...
        it('should throw for unknown profile', () => {
            expect(() => getExporter('UNKNOWN-PROFILE')).toThrow('Unknown export profile: UNKNOWN-PROFILE');
        });
    });
});
...
describe('M20.2 - Late-Entry Carry-Forward', () => {
    describe('Contract Validation', () => {
        it('should validate scan request', () => {
            ...
            expect(() => TaxCarryForwardScanRequest.parse(validRequest)).not.toThrow();
        });
        ...
        it('should validate propose request', () => {
            ...
            expect(() => TaxCarryForwardProposeRequest.parse(validRequest)).not.toThrow();
        });
        ...
        it('should validate accept request', () => {
            ...
            expect(() => TaxCarryForwardAcceptRequest.parse(validRequest)).not.toThrow();
        });
    });
});
