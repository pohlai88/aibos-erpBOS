  ...
  validate(run: any, lines: any[]): void {
    ...
    for (const line of lines) {
      ...
      if (Math.abs(line.amount - rounded) > 0.001) {
        throw new Error(
          `Box ${line.box_id} amount ${line.amount} must be rounded to 2 decimal places`
        );
      }
    }
    ...
    if (Math.abs(netPayable) > 0 && Math.abs(netPayable) < 0.01) {
      throw new Error(
        `NET_PAYABLE amount ${netPayable} is too small - must be at least 0.01`
      );
    }
  }
...
export class UkVat100XmlProfile implements ExportProfile {
  validate(run: any, lines: any[]): void {
    ...
    for (const line of lines) {
      if (line.amount % 1 !== 0) {
        throw new Error(
          `Box ${line.box_id} amount ${line.amount} must be in whole pennies for VAT100`
        );
      }
    }
    ...
    if (Math.abs(box5 - calculatedBox5) > 0) {
      throw new Error(
        `Box 5 total ${box5} does not match sum of boxes 1+2+3 (${calculatedBox5})`
      );
    }
  }
...
export async function exportWithProfile(
  ...
  if (input.profile) {
    ...
    if (!defaultProfile) {
      throw new Error(
        `No default export profile found for ${run.partner_code} ${run.version}`
      );
    }
