...
import { TaxCarryForwardScanRequestType, TaxCarryForwardProposeRequestType, TaxCarryForwardAcceptRequestType } from "@aibos/contracts";
...
export async function scanLateEntries(input: TaxCarryForwardScanRequestType, companyId: string): Promise<CarryForwardProposal[]> {
    ...
    if (closeRows.length === 0) {
        throw new Error(`Period ${from_year}-${from_month.toString().padStart(2, '0')} has not been closed`);
    }
    ...
    for (const row of docRows) {
        ...
        boxMap.set(row.box_id, (boxMap.get(row.box_id) || 0) + parseFloat(row.amount));
    }
...
export async function proposeCarryForward(input: TaxCarryForwardProposeRequestType, companyId: string, userId: string): Promise<CarryForwardResult[]> {
    ...
    for (const proposal of input.proposals) {
        ...
        await pool.query(
            ...
            [
                id, companyId, 'DEFAULT_PARTNER', fromYear, fromMonth + 1, intoYear, intoMonth + 1,
                proposal.source_ref, proposal.box_id, proposal.amount, proposal.reason, userId
            ]
        );
...
export async function acceptCarryForward(input: TaxCarryForwardAcceptRequestType, companyId: string, userId: string): Promise<CarryForwardResult[]> {
    const results: CarryForwardResult[] = [];
    ...
    for (const id of input.ids) {
        ...
        if (commitRows.length > 0) {
            throw new Error(`Target period ${cf.into_year}-${cf.into_month} is already committed`);
        }
        ...
        await pool.query(
            ...
            [
                adjustmentId, companyId, cf.partner_code, cf.into_year, cf.into_month,
                cf.box_id, cf.amount, `Carry forward from ${cf.from_year}-${cf.from_month} / ${cf.source_ref}`, userId
            ]
        );
...
export async function getCarryForwards(companyId: string, filters: {
    status?: string;
    from_year?: number;
    from_month?: number;
    into_year?: number;
