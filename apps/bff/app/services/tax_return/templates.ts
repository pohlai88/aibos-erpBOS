...
import {
  ...
  TaxReturnRunRequest,
  TaxAdjustmentUpsert,
  TaxPartnerUpsertType,
  TaxTemplateUpsertType,
  TaxBoxMapUpsertType,
  TaxReturnRunRequestType,
  TaxAdjustmentUpsertType,
} from '@aibos/contracts';

// Tax Partner Management
export async function upsertTaxPartner(
  companyId: string,
  input: TaxPartnerUpsertType,
  updatedBy: string
) {
...
export async function upsertTaxTemplate(
  companyId: string,
  input: TaxTemplateUpsertType,
  updatedBy: string
) {
...
export async function getTaxTemplate(
  companyId: string,
  partnerCode: string,
  version: string
) {
...
export async function upsertTaxBoxMap(
  companyId: string,
  input: TaxBoxMapUpsertType,
  updatedBy: string
) {
...
export async function getTaxBoxMap(
  companyId: string,
  partnerCode: string,
  version: string
) {
...
export async function runTaxReturn(
  companyId: string,
  input: TaxReturnRunRequestType,
  actor: string
) {
  const validated = TaxReturnRunRequest.parse(input);

  // Load template and mapping
  const template = await getTaxTemplate(
    companyId,
    validated.partner_code,
    validated.version
  );
  const boxMap = await getTaxBoxMap(
    companyId,
    validated.partner_code,
    validated.version
  );
  ...
  if (template.length === 0) {
    throw new Error(
      `No template found for ${validated.partner_code} version ${validated.version}`
    );
  }
  ...
  await pool.query(
    ...
    [
      ...
      validated.month,
      periodKey,
      validated.dry_run ? 'dry_run' : 'commit',
      actor,
    ]
  );
  ...
  const { rows: taxRows } = await pool.query(
    ...
    [
      companyId,
      new Date(Date.UTC(validated.year, validated.month - 1, 1))
        .toISOString()
        .split('T')[0],
      new Date(Date.UTC(validated.year, validated.month, 0))
        .toISOString()
        .split('T')[0],
    ]
  );
  ...
  for (const taxRow of taxRows) {
    const matchingRules = boxMap.filter(rule => {
      if (
        rule.account_like &&
        !taxRow.account_code.includes(rule.account_like.replace('%', ''))
      )
        return false;
      if (
        rule.cc_like &&
        taxRow.cost_center_id &&
        !taxRow.cost_center_id.includes(rule.cc_like.replace('%', ''))
      )
        return false;
      if (
        rule.project_like &&
        taxRow.project_id &&
        !taxRow.project_id.includes(rule.project_like.replace('%', ''))
      )
        return false;
      return true;
    });
    ...
    if (matchingRules.length > 0) {
      ...
      if (templateBox) {
        ...
        boxTotals.set(
          rule.box_id,
          (boxTotals.get(rule.box_id) || 0) + contribution
        );
  ...
  for (const adj of adjustments) {
    boxTotals.set(
      adj.box_id,
      (boxTotals.get(adj.box_id) || 0) + Number(adj.amount)
    );
  }
...
export async function getTaxAdjustments(
  companyId: string,
  partnerCode: string,
  year: number,
  month: number
