...
export class EnhancedEvidenceService {
  ...
  async uploadEvidence(
    ...
    if (fileStream) {
      ...
      if (computedHash !== data.sha256_hex) {
        throw new Error(
          'SHA256 mismatch: claimed hash does not match computed hash'
        );
      }
    }
    ...
    const recordId = ulid();
    await this.dbInstance.insert(evdRecord).values({
      id: recordId,
      companyId,
      objectId,
      source: data.source,
  ...
  async linkEvidence(
    ...
    const linkId = ulid();
    await this.dbInstance.insert(evdLink).values({
      id: linkId,
      companyId,
  ...
  async buildManifest(
    ...
    if (data.filters) {
      if (data.filters.pii_level_max) {
        ...
        filteredRecords = filteredRecords.filter(
          record => piiLevels.indexOf(record.piiLevel) <= maxIndex
        );
      }
    ...
    const manifestHash = this.computeSHA256(
    ...
    await this.dbInstance.insert(evdManifest).values({
      id: manifestId,
      companyId,
    ...
    for (const record of filteredRecords) {
      const link = links.find(l => l.recordId === record.id);
      if (link) {
        await this.dbInstance.insert(evdManifestLine).values({
          manifestId,
          recordId: record.id,
  ...
  async buildBinder(
    ...
    const binderId = ulid();
    await this.dbInstance.insert(evdBinder).values({
      id: binderId,
      companyId,
  ...
  async attestBinder(
    ...
    await this.dbInstance.insert(evdAttestation).values({
      id: attestationId,
      companyId,
  ...
  private async buildZipContent(manifest: any, lines: any[]): Promise<string> {
    ...
    const zipStructure = {
      ...
      evidence: lines.map((line, index) => ({
        [`${index + 1}_${line.line.title.replace(/[^a-zA-Z0-9]/g, '_')}.${this.getFileExtension(line.object.mime)}`]: `Content from ${line.object.storageUri}`,
      })),
    };
