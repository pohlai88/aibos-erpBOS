...
export class EvidenceVaultService {
  ...
  async createEvidenceManifest(
    ...
    const evidenceItems = await Promise.all(
      data.evidence_items.map(async item => ({
        id: ulid(),
        manifestId,
    ...
    const totalSizeBytes = evidenceItems.reduce(
      (sum, item) => sum + item.sizeBytes,
      0
    );
    const contentHash = await this.computeSHA256(
      evidenceItems
        .map(item => `${item.itemName}:${item.contentHash}`)
        .join('|')
    );
    ...
    const manifestData = {
      ...
      manifestHash: await this.computeSHA256(
        JSON.stringify({
          bundleName: data.bundle_name,
          bundleType: data.bundle_type,
          controlId: data.control_id,
  ...
  async queryEvidenceManifests(
    ...
    if (query.created_from) {
      conditions.push(
        sql`${ctrlEvidenceManifest.createdAt} >= ${query.created_from}`
      );
    }
    if (query.created_to) {
      conditions.push(
        sql`${ctrlEvidenceManifest.createdAt} <= ${query.created_to}`
      );
    }
  ...
  async addEvidenceItem(
    ...
    await this.dbInstance
      ...
      .set({
        ...
        contentHash: await this.computeSHA256(
  ...
  async generateEbinder(
    ...
    if (!manifestIds || manifestIds.length === 0) {
      ...
      conditions.push(
        sql`${ctrlEvidenceManifest.createdAt} >= ${data.period_start}`
      );
      conditions.push(
        sql`${ctrlEvidenceManifest.createdAt} <= ${data.period_end}`
      );
    ...
    const totalSizeBytes = manifestDetails.reduce(
      (sum, m) => sum + m.sizeBytes,
      0
    );
    const totalEvidenceItems = manifestDetails.reduce(
  ...
  async addEvidenceAttestation(
    ...
    await this.dbInstance
      .insert(ctrlEvidenceAttestation)
