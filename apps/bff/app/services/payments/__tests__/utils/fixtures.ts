...
export async function ensureBankProfile({ companyId, bankCode }: { companyId: string, bankCode: string }) {
    await pool.query(`
  ...
  `, [companyId, bankCode, 'SFTP', JSON.stringify({
        host: 'sftp.hsbc.my',
        port: 22,
        username: 'acme',
...
export async function ensureExportedRun({ companyId, runId }: { companyId: string, runId: string }) {
    await pool.query(`
...
export async function seedPayLines({ companyId, runId, invoiceIds }: { companyId: string, runId: string, invoiceIds: string[] }) {
