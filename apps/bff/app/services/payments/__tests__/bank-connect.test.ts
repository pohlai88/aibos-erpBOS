...
describe('Bank Connectivity Services', () => {
  ...
  describe('Bank Profile Management', () => {
    ...
    it('should update an existing bank profile', async () => {
      ...
      const updatedProfile = await upsertBankProfile(
        ids.companyId,
        updatedData,
        'test-user'
      );
    ...
    it('should validate SFTP config requirements', async () => {
      ...
      await expect(
        upsertBankProfile(ids.companyId, invalidData, 'test-user')
  ...
  describe.sequential('Payment Dispatch', () => {
    beforeEach(async () => {
      await ensureBankProfile({
        companyId: ids.companyId,
        bankCode: ids.bankCode,
      });
    ...
    it('should reject dispatch for non-exported run', async () => {
      ...
      await expect(
        dispatchPaymentRun(ids.companyId, dispatchData)
  ...
  describe.sequential('Bank File Fetching', () => {
    ...
    it('should handle fetch errors gracefully', async () => {
      ...
      await expect(fetchBankFiles(ids.companyId, fetchData)).rejects.toThrow(
        'Bank profile not found or inactive'
      );
    });
  });
  ...
  describe.sequential('Reason Code Normalization', () => {
    ...
    it('should normalize reason codes', async () => {
      ...
      // Test normalization
      const normalizedStatus = await normalizeReasonCode(
        ids.bankCode,
        'RJCT',
        'unknown'
      );
      ...
      const fallbackStatus = await normalizeReasonCode(
        ids.bankCode,
        'UNKNOWN',
        'ack'
      );
  ...
  describe.sequential('Job Logging', () => {
    ...
    it('should filter job logs by kind', async () => {
      ...
      const dispatchLogs = await getBankJobLogs(
        ids.companyId,
        ids.bankCode,
        'DISPATCH'
      );
      const fetchLogs = await getBankJobLogs(
        ids.companyId,
        ids.bankCode,
        'FETCH'
      );
