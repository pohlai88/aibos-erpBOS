...
export async function upsertBankProfile(
  ...
  if (kind === 'SFTP') {
    if (!config.host || !config.port || !config.username || !config.key_ref) {
      throw new Error(
        'SFTP config missing required fields: host, port, username, key_ref'
      );
    }
  } else if (kind === 'API') {
...
export async function dispatchPaymentRun(
  ...
  await logBankJob(
    ...
    JSON.stringify({ run_id, filename, dry_run }),
    true
  );
...
export async function fetchBankFiles(
  ...
  await logBankJob(
    ...
    JSON.stringify({ channels, processed, errors }),
    errors.length === 0
  );
    ...
    const mockMappings = [
      {
        ...
        status: 'exec_ok',
        reasonCode: 'ACSC',
        reasonLabel: 'AcceptedSettlementCompleted',
      },
    ];
...
export async function upsertReasonNorm(
  data: ReasonNormUpsertType
): Promise<BankReasonNorm> {
...
async function processOutboxItem(item: BankOutbox): Promise<void> {
  ...
  await logBankJob(
    ...
    JSON.stringify({ filename: item.filename, runId: item.runId }),
    true
  );
}
