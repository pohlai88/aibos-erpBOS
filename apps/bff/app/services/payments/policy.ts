...
export async function upsertApprovalPolicy(
  ...
  await pool.query(
    ...
    [
      ...
      data.max_amount,
      data.currency,
      data.require_reviewer,
      data.require_approver,
      data.require_dual_approver,
      updatedBy,
    ]
  );
  ...
  return {
    ...
    maxAmount: result.rows[0].max_amount
      ? Number(result.rows[0].max_amount)
      : undefined,
...
export async function getSupplierPolicy(
  companyId: string,
  supplierId: string
): Promise<SupplierPolicy | null> {
  const result = await pool.query(
    `
...
export async function upsertSupplierLimit(
  ...
  return {
    ...
    yearCap: result.rows[0].year_cap
      ? Number(result.rows[0].year_cap)
      : undefined,
    updatedAt: result.rows[0].updated_at.toISOString(),
    updatedBy: result.rows[0].updated_by,
  };
...
export async function getPayeeKyc(
  companyId: string,
  supplierId: string
): Promise<PayeeKyc | null> {
  const result = await pool.query(
    `
        SELECT supplier_id, residency, tax_form, tax_id, doc_type, doc_ref, doc_expires, risk_level, on_hold, notes, updated_at, updated_by
        FROM ap_payee_kyc
...
async function validatePolicyRequirements(
  ...
  for (const line of lineRows) {
    ...
    if (amount < Number(policy.min_amount)) {
      throw new Error(
        `Amount ${amount} below minimum ${policy.min_amount} for policy ${policyCode}`
      );
    }

    if (policy.max_amount && amount > Number(policy.max_amount)) {
      throw new Error(
        `Amount ${amount} exceeds maximum ${policy.max_amount} for policy ${policyCode}`
      );
    }
    ...
    if (policy.currency && policy.currency !== run.ccy) {
      throw new Error(
        `Currency ${run.ccy} not allowed for policy ${policyCode}`
      );
    }
...
async function validateSupplierLimits(
  ...
  if (limits.day_cap) {
    ...
    if (dayTotal + amount > Number(limits.day_cap)) {
      throw new Error(
        `Day cap exceeded: ${dayTotal + amount} > ${limits.day_cap}`
      );
    }
  }
  ...
  if (limits.year_cap) {
    ...
    if (yearTotal + amount > Number(limits.year_cap)) {
      throw new Error(
        `Year cap exceeded: ${yearTotal + amount} > ${limits.year_cap}`
      );
    }
  }
}
