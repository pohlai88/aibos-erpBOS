...
export async function upsertSupplierBank(
    ...
    `, [
        companyId, data.supplier_id, data.method, data.bank_name, data.iban, data.bic,
        data.acct_no, data.acct_ccy, data.country, data.active, updatedBy
    ]);
...
export async function getSupplierBanks(companyId: string): Promise<SupplierBank[]> {
    const result = await pool.query(`
        SELECT supplier_id, method, bank_name, iban, bic, acct_no, acct_ccy, country, active, updated_at, updated_by
        FROM ap_supplier_bank
...
export async function upsertPaymentPref(
    ...
    `, [
        companyId, data.supplier_id, data.pay_terms, data.pay_day_rule,
        data.min_amount, data.hold_pay, updatedBy
    ]);
    ...
    return {
        ...
        minAmount: result.rows[0].min_amount ? Number(result.rows[0].min_amount) : undefined,
...
export async function createPayRun(
    ...
    `, [runId, companyId, data.year, data.month, 'draft', data.ccy, data.present_ccy, createdBy]);
...
export async function selectInvoicesForRun(
    ...
    const eligibleInvoices = await getEligibleInvoices(companyId, run.ccy, data.filters);
    ...
    for (const invoice of eligibleInvoices) {
        ...
        const supplierBank = await getSupplierBankForInvoice(companyId, invoice.supplier_id);
        if (!supplierBank) {
            continue; // Skip invoices without bank details
        }

        // Get payment preferences
        const paymentPref = await getPaymentPrefForSupplier(companyId, invoice.supplier_id);
        ...
        const { payAmount, fxRate } = await calculatePaymentAmount(
            invoice, run.ccy, supplierBank.acct_ccy, companyId
        );
        ...
        `, [
            lineId, data.run_id, invoice.supplier_id, invoice.id, invoice.due_date,
            invoice.gross_amount, invoice.disc_amount || 0, payAmount,
            invoice.ccy, supplierBank.acct_ccy, fxRate, 'selected'
        ]);
...
export async function exportPayRun(
    ...
    await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
    `, [exportId, data.run_id, exportData.format, exportData.filename, exportData.payload, exportData.checksum]);
...
async function getSupplierBankForInvoice(companyId: string, supplierId: string): Promise<any> {
...
async function getPaymentPrefForSupplier(companyId: string, supplierId: string): Promise<any> {
    const { rows } = await pool.query(`
...
async function getPayRunWithLines(companyId: string, runId: string): Promise<PayRun> {
    const { rows: runRows } = await pool.query(`
...
async function getFileProfile(companyId: string, bankCode: string): Promise<any> {
    const { rows } = await pool.query(`
...
async function generateExportFile(run: PayRun, fileProfile: any, format: string): Promise<any> {
...
function generateCsvExport(run: PayRun, fileProfile: any): string {
    const headers = ['Supplier ID', 'Invoice ID', 'Amount', 'Currency', 'Bank Ref', 'Due Date'];
...
async function queueRemittanceNotifications(runId: string, lines: PayLine[]): Promise<void> {
    ...
    for (const supplierId of suppliers) {
        ...
        await pool.query(`
            ...
            VALUES ($1, $2, $3, $4, $5)
        `, [remittanceId, runId, supplierId, `supplier-${supplierId}@company.com`, 'queued']);
    }
}

async function postPaymentJournals(companyId: string, run: PayRun): Promise<void> {
    ...
    await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
    `, [
        journalId, companyId, new Date().toISOString(), run.ccy, 'AP_PAYMENT', run.id
    ]);
    ...
    for (const line of run.lines || []) {
        ...
        await pool.query(`
            ...
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        `, [
            lineId, journalId, '2000', 'C', line.payAmount, line.payCcy, 'SUPPLIER', line.supplierId
        ]);
    }
