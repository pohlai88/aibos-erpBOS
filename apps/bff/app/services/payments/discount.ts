...
export function parsePaymentTerms(
  termsText: string
): { discount_pct: number; discount_days: number; net_days: number } | null {
...
export async function optimizeAndCommitDiscountRun(
  ...
  run = await scanDiscountCandidates(
    companyId,
    {
...
async function commitDiscountRun(
  companyId: string,
  runId: string,
  createdBy: string
): Promise<void> {
...
export async function getDiscountRun(
  companyId: string,
  runId: string
): Promise<DiscountRun> {
...
export async function listDiscountRuns(
  companyId: string,
  status?: string
): Promise<DiscountRun[]> {
  let query = `
