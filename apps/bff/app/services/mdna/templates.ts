...
export class MdnaService {
     ...
     */
    async queryTemplates(companyId: string, query: MdnaTemplateQueryType): Promise<MdnaTemplateResponseType[]> {
     ...
     */
    async approveTemplate(companyId: string, templateId: string, userId: string): Promise<void> {
        await db
            .update(mdnaTemplate)
            .set({
                status: "APPROVED",
    ...
    async updateDraft(
        ...
        const [updatedDraft] = await db
            ...
            .where(and(
                eq(mdnaDraft.id, draftId),
                eq(mdnaDraft.companyId, companyId)
            ))
            .returning();
     ...
     * Query MD&A drafts
     */
    async queryDrafts(companyId: string, query: MdnaDraftQueryType): Promise<MdnaDraftResponseType[]> {
     ...
     * Approve MD&A draft
     */
    async approveDraft(companyId: string, draftId: string, userId: string): Promise<void> {
        await db
            .update(mdnaDraft)
            .set({
                status: "APPROVED",
    ...
    async publishMdna(
        ...
        if (draft.status !== "APPROVED") {
            throw new Error(`MD&A draft ${draftId} must be approved before publishing`);
        }
     ...
     */
    async queryPublished(companyId: string, query: MdnaPublishQueryType): Promise<MdnaPublishResponseType[]> {
...
<body>
     ...
     */
    private async storeHtml(content: string, companyId: string, draftId: string): Promise<string> {
