...
export class RbCreditsService {
    ...
    async applyCreditMemo(
        ...
        if (data.amount > remainingAmount) {
            throw new Error(`Credit amount ${data.amount} exceeds remaining memo amount ${remainingAmount}`);
        }

        // Create credit application
        await this.dbInstance
            .insert(rbCreditApply)
            .values({
                id: ulid(),
                memoId: data.memo_id,
    ...
    async autoApplyCredits(
        ...
        const availableCredit = await this.getAvailableCredit(companyId, customerId);
        ...
        for (const invoice of invoices) {
            ...
            if (applyAmount > 0) {
                ...
                for (const memo of memos) {
                    ...
                    if (memoRemaining > 0) {
                        const applyFromMemo = Math.min(applyAmount, memoRemaining, remainingCredit);
