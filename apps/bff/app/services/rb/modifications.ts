...
export class RevModificationService {
    ...
    return {
      ...
      lines: changeLines.map(line => {
        ...
        return {
          ...
          price_delta: lineData.priceDelta
            ? Number(lineData.priceDelta)
            : undefined,
  ...
  async applyChangeOrder(
    ...
    await this.processChangeOrderModification(
      companyId,
      userId,
  ...
  private async processChangeOrderModification(
    ...
    switch (treatment) {
      case 'SEPARATE':
        await this.processSeparateContract(
          companyId,
          userId,
          changeOrder,
          changeLines
      ...
      case 'TERMINATION_NEW':
        await this.processTerminationAndNew(
          companyId,
          userId,
          changeOrder,
          changeLines
      ...
      case 'PROSPECTIVE':
        ...
        break;
      case 'RETROSPECTIVE':
        await this.processRetrospective(
          companyId,
          userId,
          changeOrder,
          changeLines
  ...
  private async processTerminationAndNew(
    ...
    console.log(
      `Processing termination and new for contract ${changeOrder.contractId}`
    );
  }
  ...
  async upsertVCPolicy(
    ...
    const policy = await this.dbInstance
      ...
      .values({
        ...
        constraintProbabilityThreshold:
          data.constraint_probability_threshold.toString(),
    ...
    return {
      ...
      constraint_probability_threshold: Number(
        policyData.constraintProbabilityThreshold
      ),
  ...
  async upsertVCEstimate(
    ...
    const threshold = policy.length
      ? Number(policy[0]?.constraintProbabilityThreshold)
      : 0.5;
    ...
    const estimate = await this.dbInstance
      ...
      .onConflictDoUpdate({
        target: [
          revVcEstimate.companyId,
          revVcEstimate.contractId,
          revVcEstimate.pobId,
          revVcEstimate.year,
          revVcEstimate.month,
        ],
  ...
  async queryChangeOrders(
    ...
    if (query.effective_date_from) {
      whereConditions.push(
        gte(revChangeOrder.effectiveDate, query.effective_date_from)
      );
    }
    if (query.effective_date_to) {
      whereConditions.push(
        lte(revChangeOrder.effectiveDate, query.effective_date_to)
      );
    }
  ...
  async getDisclosures(
    ...
    const modifications = await this.dbInstance
      .select()
      .from(revModRegister)
      .where(and(eq(revModRegister.companyId, companyId)))
