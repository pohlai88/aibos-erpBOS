...
describe('M25 Revenue & Billing - Core Validation', () => {
  describe('Implementation Status', () => {
    ...
    it('should validate STAIR pricing calculation', () => {
      ...
      const tiers = [
        { from: 1, to: 10, price: 5.0 },
        { from: 11, to: 50, price: 4.0 },
        { from: 51, to: 999999, price: 3.0 },
      ];
      ...
      function calculateStairPrice(quantity: number, tiers: any[]): number {
        ...
        const applicableTier = tiers.find(
          tier => quantity >= tier.from && quantity <= tier.to
        );
        if (!applicableTier) {
  ...
  describe('Business Logic', () => {
    ...
    it('should validate usage rollup logic', () => {
      ...
      function rollupUsage(events: Array<{ quantity: number; unit: string }>): {
        total_quantity: number;
        units: string[];
      } {
        const total_quantity = events.reduce(
...
console.log(
  '🎉 M25 Revenue & Billing Core Validation tests completed successfully!'
);
