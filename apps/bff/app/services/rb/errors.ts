...
export class RbValidator {
    ...
    static validatePrice(data: any): void {
        ...
        if (!data.model || !['FLAT', 'TIERED', 'STAIR', 'VOLUME'].includes(data.model)) {
            throw new RbValidationError('model must be FLAT, TIERED, STAIR, or VOLUME');
        }
        if (data.model === 'FLAT' && (!data.unit_amount || data.unit_amount <= 0)) {
            throw new RbValidationError('unit_amount must be positive for FLAT pricing');
        }
        if (data.model === 'TIERED' && (!data.meta?.tiers || !Array.isArray(data.meta.tiers))) {
            throw new RbValidationError('tiers array is required for TIERED pricing');
        }
        if (data.model === 'STAIR' && (!data.meta?.tiers || !Array.isArray(data.meta.tiers))) {
            throw new RbValidationError('tiers array is required for STAIR pricing');
        }
        if (data.model === 'VOLUME' && (!data.unit_amount || data.unit_amount <= 0)) {
            throw new RbValidationError('unit_amount must be positive for VOLUME pricing');
        }
    }
}
