...
export class RbContractsService {
     ...
     */
    async getCustomerContracts(companyId: string, customerId: string): Promise<ContractResponseType[]> {
        ...
        if (query.customer_id) {
            ...
            const subscriptions = await this.dbInstance
                ...
                .where(and(
                    ...
                    query.bill_anchor ? eq(rbSubscription.billAnchor, query.bill_anchor) : sql`1=1`
                ))
    ...
    async upgradeSubscription(
        ...
        const newPrices = await this.dbInstance
            ...
            .where(and(
                ...
                eq(rbPrice.id, data.new_price_id)
            ))
            .limit(1);
