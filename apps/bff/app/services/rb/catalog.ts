...
import { rbProduct, rbPriceBook, rbPrice } from '@aibos/db-adapter/schema';
import type {
  ProductUpsertType,
  PriceBookUpsertType,
  PriceUpsertType,
...
export class RbCatalogService {
  ...
  async getProduct(
    ...
    const products = await this.dbInstance
      ...
      .from(rbProduct)
      .where(
        and(eq(rbProduct.companyId, companyId), eq(rbProduct.id, productId))
      )
      .limit(1);
  ...
  async getProductPrices(
    ...
    const prices = await this.dbInstance
      ...
      .from(rbPrice)
      .where(
        and(eq(rbPrice.companyId, companyId), eq(rbPrice.productId, productId))
      );
  ...
  async calculatePrice(
    ...
    if (prices.length === 0) {
      throw new Error(
        `No price found for product ${productId} in book ${bookId}`
      );
    }
  ...
  private calculateVolumePrice(price: any, quantity: number): number {
    ...
    const discounts = price.meta.discounts.sort(
