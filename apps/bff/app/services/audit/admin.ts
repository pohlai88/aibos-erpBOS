...
export class AuditAdminService {
        ...
        try {
            ...
            if (existingGrant.rows.length > 0) {
                ...
                await client.query(
                    ...
                    [grantId, companyId, auditorId, data.scope, data.object_id, data.can_download, data.expires_at, userId]
                );
            }
    ...
    async revokeGrant(
        ...
        try {
            ...
            // Delete the grant
            await client.query(
                `DELETE FROM audit_grant WHERE id = $1`,
                [grantId]
            );
            ...
            if (existingPolicy.rows.length > 0) {
                ...
                await client.query(
                    `UPDATE audit_watermark_policy 
                     ...
                     WHERE company_id = $7`,
                    [data.text_template, data.diagonal, data.opacity,
                    data.font_size, data.font_color, userId, companyId]
                );
            } else {
                // Create new policy
                await client.query(
                    ...
                    [companyId, data.text_template, data.diagonal, data.opacity,
                        data.font_size, data.font_color, userId]
                );
            }
     ...
     */
    private async validateObjectExists(client: any, scope: string, objectId: string): Promise<void> {
        switch (scope) {
            case 'ATTEST_PACK':
