  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      const input = {
        ...
        present_ccy: pick(
          r,
          map.present_ccy ?? 'present_ccy',
          defs.present_ccy
        ),
        in_service: pick(r, map.in_service ?? 'in_service'),
        life_m: r[map.life_m ?? 'life_m']
          ? Number(r[map.life_m ?? 'life_m'])
          : undefined,
        method: (r[map.method ?? 'method'] || defs.method || 'SL') as
          | 'SL'
          | 'DDB',
 ...
 */
export async function validateCapexCsvStructure(
  text: string
