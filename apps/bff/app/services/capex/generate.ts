...
function monthsBetween(startISO: string, months: number): Array<{ year: number; month: number }> {
...
export async function generateSchedules(
    ...
    const plansResult = await pool.query(plansQuery, planId ? [companyId, planId] : [companyId]);
    ...
    for (const plan of plans) {
        ...
        if (assetClassResult.rows.length === 0) {
            console.warn(`Asset class ${plan.asset_class} not found for plan ${plan.id}`);
            continue;
        }
        ...
        if (rows.length > 0) {
            ...
            ON CONFLICT (company_id, plan_id, year, month) DO NOTHING`,
                        [row.id, row.company_id, row.plan_id, row.year, row.month, row.currency, row.present_ccy, row.amount]
                    );
                }
...
export async function getUnbookedSchedules(companyId: string, year: number, month: number) {
    const result = await pool.query(
