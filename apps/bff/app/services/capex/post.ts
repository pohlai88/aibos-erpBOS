...
export async function postDepreciation(
  ...
  for (const [currentPlanId, planSchedules] of schedulesByPlan) {
    ...
    if (!postingMap) {
      console.warn(
        `No posting map found for asset class ${plan.asset_class}, skipping`
      );
      continue;
    }

    // Calculate total depreciation for this plan
    const totalAmount = planSchedules.reduce(
    ...
    const journalEntry: JournalEntry = {
      ...
      memo:
        memo ??
        `Depreciation ${year}-${String(month).padStart(2, '0')} ${plan.description}`,
      lines: journalLines,
      tags: {
        module: 'capex',
    ...
    if (!dryRun) {
      try {
        ...
        console.error(
          `Failed to post journal for plan ${currentPlanId}:`,
          error
        );
        throw error;
      }
