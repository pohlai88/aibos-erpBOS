...
import { attestTask, attestCampaign, outbox } from '@aibos/db-adapter/schema';
import type { AttestSlaStateType } from '@aibos/contracts';
...
export class AttestSlaService {
   ...
   */
  private async emitSlaEvent(
    taskId: string,
    companyId: string,
    slaState: AttestSlaStateType
  ): Promise<void> {
    let topic: string;
