...
export class AttestCampaignService {
  ...
  async issueCampaign(
    ...
    await this.dbInstance
      ...
      .onConflictDoUpdate({
        target: [
          attestCampaign.companyId,
          attestCampaign.programId,
          attestCampaign.period,
        ],
        set: {
    ...
    await this.createTasksForCampaign(
      campaignId,
      companyId,
      userId,
      campaignData.dueAt
  ...
  async listCampaigns(
    ...
    campaigns: CampaignResponseType[];
    total: number;
    hasMore: boolean;
  }> {
    // Get total count
    ...
    const campaignResponses: CampaignResponseType[] = campaigns.map(
      campaign => ({
        id: campaign.id,
  ...
  private async createTasksForCampaign(
    ...
    const assignments = await this.dbInstance
      ...
      .innerJoin(
        attestCampaign,
        eq(attestAssignment.programId, attestCampaign.programId)
      )
      .where(
        and(
   ...
   */
  private async emitCampaignIssuedEvent(
    campaignId: string,
    companyId: string
  ): Promise<void> {
    await this.dbInstance.insert(outbox).values({
   ...
   */
  private async emitCampaignClosedEvent(
    campaignId: string,
    companyId: string
  ): Promise<void> {
    await this.dbInstance.insert(outbox).values({
