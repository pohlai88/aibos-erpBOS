...
export class AttestPackService {
     ...
     */
    async buildPack(taskId: string, companyId: string, signerId: string): Promise<PackResponseType> {
        ...
        const [template] = await this.dbInstance
            .select()
            .from(attestTemplate)
            .innerJoin(attestCampaign, eq(attestTemplate.id, attestCampaign.templateId))
     ...
     * Get pack by task ID
     */
    async getPack(taskId: string, companyId: string): Promise<PackResponseType | null> {
        const [pack] = await this.dbInstance
            .select()
            .from(attestPack)
            .innerJoin(attestTask, eq(attestPack.taskId, attestTask.id))
     ...
     */
    private async emitPackSignedEvent(taskId: string, companyId: string, signerId: string, sha256: string): Promise<void> {
        await this.dbInstance
            .insert(outbox)
            .values({
                id: ulid(),
