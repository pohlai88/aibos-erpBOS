...
export class AttestProgramsService {
    ...
    async upsertTemplate(
        ...
        await this.dbInstance
            ...
            .values(templateData)
            .onConflictDoUpdate({
                target: [attestTemplate.companyId, attestTemplate.code, attestTemplate.version],
                set: {
    ...
    async listTemplates(
        ...
        limit: number = 100,
        offset: number = 0
    ): Promise<{ templates: AttestTemplateResponseType[]; total: number; hasMore: boolean }> {
        // Get total count
        const [totalResult] = await this.dbInstance
            .select({ count: count() })
            .from(attestTemplate)
        ...
        const templateResponses: AttestTemplateResponseType[] = templates.map(template => ({
            id: template.id,
    ...
    async upsertAssignment(
        ...
        await this.dbInstance
            ...
            .onConflictDoUpdate({
                target: [attestAssignment.companyId, attestAssignment.programId, attestAssignment.scopeKey, attestAssignment.assigneeId],
                set: {
    ...
    async listAssignments(
        ...
        limit: number = 100,
        offset: number = 0
    ): Promise<{ assignments: AttestAssignmentResponseType[]; total: number; hasMore: boolean }> {
        ...
        if (programCode) {
            ...
            const assignments = await this.dbInstance
                ...
                .from(attestAssignment)
                .innerJoin(attestProgram, eq(attestAssignment.programId, attestProgram.id))
                .where(and(
            ...
            const assignmentResponses: AttestAssignmentResponseType[] = assignments.map(assignment => ({
                id: assignment.id,
