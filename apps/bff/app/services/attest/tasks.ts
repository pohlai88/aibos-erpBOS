...
export class AttestTasksService {
    ...
    async listTasks(
        ...
        const tasks = await this.dbInstance
            ...
            .from(attestTask)
            .where(and(...conditions))
            .orderBy(desc(attestTask.slaState), desc(attestTask.dueAt), asc(attestTask.createdAt))
            .limit(query.limit)
            .offset(query.offset);
     ...
     */
    private async emitTaskSubmittedEvent(taskId: string, companyId: string, userId: string): Promise<void> {
        await this.dbInstance
            .insert(outbox)
            .values({
                id: ulid(),
     ...
     */
    private async emitTaskReturnedEvent(taskId: string, companyId: string, userId: string, reason: string): Promise<void> {
        await this.dbInstance
            .insert(outbox)
            .values({
                id: ulid(),
     ...
     */
    private async emitTaskApprovedEvent(taskId: string, companyId: string, userId: string): Promise<void> {
        await this.dbInstance
            .insert(outbox)
            .values({
                id: ulid(),
