...
import { attestProgram, attestTemplate, attestAssignment, attestCampaign, attestTask } from "@aibos/db-adapter/schema";
...
describe("M26.7 Attestations Portal", () => {
    ...
    beforeEach(async () => {
        ...
        await db.delete(attestCampaign).where(eq(attestCampaign.companyId, testCompanyId));
        await db.delete(attestAssignment).where(eq(attestAssignment.companyId, testCompanyId));
        await db.delete(attestTemplate).where(eq(attestTemplate.companyId, testCompanyId));
        await db.delete(attestProgram).where(eq(attestProgram.companyId, testCompanyId));
    });
    ...
    describe("AttestProgramsService", () => {
        ...
        it("should create and list attestation assignments", async () => {
            ...
            await programsService.upsertProgram(testCompanyId, testUserId, programData);
    ...
    describe("AttestCampaignService", () => {
        it("should issue a campaign and create tasks", async () => {
            ...
            await programsService.upsertTemplate(testCompanyId, testUserId, templateData);
            ...
            await programsService.upsertAssignment(testCompanyId, testUserId, assignmentData);
    ...
    describe("AttestTasksService", () => {
        it("should submit a task response", async () => {
            ...
            const campaign = await campaignService.issueCampaign(testCompanyId, testUserId, {
                programCode: "TEST-302",
                templateCode: "TEST-TEMPLATE",
                period: "2025-Q1",
                dueAt: "2025-04-15T16:00:00Z",
