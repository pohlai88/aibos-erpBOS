...
export class ArCashApplicationService {
  ...
  async runCashApplication(
    ...
    return {
      ...
      auto_matched: results.reduce(
        (sum, r) => sum + (r.status === 'matched' ? 1 : 0),
        0
      ),
      partial_matches: results.reduce(
        (sum, r) => sum + (r.status === 'partial' ? 1 : 0),
        0
      ),
      unmatched: results.reduce(
        (sum, r) => sum + (r.status === 'unmatched' ? 1 : 0),
        0
      ),
