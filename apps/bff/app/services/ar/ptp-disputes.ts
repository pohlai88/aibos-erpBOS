...
import { arPtp, arDispute } from '@aibos/db-adapter/schema';
import type {
  PtpCreateType,
  PtpResolveType,
  DisputeCreateType,
...
export class ArPtpDisputesService {
  ...
  async resolvePtp(
    ...
    await this.dbInstance
      ...
      .where(and(eq(arPtp.id, req.id), eq(arPtp.companyId, companyId)));
  ...
  async resolveDispute(
    ...
    await this.dbInstance
      ...
      .where(and(eq(arDispute.id, req.id), eq(arDispute.companyId, companyId)));
  ...
  async getCustomerPtps(
    ...
    const ptps = await this.dbInstance
      ...
      .from(arPtp)
      .where(
        and(eq(arPtp.companyId, companyId), eq(arPtp.customerId, customerId))
      )
