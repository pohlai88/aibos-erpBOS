...
export class ArPortalLedgerService {
  ...
  async getCustomerLedger(
    req: PortalLedgerReqType
  ): Promise<PortalLedgerResType> {
    // Validate token
    const tokenRecord = await this.dbInstance
      .select()
      .from(arPortalLedgerToken)
    ...
    const openingBalance =
      filteredLines.length > 0
        ? Number(filteredLines[0]!.balance) -
          Number(filteredLines[0]!.debit) +
          Number(filteredLines[0]!.credit)
        : 0;
    const closingBalance =
      filteredLines.length > 0
        ? Number(filteredLines[filteredLines.length - 1]!.balance)
        : 0;
    ...
    return {
      ...
      latest_statement: {
        pdf_url: pdfArtifact
          ? `/api/ar/statements/artifacts/${pdfArtifact.id}`
          : undefined,
        csv_url: csvArtifact
          ? `/api/ar/statements/artifacts/${csvArtifact.id}`
          : undefined,
        as_of_date: run.asOfDate,
      },
    };
  ...
  async getLatestStatementArtifacts(token: string): Promise<{
    ...
    return {
      ...(pdfArtifact?.id && {
        pdf_url: `/api/ar/statements/artifacts/${pdfArtifact.id}`,
      }),
      ...(csvArtifact?.id && {
        csv_url: `/api/ar/statements/artifacts/${csvArtifact.id}`,
      }),
