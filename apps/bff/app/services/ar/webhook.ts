...
export class ArWebhookService {
    ...
    async processWebhook(
        ...
        try {
            ...
            await this.addToDlq(companyId, req, error instanceof Error ? error.message : 'Unknown error');
            return {
                success: false,
                processed: false,
    ...
    private async processStripeWebhook(
        ...
        if (eventType === 'payment_intent.succeeded') {
            return await this.handlePaymentSuccess(companyId, paymentIntent as Record<string, unknown>);
        } else if (eventType === 'payment_intent.payment_failed') {
            return await this.handlePaymentFailure(companyId, paymentIntent as Record<string, unknown>);
        } else if (eventType === 'charge.dispute.created') {
            return await this.handleChargeback(companyId, (payload.data as Record<string, unknown>)?.object as Record<string, unknown>);
        }
