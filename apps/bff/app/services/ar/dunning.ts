...
export class ArDunningService {
  ...
  async runDunning(
    ...
    try {
      ...
      for (const [key, invoices] of buckets) {
        ...
        const policies = await this.getDunningPolicy(
          companyId,
          customerId,
          bucket
        );
        ...
        const customerName: string =
          invoices[0]?.customerName || 'Unknown Customer';
   ...
   */
  async buildAgingBuckets(
    companyId: string,
    asOfDate: string
  ): Promise<Map<string, ArInvoice[]>> {
  ...
  async getAllDunningPolicies(
    ...
    const policies = await this.dbInstance
      ...
      .orderBy(
        arDunningPolicy.policyCode,
        arDunningPolicy.fromBucket,
        arDunningPolicy.stepIdx
      );
