...
export class ArStatementService {
     ...
     */
    async getFinanceChargePolicy(companyId: string): Promise<FinanceChargePolicyUpsertType | null> {
    ...
    async runStatementGeneration(
        ...
        try {
            ...
            const ledgerLines = await this.composeCustomerLedgers(companyId, asOfDate, req.present);

            // Calculate finance charges if enabled
            const financeChargeLines = await this.calculateFinanceCharges(companyId, asOfDate, req.present);
            ...
            const allLines = [...ledgerLines, ...financeChargeLines].sort((a, b) => {
                ...
                const dateA = typeof a.docDate === 'string' ? new Date(a.docDate) : a.docDate;
                const dateB = typeof b.docDate === 'string' ? new Date(b.docDate) : b.docDate;
            ...
            for (const line of allLines) {
                try {
                    await this.dbInstance.insert(arStatementLine).values({
                        ...
                        dueDate: line.dueDate ? line.dueDate.toISOString().split('T')[0]! : null,
                        ref: line.ref,
                        memo: line.memo,
                    ...
                    errors.push(`Failed to insert line for customer ${line.customerId}: ${error}`);
                }
            }
            ...
            if (req.include_pdf || req.include_csv) {
                for (const customerId of customers) {
                    try {
                        ...
                        errors.push(`Failed to generate artifacts for customer ${customerId}: ${error}`);
                    }
                }
            }

            // Update run status
            const status = errors.length > 0 ? "error" : (req.finalize ? "finalized" : "draft");
            await this.dbInstance
    ...
    private async calculateFinanceCharges(
        ...
        for (const invoice of overdueInvoices) {
            ...
            const principal = Number(invoice.grossAmount) - Number(invoice.paidAmount);
            ...
            if (principal > 0 && daysOverdue > p.graceDays) {
                ...
                if (p.compMethod === "simple") {
                    ...
                    // Daily compounding
                    charge = principal * Math.pow(1 + Number(p.annualPct) / 365, daysOverdue) - principal;
                }
    ...
    private calculateAgingBucket(dueDate: Date | string, asOfDate: Date): string {
        ...
        const daysPastDue = Math.floor((asOfDate.getTime() - due.getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    private async generateStatementPDF(runId: string, customerId: string): Promise<void> {
     ...
     */
    private async generateStatementCSV(runId: string, customerId: string): Promise<void> {
        const artifactId = ulid();
    ...
    async sendStatementEmails(
        ...
        for (const customer of customers) {
            try {
                ...
                emailsFailed++;
                errors.push(`Failed to send email to customer ${customer.customerId}: ${error}`);
            }
        }
    ...
    async downloadStatementArtifact(
        ...
        const artifact = await this.dbInstance
            .select()
            .from(arStatementArtifact)
            .innerJoin(arStatementRun, eq(arStatementArtifact.runId, arStatementRun.id))
            .where(
                and(
