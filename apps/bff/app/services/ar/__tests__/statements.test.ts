...
describe('AR Customer Statements & Portal Ledger Service (M24.3)', () => {
    ...
    beforeEach(async () => {
        // Clean up test data
        await db.execute(sql`DELETE FROM ar_statement_email WHERE run_id IN (SELECT id FROM ar_statement_run WHERE company_id = ${testCompanyId})`);
        await db.execute(sql`DELETE FROM ar_statement_artifact WHERE run_id IN (SELECT id FROM ar_statement_run WHERE company_id = ${testCompanyId})`);
        await db.delete(arStatementLine).where(eq(arStatementLine.companyId, testCompanyId));
        await db.delete(arStatementRun).where(eq(arStatementRun.companyId, testCompanyId));
        await db.delete(arPortalLedgerToken).where(eq(arPortalLedgerToken.companyId, testCompanyId));
        await db.delete(arFinanceChargePolicy).where(eq(arFinanceChargePolicy.companyId, testCompanyId));
        await db.delete(arCashAppLink).where(sql`cash_app_id IN (SELECT id FROM ar_cash_app WHERE company_id = ${testCompanyId})`);
    ...
    describe('Finance Charge Policy Management', () => {
        it('should create and update finance charge policy', async () => {
            ...
            expect(Number(savedPolicy[0]!.minFee)).toBe(5.00);
    ...
    describe('Portal Ledger Access', () => {
        it('should generate and validate portal ledger token', async () => {
            ...
            const { token, expiresAt } = await portalLedgerService.generateLedgerToken(
                testCompanyId,
                testCustomerId,
                testUserId,
                60
        ...
        it('should reject invalid or expired tokens', async () => {
            ...
            await expect(portalLedgerService.getCustomerLedger(ledgerRequest))
    ...
    describe('Performance Tests', () => {
        it('should generate statement for 100 customers within 5 seconds', async () => {
            ...
            const customers = Array.from({ length: 100 }, (_, i) => `perf-customer-${i}`);
