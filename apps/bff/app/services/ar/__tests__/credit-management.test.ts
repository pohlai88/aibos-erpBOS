...
describe('ArCreditManagementService', () => {
  ...
  beforeEach(async () => {
    // Clean up test data
    await db
      .delete(arCollectionsKpi)
      .where(eq(arCollectionsKpi.companyId, testCompanyId));
    await db
      .delete(arCollectionsNote)
      .where(eq(arCollectionsNote.companyId, testCompanyId));
    await db
      .delete(arCreditHoldLog)
      .where(eq(arCreditHoldLog.companyId, testCompanyId));
    await db
      .delete(arCustomerCredit)
      .where(eq(arCustomerCredit.companyId, testCompanyId));
    await db
      .delete(arCreditPolicy)
      .where(eq(arCreditPolicy.companyId, testCompanyId));
  });
  ...
  describe('calculateExposureAndDso', () => {
    it('should calculate exposure and DSO correctly', async () => {
      ...
      const result = await service.calculateExposureAndDso(
        testCompanyId,
        testCustomerId,
        asOfDate
      );
  ...
  describe('addCollectionsNote', () => {
    it('should add collections note successfully', async () => {
      ...
      await service.addCollectionsNote(
        testCompanyId,
        {
  ...
  describe('generateKpiSnapshot', () => {
    it('should generate KPI snapshot', async () => {
      ...
      const result = await service.generateKpiSnapshot(
        testCompanyId,
        '2024-01-15'
      );
