...
describe('AR Portal & Pay-Now Service (M24.2) - Fixed', () => {
  ...
  beforeEach(async () => {
    ...
    await db
      .delete(arCheckoutIntent)
      .where(eq(arCheckoutIntent.companyId, testCompanyId));
    await db
      .delete(arSavedMethod)
      .where(eq(arSavedMethod.companyId, testCompanyId));
    await db
      .delete(arSurchargePolicy)
      .where(eq(arSurchargePolicy.companyId, testCompanyId));
    await db
  ...
  describe('Surcharge Policy Management', () => {
    it('should create surcharge policy', async () => {
      ...
      expect(Number(policy[0]?.minFee)).toBe(1.0);
      expect(Number(policy[0]?.capFee)).toBe(10.0);
    });
  });
