    ...
    try {
      ...
      for (const customer of customers) {
        try {
          ...
          // Calculate metrics
          const metrics = await this.calculateExposureAndDso(
            companyId,
            customerId,
            asOfDate
          );
          ...
          if (metrics.exposure > Number(credit.creditLimit)) {
            breaches.push(
              `Exposure ${metrics.exposure} exceeds limit ${credit.creditLimit}`
            );
          }
          ...
          if (
            metrics.dso >
            Number(policy.dsoTarget) + Number(policy.graceDays)
          ) {
            breaches.push(
              `DSO ${metrics.dso} exceeds target ${policy.dsoTarget} + grace ${policy.graceDays}`
            );
          }
          ...
          if (
            credit.riskScore &&
            Number(credit.riskScore) * Number(policy.riskWeight) > 0.7
          ) {
            breaches.push(
              `Risk score ${credit.riskScore} * weight ${policy.riskWeight} > 0.7`
            );
          }
          ...
          if (metrics.ptpOpen > Number(policy.ptpTolerance)) {
            breaches.push(
              `Open PTPs ${metrics.ptpOpen} exceed tolerance ${policy.ptpTolerance}`
            );
          }
          ...
          console.error(
            `Error evaluating customer ${customer.customer_id}:`,
            error
          );
          errors++;
        }
