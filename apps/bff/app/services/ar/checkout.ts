...
export class ArCheckoutService {
  ...
  async createIntent(
    ...
    // Calculate surcharge
    const surcharge = await this.surchargeService.calculateSurcharge(
      companyId,
      totalAmount
    );
   ...
   */
  private async processCashApplication(companyId: string, intent: any) {
    const invoices =
      typeof intent.invoices === 'string'
  ...
  private async sendReceiptEmail(
    ...
    try {
      ...
      const customerEmail =
        (await this.getCustomerEmail(intent.customerId)) ||
        `customer-${intent.customerId}@example.com`;
      ...
      const emailResult = await emailService.sendReceipt(
        ...
        currency,
        receiptUrl,
        (await this.getCustomerName(intent.customerId)) ||
  ...
  async listIntents(
    ...
    return {
      intents: intents.map(intent => ({
        ...
        invoices: intent.invoices as Array<{
          invoice_id: string;
          amount: number;
        }>,
      })),
