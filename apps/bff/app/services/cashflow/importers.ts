    ...
    for (const record of records) {
      try {
        ...
        const driverData: CfDriverWeekUpsertType = {
          year: parseInt(
            recordObj[mapping.year || 'year'] || recordObj['year'] || '0'
          ),
          iso_week: parseInt(
            recordObj[mapping.iso_week || 'iso_week'] ||
              recordObj['iso_week'] ||
              '0'
          ),
          driver_code:
            recordObj[mapping.driver_code || 'driver_code'] ||
            recordObj['driver_code'] ||
            '',
          cost_center:
            recordObj[mapping.cost_center || 'cost_center'] ||
            recordObj['cost_center'],
          project:
            recordObj[mapping.project || 'project'] || recordObj['project'],
          amount: parseFloat(
            recordObj[mapping.amount || 'amount'] || recordObj['amount'] || '0'
          ),
          scenario,
        };
        ...
        if (driverData.iso_week < 1 || driverData.iso_week > 53) {
          errors.push(
            `Row ${rowsProcessed}: Invalid ISO week ${driverData.iso_week}`
          );
          rowsSkipped++;
          continue;
        }
        ...
        errors.push(
          `Row ${rowsProcessed}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
        rowsSkipped++;
      }
    }
  ...
  try {
    ...
    for (const record of records) {
      try {
        ...
        const txnData = {
          txnDate:
            recordObj[mapping.txn_date || 'txn_date'] ||
            recordObj['txn_date'] ||
            '',
          amount: parseFloat(
            recordObj[mapping.amount || 'amount'] || recordObj['amount'] || '0'
        ...
        const uniqHash = createTransactionHash(
          acctCode,
          txnData.txnDate,
          txnData.amount,
          txnData.memo
