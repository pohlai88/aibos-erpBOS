...
export async function upsertCfMap(
    ...
    `, [companyId, data.map_code, data.account_like, data.cf_section, data.sign, data.note, updatedBy]);
...
export async function getCfMaps(companyId: string, mapCode?: string): Promise<CfMap[]> {
    let query = `
        SELECT map_code, account_like, cf_section, sign, note, updated_at, updated_by
        FROM cf_map
        WHERE company_id = $1
...
export async function runIndirectCashFlow(
    ...
    if (!data.dry_run) {
        ...
        if (lockRows.length > 0) {
            throw new Error(`Indirect cash flow already committed for ${data.year}-${data.month}`);
        }
    }
    ...
    `, [runId, companyId, 'INDIRECT', data.year, data.month, data.dry_run ? 'dry_run' : 'commit', data.present, data.scenario, actor]);
...
async function buildIndirectCashFlowLines(
    ...
    const currentBalances = await getGlBalancesForPeriod(companyId, data.year, data.month);
    const previousBalances = await getGlBalancesForPeriod(companyId, data.year, data.month - 1);
...
async function getGlBalancesForPeriod(companyId: string, year: number, month: number): Promise<any[]> {
...
function getAccountBalance(balances: any[], accountPattern: string): number {
    ...
    return matchingBalances.reduce((sum, balance) => sum + Number(balance.balance), 0);
}
...
function calculateNetIncome(balances: any[]): number {
    ...
    const totalRevenue = revenueBalances.reduce((sum, b) => sum + Number(b.balance), 0);
    const totalExpenses = expenseBalances.reduce((sum, b) => sum + Number(b.balance), 0);
...
function calculateCashFlowSummary(lines: CfLine[]): {
    ...
    const operatingCashFlow = operatingLines.reduce((sum, l) => sum + l.amount, 0);
    const investingCashFlow = investingLines.reduce((sum, l) => sum + l.amount, 0);
    const financingCashFlow = financingLines.reduce((sum, l) => sum + l.amount, 0);
...
export async function runDirect13WeekCashFlow(
    ...
    if (!data.dry_run) {
        ...
        if (lockRows.length > 0) {
            throw new Error(`13-week direct cash flow already committed for ${lockKey}`);
        }
    }
    ...
    await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
    `, [runId, companyId, 'DIRECT13', year, data.start_date, data.dry_run ? 'dry_run' : 'commit', data.present, data.scenario, actor]);
...
async function buildDirect13WeekCashFlowLines(
    ...
    for (let week = 0; week < data.weeks; week++) {
        ...
        const receipts = await getWeeklyReceipts(companyId, year, isoWeek, data.scenario);
        ...
        const payments = await getWeeklyPayments(companyId, year, isoWeek, data.scenario);
        ...
        const adjustments = await getWeeklyAdjustments(companyId, year, isoWeek, data.scenario);
...
async function getStartingCashBalance(companyId: string, startDate: Date): Promise<number> {
...
export async function runConsolidatedCashFlow(
    ...
    `, [
        ...
        data.present, data.scenario, actor
    ]);
    ...
    for (const entity of entityRows) {
        ...
        const convertedLines = data.present && data.present !== entity.ccy
            ? await convertLinesToPresentationCurrency(entityLines, entity.ccy, data.present, companyId)
            : entityLines;
...
async function buildEntityIndirectCashFlowLines(
    ...
    const currentBalances = await getEntityGlBalances(entityCode, data.year, data.month);
...
async function buildEntityDirect13WeekCashFlowLines(
    ...
    for (let week = 0; week < data.weeks; week++) {
        ...
        const receipts = await getEntityWeeklyReceipts(entityCode, year, isoWeek, data.scenario);
        if (Math.abs(receipts) > 0.01) {
            lines.push({
                id: ulid(),
                runId,
        ...
        const payments = await getEntityWeeklyPayments(entityCode, year, isoWeek, data.scenario);
        if (Math.abs(payments) > 0.01) {
            lines.push({
                id: ulid(),
                runId,
...
async function convertLinesToPresentationCurrency(
    ...
    return lines.map(line => ({
        ...
        note: line.note ? `${line.note} (converted from ${sourceCcy})` : `Converted from ${sourceCcy}`
    }));
}
