...
export class ControlsRunnerService {
  ...
  async executeControlRun(
    ...
    await this.dbInstance.insert(ctrlRun).values({
      id: runId,
      companyId,
      controlId,
    ...
    try {
      ...
      const resultId = ulid();
      await this.dbInstance.insert(ctrlResult).values({
        id: resultId,
        ctrlRunId: runId,
        status: result.status,
      ...
      for (const exception of result.exceptions) {
        ...
        await this.dbInstance.insert(ctrlException).values({
          id: exceptionId,
          ctrlRunId: runId,
          code: exception.code,
   ...
   */
  private async executeScriptControl(
    control: any,
    companyId: string
   ...
   */
  private async executeSqlControl(
    control: any,
    companyId: string
  ): Promise<ControlResult> {
    ...
    try {
      ...
      const materialityThreshold =
        control.autoConfig?.materiality_threshold || 0;
      ...
      for (const result of resultsArray) {
        if (
          result.variance &&
          Math.abs(result.variance) > materialityThreshold
        ) {
          exceptions.push({
   ...
   */
  private async executePolicyControl(
    control: any,
    companyId: string
  ): Promise<ControlResult> {
  ...
  async getControlRun(
    ...
    const runs = await this.dbInstance
      .select()
      .from(ctrlRun)
      .where(and(eq(ctrlRun.id, runId), eq(ctrlRun.companyId, companyId)))
      .limit(1);
  ...
  async addEvidence(
    ...
    const runs = await this.dbInstance
      ...
      .from(ctrlRun)
      .where(
        and(eq(ctrlRun.id, data.ctrl_run_id), eq(ctrlRun.companyId, companyId))
      )
      .limit(1);
    ...
    await this.dbInstance.insert(ctrlEvidence).values(evidenceData);
