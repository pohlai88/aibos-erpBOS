...
export class ControlsExceptionsService {
  ...
  async updateException(
    ...
    const exceptions = await this.dbInstance
      ...
      .where(
        and(eq(ctrlException.id, data.id), eq(ctrlRun.companyId, companyId))
      )
      .limit(1);
    ...
    if (
      data.remediation_state === 'RESOLVED' &&
      exception.remediationState !== 'RESOLVED'
    ) {
  ...
  async getException(
    ...
    const exceptions = await this.dbInstance
      ...
      .innerJoin(ctrlRun, eq(ctrlException.ctrlRunId, ctrlRun.id))
      .where(
        and(eq(ctrlException.id, exceptionId), eq(ctrlRun.companyId, companyId))
      )
      .limit(1);
  ...
  async queryExceptions(
    ...
    if (query.remediation_state) {
      conditions.push(
        eq(ctrlException.remediationState, query.remediation_state)
      );
    }
  ...
  async escalateAgingExceptions(
    ...
    const agingArray = Array.isArray(agingExceptions)
      ? agingExceptions
      : [agingExceptions];
