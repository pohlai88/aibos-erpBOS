...
import { journalEntries, journalLines, account } from "@aibos/db-adapter/schema";
...
export async function subledgerTieout(
    ...
    try {
        ...
        const mockGlBalance = 100000 + (Math.random() > 0.5 ? 1 : -1) * Math.random() * 2000;
...
export async function bankReconDiff(
    ...
    try {
        ...
        const mockGlCashBalance = 500000 + (Math.random() > 0.5 ? 1 : -1) * Math.random() * 1000;
    ...
    try {
        ...
        if (!lockExists) {
            exceptions.push({
                ...
                message: 'FX Revaluation has not been performed or locked for the period',
                material: true
            });
        }
...
export async function rpoRollforward(
    ...
    try {
        ...
        const expectedClosingBalance = openingBalance + bookings - recognition - modifications;
        const actualClosingBalance = expectedClosingBalance + (Math.random() > 0.5 ? 1 : -1) * Math.random() * 10000;
...
export async function fluxCommentsRequired(
    ...
    try {
        ...
        return {
            status: exceptions.length > 0 ? 'FAIL' : 'PASS',
            detail: {
                material_flux_lines_without_comments: materialFluxWithoutComments.length,
                materiality_threshold: materialityThreshold,
...
export async function cashflowBridge(
    ...
    try {
        ...
        const directCashDelta = indirectCashDelta + (Math.random() > 0.5 ? 1 : -1) * Math.random() * 3000;
