...
export class CertificationsService {
    ...
    async upsertCertTemplate(
        companyId: string,
        userId: string,
        data: CertStatementUpsert
    ): Promise<CertStatementResponseType> {
     ...
     */
    async getCertTemplate(companyId: string, templateId: string): Promise<CertTemplateResponseType> {
    ...
    async signCertification(
        ...
        if (existingSignoffs.length > 0) {
            throw new Error("Certification sign-off already exists for this run/level/role combination");
        }
        ...
        const signoffData = {
            ...
            statementText: statement.text, // Snapshot of statement text at time of signing
     ...
     */
    async getCertSignoff(companyId: string, signoffId: string): Promise<CertSignoffResponseType> {
        const signoffs = await this.dbInstance
            .select()
            .from(certSignoff)
            .where(and(
                eq(certSignoff.id, signoffId),
                eq(certSignoff.companyId, companyId)
            ))
            .limit(1);
     ...
     */
    async getCertificationStatus(companyId: string, runId: string): Promise<{
        run_id: string;
        entity_level: {
        ...
        const signoffs = await this.dbInstance
            ...
            .from(certSignoff)
            .where(and(
                eq(certSignoff.runId, runId),
                eq(certSignoff.companyId, companyId)
            ));

        const entitySignoffs = signoffs.filter(s => s.level === "ENTITY");
        const consolidatedSignoffs = signoffs.filter(s => s.level === "CONSOLIDATED");

        const managerSignoff = entitySignoffs.find(s => s.signerRole === "MANAGER");
        const controllerSignoff = entitySignoffs.find(s => s.signerRole === "CONTROLLER");
