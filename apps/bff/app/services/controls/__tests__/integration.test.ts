...
describe("M26.1 Auto-Controls & Certifications", () => {
    ...
    describe("ControlsAdminService", () => {
        it("should create and query controls", async () => {
            ...
            const control = await adminService.upsertControl(companyId, userId, controlData);
        ...
        it("should create and query assignments", async () => {
            ...
            const assignment = await adminService.upsertAssignment(companyId, userId, assignmentData);
    ...
    describe("ControlsRunnerService", () => {
        it("should execute control runs and handle results", async () => {
            ...
            const run = await runnerService.executeControlRun(companyId, userId, runData);
        ...
        it("should add evidence to control runs", async () => {
            ...
            const evidence = await runnerService.addEvidence(companyId, userId, evidenceData);
    ...
    describe("ControlsExceptionsService", () => {
        ...
        it("should provide open exceptions summary", async () => {
            const summary = await exceptionsService.getOpenExceptionsSummary(companyId);
    ...
    describe("CertificationsService", () => {
        it("should manage certification templates", async () => {
            ...
            const template = await certsService.upsertCertTemplate(companyId, userId, templateData);

            expect(template.code).toBe("MANAGER_STD");
            expect(template.text).toBe("I certify that the financial statements are accurate and complete.");
        ...
        it("should handle certification sign-offs", async () => {
            ...
            const template = await certsService.upsertCertTemplate(companyId, userId, {
                code: "MANAGER_STD",
                text: "I certify that the financial statements are accurate and complete.",
                level: "ENTITY" as const,
                active: true
            ...
            const signoff = await certsService.signCertification(companyId, userId, signoffData);
        ...
        it("should provide certification status", async () => {
            const status = await certsService.getCertificationStatus(companyId, runId);
