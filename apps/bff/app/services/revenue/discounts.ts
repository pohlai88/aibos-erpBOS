...
export class RevDiscountService {
    ...
    async queryDiscountRules(
        ...
        if (query.effective_to) {
            conditions.push(lte(revDiscountRule.effectiveTo || revDiscountRule.effectiveFrom, query.effective_to));
        }
        ...
        const rules = await this.dbInstance
            ...
            .orderBy(desc(revDiscountRule.priority), desc(revDiscountRule.effectiveFrom), desc(revDiscountRule.createdAt))
            .limit(query.limit)
            .offset(query.offset);
    ...
    validateDiscountRuleParams(kind: string, params: any): boolean {
        switch (kind) {
            case "PROP":
                return typeof params.pct === "number" && params.pct >= 0 && params.pct <= 1;
            case "RESIDUAL":
                return typeof params.pct === "number" && params.pct >= 0 && params.pct <= 1 &&
                    Array.isArray(params.residual_products);
            case "TIERED":
                return typeof params.threshold === "number" && params.threshold > 0 &&
                    typeof params.pct === "number" && params.pct >= 0 && params.pct <= 1;
            case "PROMO":
                return typeof params.pct === "number" && params.pct >= 0 && params.pct <= 1 &&
                    typeof params.start_date === "string" && typeof params.end_date === "string";
            case "PARTNER":
                return typeof params.pct === "number" && params.pct >= 0 && params.pct <= 1 &&
    ...
    private mapRuleToResponse(rule: any): DiscountRuleResponseType {
        return {
            ...
            max_usage_amount: rule.maxUsageAmount ? parseFloat(rule.maxUsageAmount) : undefined,
