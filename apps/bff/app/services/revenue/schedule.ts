...
import { revSchedule, revPob } from '@aibos/db-adapter/schema';
import type {
...
export class RevScheduleService {
  ...
  async buildSchedule(
    ...
    const pobs = await this.dbInstance
      .select()
      .from(revPob)
      .where(and(eq(revPob.companyId, companyId), eq(revPob.id, data.pob_id)))
      .limit(1);
    ...
    const endDate = data.end_date
      ...
      : new Date(pob.endDate || startDate);
  ...
  private calculateSchedule(
    ...
    switch (method) {
      ...
      case 'RATABLE_DAILY': {
        ...
        const dailyAmount = this.calculateDailyRatable(
          amount,
          startDate,
          endDate
        );
      ...
      case 'RATABLE_MONTHLY': {
        ...
        const monthlyAmount = this.calculateMonthlyRatable(
          amount,
          startDate,
          endDate
        );
   ...
   */
  private calculateMonthlyRatable(
    totalAmount: number,
    startDate: Date,
    endDate: Date
  ): number {
