...
export class RevRecognitionService {
  ...
  async runRecognition(
    ...
    if (!data.dry_run) {
      ...
      if (existingLocks.length > 0) {
        throw new Error(
          `Period ${data.year}-${data.month.toString().padStart(2, '0')} is already posted`
        );
      }
    }
    ...
    for (const schedule of schedules) {
      ...
      const pobs = await this.dbInstance
        ...
        .from(revPob)
        .where(
          and(eq(revPob.companyId, companyId), eq(revPob.id, schedule.pobId))
        )
        .limit(1);
      ...
      const lineId = ulid();
      const drAccount = isUnbilled
        ? policy.unbilledArAccount
        : policy.deferredRevAccount;
      ...
      await this.dbInstance
        ...
        .set({
          ...
          status:
            plannedAmount === recognizedAmount + deltaAmount
              ? 'DONE'
              : 'PARTIAL',
          updatedAt: new Date(),
        })
  ...
  async getRecognitionLines(
    ...
    const lines = await this.dbInstance
      ...
      .from(revRecLine)
      .where(
        and(eq(revRecLine.companyId, companyId), eq(revRecLine.runId, runId))
      )
