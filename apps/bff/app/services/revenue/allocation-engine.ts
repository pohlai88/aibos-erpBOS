...
export class RevAllocationEngineService {
    ...
    private async allocateByRelativeSsp(
        ...
        const sspValues = await Promise.all(
            invoiceData.lines.map(async (line: any) => {
                ...
                if (!compliance.compliant) {
                    corridorFlags.push(`${line.product_id}: variance ${(compliance.variance! * 100).toFixed(1)}%`);
                }
        ...
        const totalSspWeighted = sspValues.reduce((sum, item) => sum + item.ssp * item.amount, 0);
        ...
        for (const line of invoiceData.lines) {
            ...
            const roundedAmount = this.roundAmount(allocatedAmount, sspPolicy.rounding);
    ...
    private async allocateByResidual(
        ...
        const nonResidualLines = invoiceData.lines.filter((line: any) =>
        ...
        for (const line of nonResidualLines) {
            ...
            if (ssp) {
                const allocatedAmount = Math.min(line.amount, netAmount - allocatedToNonResidual);
                const roundedAmount = this.roundAmount(allocatedAmount, sspPolicy.rounding);
        ...
        if (remainingAmount > 0 && residualLines.length > 0) {
            ...
            for (const line of residualLines) {
                const roundedAmount = this.roundAmount(amountPerResidualProduct, sspPolicy.rounding);
            ...
            if (oldSsp !== newSsp) {
                ...
                if (!data.dry_run) {
                    ...
                    await this.dbInstance
                        ...
                        .set({
                            ssp: newSsp.toString(),
                            allocatedAmount: (parseFloat(pob.allocatedAmount) + reallocationDelta).toString()
                        })
