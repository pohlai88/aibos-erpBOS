...
export class RevBundleService {
  ...
  async queryBundles(
    ...
    if (query.effective_to) {
      conditions.push(
        lte(
          revBundle.effectiveTo || revBundle.effectiveFrom,
          query.effective_to
        )
      );
    }
  ...
  async getBundle(
    ...
    const [bundle] = await this.dbInstance
      ...
      .from(revBundle)
      .where(
        and(eq(revBundle.id, bundleId), eq(revBundle.companyId, companyId))
      )
      .limit(1);
   ...
   */
  async validateBundleWeights(
    components: Array<{ weight_pct: number }>
  ): Promise<boolean> {
    const totalWeight = components.reduce(
      (sum, comp) => sum + comp.weight_pct,
      0
    );
  ...
  async getBundlesByProduct(
    ...
    const bundles = await this.dbInstance
      ...
      .innerJoin(
        revBundleComponent,
        eq(revBundle.id, revBundleComponent.bundleId)
      )
      .where(
        and(
          eq(revBundle.companyId, companyId),
    ...
    const result = Array.from(bundleMap.values()).map(
      ({ bundle, components }) => {
