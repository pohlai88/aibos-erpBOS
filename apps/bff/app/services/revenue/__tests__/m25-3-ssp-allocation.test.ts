...
describe('M25.3: SSP Catalog, Bundles & Discounts Allocation', () => {
  ...
  describe('SSP Admin Service', () => {
    ...
    it('should check corridor compliance', async () => {
      ...
      expect(result.variance).toBe(0.2);
    });
  });
  ...
  describe('Bundle Service', () => {
    it('should upsert bundle with components', async () => {
      ...
      const result = await bundleService.upsertBundle('company-1', 'user-1', {
        bundle_sku: 'BUNDLE-A',
        name: 'Premium Bundle',
        components: [
          {
  ...
  describe('Discount Service', () => {
    ...
    it('should validate discount rule parameters', () => {
      expect(
        discountService.validateDiscountRuleParams('PROP', { pct: 0.1 })
      ).toBe(true);
      expect(
        discountService.validateDiscountRuleParams('PROP', { pct: 1.5 })
      ).toBe(false);
      expect(
        discountService.validateDiscountRuleParams('TIERED', {
          threshold: 1000,
          pct: 0.1,
        })
      ).toBe(true);
  ...
  describe('Allocation Engine Service', () => {
    it('should determine allocation strategy correctly', async () => {
      ...
      const strategy1 = await allocationEngineService[
        'determineAllocationStrategy'
      ]('company-1', invoiceData, 'AUTO');
      ...
      const strategy2 = await allocationEngineService[
        'determineAllocationStrategy'
      ]('company-1', invoiceData, 'AUTO');
  ...
  describe('Alerts Service', () => {
    it('should check corridor breaches', async () => {
      ...
      expect(result.breaches[0]?.variance_pct).toBeCloseTo(0.2, 2); // 20% variance
    });
