...
import { eq, and, desc, sql, gte, lte, asc, isNull, inArray } from "drizzle-orm";
import {
    revSspCatalog,
    revSspEvidence,
    revSspPolicy,
...
export class RevSspAdminService {
    ...
    async querySspCatalog(
        ...
        if (query.effective_to) {
            conditions.push(lte(revSspCatalog.effectiveTo || revSspCatalog.effectiveFrom, query.effective_to));
        }
    ...
    private mapCatalogToResponse(catalog: any): SspCatalogResponseType {
        return {
            ...
            corridor_min_pct: catalog.corridorMinPct ? parseFloat(catalog.corridorMinPct) : undefined,
            corridor_max_pct: catalog.corridorMaxPct ? parseFloat(catalog.corridorMaxPct) : undefined,
