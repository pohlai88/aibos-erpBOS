...
export class RevEventsService {
     ...
     */
    async processEvents(companyId: string): Promise<{ processed: number; errors: number }> {
        // Get unprocessed events
        const events = await this.dbInstance
            .select()
            .from(revEvent)
            .where(and(
                eq(revEvent.companyId, companyId),
                isNull(revEvent.processedAt)
            ))
    ...
    async queryEvents(
        ...
        if (query.occurred_at_from) {
            conditions.push(gte(revEvent.occurredAt, new Date(query.occurred_at_from)));
        }
