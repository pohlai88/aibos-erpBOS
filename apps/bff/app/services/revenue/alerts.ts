...
export class RevAlertsService {
  ...
  async checkCorridorBreaches(companyId: string): Promise<{
    ...
    const currencyGroups = sspEntries.reduce(
      (groups, entry) => {
  ...
  async generateSspStateSnapshot(companyId: string): Promise<{
    ...
    const methods = sspEntries.reduce(
      (acc, entry) => {
  ...
  async getAllocationAuditSummary(
    ...
    const avgProcessingTime =
      audits.reduce((sum, a) => sum + (a.processingTimeMs || 0), 0) /
        totalAllocations || 0;

    const methodBreakdown = audits.reduce(
      (acc, audit) => {
    ...
    const totalAmountAllocated = audits.reduce(
      (sum, audit) => sum + parseFloat(audit.totalAllocatedAmount),
      0
    );
  ...
  async checkSspPolicyCompliance(companyId: string): Promise<{
    ...
    const productsWithoutSsp = await this.dbInstance
      ...
      .where(and(eq(rbProduct.companyId, companyId), isNull(revSspCatalog.id)));
