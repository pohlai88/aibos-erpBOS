...
export class RevRpoService {
  ...
  async createSnapshot(
    ...
    const rpoAmounts = await this.calculateRPOAmounts(
      companyId,
      data.as_of_date
    );
    ...
    await this.dbInstance
      ...
      .onConflictDoUpdate({
        target: [
          revRpoSnapshot.companyId,
          revRpoSnapshot.asOfDate,
          revRpoSnapshot.currency,
        ],
        set: {
   ...
   * Calculate RPO amounts from open POBs
   */
  private async calculateRPOAmounts(
    companyId: string,
    asOfDate: string
  ): Promise<{
    total: number;
...
export class RevArtifactsService {
  ...
  async exportRun(
    ...
    const runs = await this.dbInstance
      ...
      .from(revRecRun)
      .where(
        and(eq(revRecRun.companyId, companyId), eq(revRecRun.id, data.run_id))
      )
      .limit(1);
