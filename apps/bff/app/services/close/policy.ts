...
export class ClosePolicyService {
     ...
     */
    async getClosePolicy(companyId: string): Promise<ClosePolicyResponseType | null> {
     ...
     */
    async computeMaterialityThreshold(companyId: string, baseAmount: number): Promise<{
        absThreshold: number;
        pctThreshold: number;
        isMaterial: boolean;
    }> {
        ...
        const isMaterial = Math.abs(baseAmount) >= absThreshold || Math.abs(baseAmount) >= pctThreshold;
