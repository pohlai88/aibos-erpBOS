...
export class CloseBoardService {
  ...
  async listBoard(
    ...
    const items = await this.dbInstance
      ...
      .where(and(...conditions))
      .orderBy(
        desc(closeItem.slaState),
        desc(closeItem.agingDays),
        asc(closeItem.dueAt)
      )
      .limit(query.limit)
      .offset(query.offset);
  ...
  async bulkCreateFromSources(
    ...
    try {
      ...
      if (sources.includes('controls')) {
        const controlsResult = await this.ingestFromControls(
          companyId,
          userId,
          period
        );
      ...
      if (sources.includes('deficiencies')) {
        const deficiencyResult = await this.ingestFromDeficiencies(
          companyId,
          userId,
          period
        );
      ...
      if (sources.includes('certs')) {
        const certResult = await this.ingestFromCerts(
          companyId,
          userId,
          period
        );
      ...
      if (sources.includes('tasks')) {
        const taskResult = await this.ingestFromTasks(
          companyId,
          userId,
          period
        );
      ...
      errors.push(
        `Bulk ingest failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  ...
  async addComment(
    ...
    await this.dbInstance.insert(closeItemComment).values(commentData);
  ...
  async linkEvidence(
    ...
    await this.dbInstance.insert(closeItemEvdLink).values(linkData);
