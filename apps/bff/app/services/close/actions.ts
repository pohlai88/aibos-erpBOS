...
import { closeItem, closeItemAction, outbox } from '@aibos/db-adapter/schema';
import type {
  CloseBulkActionType,
  ...
  async applyAction(
    ...
    for (const itemId of data.itemIds) {
      try {
        ...
        await this.dbInstance.insert(closeItemAction).values(actionData);

        // Update item based on action
        await this.updateItemFromAction(
          itemId,
          data.action,
          data.payload,
          userId
        ...
        console.error(
          `Failed to apply action ${data.action} to item ${itemId}:`,
          error
        );
