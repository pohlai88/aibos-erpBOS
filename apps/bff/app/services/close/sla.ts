...
export class CloseSlaService {
     ...
     */
    async getSlaPolicy(companyId: string, code: string = "MONTH_END"): Promise<SlaPolicyResponseType | null> {
        const [result] = await this.dbInstance
            .select()
            .from(closeSlaPolicy)
            .where(and(
     ...
     */
    async tickSla(companyId: string, period: string): Promise<{
        updated: number;
        escalated: number;
        events: string[];
    }> {
        ...
        for (const item of items) {
            ...
            const hoursOverdue = Math.max(0, (now.getTime() - dueDate.getTime()) / (1000 * 60 * 60));
     ...
     */
    private async emitEscalationEvent(item: any, policy: SlaPolicyResponseType): Promise<void> {
        const eventData = {
            itemId: item.id,
            title: item.title,
     ...
     */
    async getSlaMetrics(companyId: string, period: string): Promise<{
        totalItems: number;
        onTime: number;
        late: number;
        escalated: number;
