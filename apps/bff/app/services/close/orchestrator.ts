...
export class CloseOrchestratorService {
  ...
  async createCloseRun(
    ...
    if (existingRun.length > 0) {
      throw new Error(
        `Close run already exists for period ${data.year}-${data.month}`
      );
    }
  ...
  private async createDefaultTasks(
    ...
    const defaultTasks = [
      ...
      {
        code: 'AP_AGING',
        title: 'Accounts Payable Aging Review',
        priority: 8,
        evidence_required: true,
      },
      {
   ...
   * Query close runs
   */
  async queryCloseRuns(
    companyId: string,
    query: CloseRunQueryType
  ): Promise<CloseRunResponseType[]> {
  ...
  async upsertCloseTask(
    ...
    const existingTask = await db
      .select()
      .from(closeTask)
      .where(and(eq(closeTask.runId, runId), eq(closeTask.code, data.code)))
      .limit(1);
   ...
   * Query close tasks
   */
  async queryCloseTasks(
    companyId: string,
    query: CloseTaskQueryType
  ): Promise<CloseTaskResponseType[]> {
    let whereConditions = [];
   ...
   */
  async queryCloseEvidence(
    companyId: string,
    query: CloseEvidenceQueryType
  ): Promise<CloseEvidenceResponseType[]> {
    let whereConditions = [];
  ...
  async lockEntity(
    companyId: string,
    userId: string,
    data: CloseLockRequestType
  ): Promise<CloseLockResponseType> {
  ...
  async queryKpis(
    ...
    if (query.computed_at_from !== undefined) {
      whereConditions.push(
        sql`${closeKpi.computedAt} >= ${query.computed_at_from}`
      );
    }
    if (query.computed_at_to !== undefined) {
      whereConditions.push(
        sql`${closeKpi.computedAt} <= ${query.computed_at_to}`
      );
    }
