...
export class LeaseRestorationService {
  ...
  private calculatePresentValue(
    ...
    const yearsToRestoration = Math.max(
      0,
      (leaseEnd.getTime() - asOf.getTime()) / (365.25 * 24 * 60 * 60 * 1000)
    );
  ...
  async postRestoration(
    ...
    const leaseData = await this.dbInstance
      ...
      .from(lease)
      .where(
        and(eq(lease.id, restoration.leaseId), eq(lease.companyId, companyId))
      )
      .limit(1);
    ...
    const movements = await this.calculateRestorationMovements(
      restoration,
      year,
      month
    );
    ...
    const journalLines = this.generateRestorationJournalEntries(
      restoration,
      movements,
      year,
      month
  ...
  private async calculateRestorationMovements(
    ...
    const previousPeriod = await this.getPreviousPeriodBalance(
      restoration.id,
      year,
      month
    );
    ...
    const unwind = this.calculateUnwindInterest(
      previousPeriod,
      Number(restoration.discountRate)
    );
  ...
  async queryRestorations(
    ...
    if (query.as_of_date_from) {
      additionalConditions.push(
        gte(leaseRestoration.asOfDate, query.as_of_date_from)
      );
    }

    if (query.as_of_date_to) {
      additionalConditions.push(
        lte(leaseRestoration.asOfDate, query.as_of_date_to)
      );
    }
