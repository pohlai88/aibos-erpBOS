  ...
  describe('Performance Requirements', () => {
    ...
    it('should build onerous roll for 2k items p95 < 1.2s', async () => {
      ...
      const promises = mockAssessments.map(async assessment => {
        return onerousRollService.buildRoll(mockCompanyId, mockUserId, {
    ...
    it('should handle concurrent operations efficiently', async () => {
      ...
      const concurrentPromises = [
        ...
        ...mockCGUs.map(cgu =>
          cguAllocator.calculateCGUCarryingAmounts(
            mockCompanyId,
            cgu.id,
            '2024-01-01'
          )
      ...
      console.log(
        `Concurrent operations completed in ${duration.toFixed(2)}ms`
      );
    });
    ...
    it('should handle memory efficiently with large datasets', async () => {
      ...
      console.log(
        `Large dataset processing completed in ${duration.toFixed(2)}ms`
      );
    });
  });
});
