...
import type { SubleaseCreateReqType, SlbCreateReqType } from '@aibos/contracts';
...
describe('M28.5: Sublease & SLB Services', () => {
  ...
  afterEach(async () => {
    ...
    await db
      ...
      .where(eq(slbAllocation.slbId, 'test-slb-id'));
    await db.delete(slbTxn).where(eq(slbTxn.companyId, testCompanyId));
    await db
      .delete(subleaseSchedule)
      .where(eq(subleaseSchedule.subleaseId, 'test-sublease-id'));
    await db
      .delete(subleaseCf)
      .where(eq(subleaseCf.subleaseId, 'test-sublease-id'));
  ...
  describe('SubleaseScheduler', () => {
    ...
    it('should build finance sublease schedule', async () => {
      const result = await service.buildSchedule(testCompanyId, testUserId, {
        subleaseId,
      });
  ...
  describe('EvidencePackService', () => {
    ...
    it('should create evidence pack for sublease', async () => {
      ...
      const retrievedPackId = await service.getSubleaseEvidencePack(
        testCompanyId,
        subleaseId
      );
    ...
    it('should create evidence pack for SLB transaction', async () => {
      ...
      // Verify evidence pack was linked
      const retrievedPackId = await service.getSlbEvidencePack(
        testCompanyId,
        slbId
      );
      expect(retrievedPackId).toBe(evidencePackId);
