...
import {
  ImpairIndicatorService,
  ImpairMeasureService,
  ImpairPostingService,
} from '../impairment';
...
describe('ImpairMeasureService', () => {
  ...
  afterEach(async () => {
    ...
    await db
      .delete(leaseImpTest)
      .where(eq(leaseImpTest.companyId, testCompanyId));
    await db
  ...
  describe('assessImpairment', () => {
    it('should assess impairment for component level', async () => {
      ...
      const result = await service.assessImpairment(
        testCompanyId,
        testUserId,
        data
      );
    ...
    it('should throw error for missing component IDs at component level', async () => {
      ...
      await expect(
        ...
        'Component IDs are required for component-level testing'
      );
    });
  ...
  describe('getImpairmentTestDetail', () => {
    it('should return detailed test information', async () => {
      ...
      const testDetail = await service.getImpairmentTestDetail(
        testCompanyId,
        result.test_id
      );
...
describe('ImpairPostingService', () => {
  ...
  afterEach(async () => {
    ...
    await db
      ...
      .where(eq(leaseImpPostLock.companyId, testCompanyId));
    await db
      .delete(leaseImpPost)
      .where(eq(leaseImpPost.companyId, testCompanyId));
    await db.delete(leaseImpLine).where(eq(leaseImpLine.impairTestId, ulid()));
    await db
      .delete(leaseImpTest)
      .where(eq(leaseImpTest.companyId, testCompanyId));
  ...
  describe('postImpairment', () => {
    it('should post impairment charges', async () => {
      ...
      const assessResult = await measureService.assessImpairment(
        testCompanyId,
        testUserId,
        assessData
      );
      ...
      const result = await service.postImpairment(
        testCompanyId,
        testUserId,
        postData
      );
