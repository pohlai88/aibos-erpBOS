...
export class CGUAllocator {
    ...
    async calculateCGUCarryingAmounts(
        ...
        const components = await this.dbInstance
            ...
            .from(leaseCguLink)
            .innerJoin(leaseComponent, eq(leaseCguLink.leaseComponentId, leaseComponent.id))
    ...
    async validateCGUAllocation(cguId: string): Promise<{
        ...
        const totalWeight = allocations.reduce((sum, alloc) => sum + Number(alloc.weight), 0);
    ...
    async getCGUSummary(
        ...
        const cguData = await this.dbInstance
            ...
            .from(leaseCgu)
            .where(and(
                eq(leaseCgu.companyId, companyId),
                eq(leaseCgu.id, cguId)
            ))
            .limit(1);
        ...
        const carryingAmounts = await this.calculateCGUCarryingAmounts(companyId, cguId, asOfDate);
        const totalCarryingAmount = carryingAmounts.reduce((sum, comp) => sum + comp.carryingAmount, 0);
