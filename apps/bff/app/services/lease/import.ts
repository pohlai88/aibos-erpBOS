...
export class LeaseImportService {
    ...
    async importFromCSV(
        ...
        if (contentHash) {
            ...
            if (existingImport.length > 0) {
                return { imported: 0, skipped: 0, errors: ["Import already processed"] };
            }
        }
     ...
     */
    private mapRowToLease(row: Record<string, string>, mapping: LeaseImportReqType['mapping']): LeaseUpsertType {
        return {
            ...
            payment_frequency: this.mapPaymentFrequency(row[mapping.payment_frequency] || ''),
     ...
     */
    private mapRowToCashflows(row: Record<string, string>, mapping: LeaseImportReqType['mapping']): LeaseCashflowRowType[] {
        try {
            ...
            throw new Error(`Invalid cashflows format: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
     ...
     */
    private validateLeaseData(leaseData: LeaseUpsertType, cashflows: LeaseCashflowRowType[]): void {
     ...
     */
    private mapAssetClass(assetClass: string): 'Land/Building' | 'IT/Equipment' | 'Vehicles' | 'Others' {
        const normalized = assetClass.toLowerCase().trim();

        if (normalized.includes('land') || normalized.includes('building') || normalized.includes('office')) {
            return 'Land/Building';
        }
        if (normalized.includes('it') || normalized.includes('equipment') || normalized.includes('computer')) {
            return 'IT/Equipment';
        }
        if (normalized.includes('vehicle') || normalized.includes('car') || normalized.includes('truck')) {
            return 'Vehicles';
        }
     ...
     */
    private mapPaymentFrequency(frequency: string): 'MONTHLY' | 'QUARTERLY' | 'ANNUALLY' {
