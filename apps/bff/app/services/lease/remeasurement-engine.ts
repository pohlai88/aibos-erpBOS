...
export class RemeasurementEngine {
    ...
    async applyRemeasurement(
        ...
        const preCarrying = await this.snapshotCarryingAmounts(companyId, modification.leaseId, components);
        ...
        switch (modification.kind) {
            case 'INDEXATION':
                ({ postCarrying, deltas, scheduleDeltas } = await this.calculateIndexationImpact(
                    companyId, modification, modLines, components, preCarrying
                ));
                break;
            case 'CONCESSION':
                ({ postCarrying, deltas, scheduleDeltas } = await this.calculateConcessionImpact(
                    companyId, modification, modLines, components, preCarrying
                ));
                break;
            case 'SCOPE':
                ({ postCarrying, deltas, scheduleDeltas } = await this.calculateScopeImpact(
                    companyId, modification, modLines, components, preCarrying
                ));
                break;
            case 'TERM':
                ({ postCarrying, deltas, scheduleDeltas } = await this.calculateTermImpact(
                    companyId, modification, modLines, components, preCarrying
                ));
                break;
            default:
    ...
    async postRemeasurement(
        ...
        try {
            ...
            // Calculate totals
            const totalLiabilityDelta = deltas.reduce((sum, delta) => sum + Number(delta.liabDelta), 0);
            const totalRouDelta = deltas.reduce((sum, delta) => sum + Number(delta.rouDelta), 0);
    ...
    private async calculateIndexationImpact(
        ...
        const postCarrying = {
            ...
            component_carryings: preCarrying.component_carryings.map((comp: any) => ({
                ...comp,
                carrying_amount: comp.carrying_amount + (totalIndexationImpact * Number(comp.carrying_amount) / preCarrying.total_rou)
            }))
        };
        ...
        const deltas = {
            ...
            component_deltas: preCarrying.component_carryings.map((comp: any) => ({
                ...
                liability_delta: totalIndexationImpact * Number(comp.carrying_amount) / preCarrying.total_rou,
                rou_delta: totalIndexationImpact * Number(comp.carrying_amount) / preCarrying.total_rou
            }))
        };
        ...
        for (const comp of preCarrying.component_carryings) {
            const componentDelta = totalIndexationImpact * Number(comp.carrying_amount) / preCarrying.total_rou;
            scheduleDeltas.push({
                component_id: comp.component_id,
                year: currentYear,
                month: currentMonth,
    ...
    private async calculateConcessionImpact(
        ...
        const postCarrying = {
            ...
            component_carryings: preCarrying.component_carryings.map((comp: any) => ({
                ...comp,
                carrying_amount: comp.carrying_amount + (totalConcessionImpact * Number(comp.carrying_amount) / preCarrying.total_rou)
            }))
        };
        ...
        const deltas = {
            ...
            component_deltas: preCarrying.component_carryings.map((comp: any) => ({
                component_id: comp.component_id,
                component_code: comp.component_code,
                liability_delta: totalConcessionImpact * Number(comp.carrying_amount) / preCarrying.total_rou,
                rou_delta: totalConcessionImpact * Number(comp.carrying_amount) / preCarrying.total_rou
            }))
        };
        ...
        for (const comp of preCarrying.component_carryings) {
            const componentDelta = totalConcessionImpact * Number(comp.carrying_amount) / preCarrying.total_rou;
            scheduleDeltas.push({
                component_id: comp.component_id,
                year: currentYear,
                month: currentMonth,
    ...
    private async calculateScopeImpact(
        ...
        const postCarrying = {
            ...
            component_carryings: preCarrying.component_carryings.map((comp: any) => ({
                ...comp,
                carrying_amount: comp.carrying_amount + (totalScopeImpact * Number(comp.carrying_amount) / preCarrying.total_rou)
            }))
        };
        ...
        const deltas = {
            ...
            component_deltas: preCarrying.component_carryings.map((comp: any) => ({
                component_id: comp.component_id,
                component_code: comp.component_code,
                liability_delta: totalScopeImpact * Number(comp.carrying_amount) / preCarrying.total_rou,
                rou_delta: totalScopeImpact * Number(comp.carrying_amount) / preCarrying.total_rou
            }))
        };
        ...
        for (const comp of preCarrying.component_carryings) {
            const componentDelta = totalScopeImpact * Number(comp.carrying_amount) / preCarrying.total_rou;
            scheduleDeltas.push({
                component_id: comp.component_id,
                year: currentYear,
                month: currentMonth,
     ...
     * Create schedule deltas
     */
    private async createScheduleDeltas(modId: string, scheduleDeltas: any[]): Promise<void> {
