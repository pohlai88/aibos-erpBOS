...
export class ConcessionService {
    ...
    async applyConcession(
        ...
        if (policy.method === 'STRAIGHT_LINE') {
            await this.applyStraightLineConcession(modId, modLines, components, policy);
        } else {
            await this.applyTrueModificationConcession(modId, modLines, components, policy);
        }
        ...
        for (const line of modLines) {
            if (line.action === 'RENT_FREE' || line.action === 'DEFERRAL') {
                ...
                if (policy.componentAlloc === 'PRORATA') {
                    ...
                    const totalRou = components.reduce((sum, comp) => sum + Number(comp.pctOfRou), 0);
