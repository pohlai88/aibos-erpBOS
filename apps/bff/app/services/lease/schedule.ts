...
export class LeaseScheduleService {
    ...
    async buildSchedule(leaseId: string): Promise<void> {
        ...
        for (let month = 0; month < totalMonths; month++) {
            ...
            let fxReval = 0;
            if (leaseRecord.presentCcy && leaseRecord.presentCcy !== leaseRecord.ccy) {
    ...
    async generateMaturityAnalysis(
        ...
        const maturityBands = [
            { band: "≤1y", undiscounted_amount: 0, discounted_amount: 0, lease_count: 0 },
            { band: "1-2y", undiscounted_amount: 0, discounted_amount: 0, lease_count: 0 },
            { band: "2-3y", undiscounted_amount: 0, discounted_amount: 0, lease_count: 0 },
            { band: "3-5y", undiscounted_amount: 0, discounted_amount: 0, lease_count: 0 },
            { band: ">5y", undiscounted_amount: 0, discounted_amount: 0, lease_count: 0 }
        ];
    ...
    private async calculateFXRevaluation(
        ...
        try {
            ...
            const rate = rates.find(r =>
                r.src_ccy === functionalCcy && r.dst_ccy === presentationCcy
            );

            if (!rate) {
                console.warn(`FX rate not found for ${functionalCcy} to ${presentationCcy} on ${date.toISOString()}`);
                return 0;
            }
    ...
    async getFXRate(
        ...
        try {
            ...
            const rate = rates.find(r =>
                r.src_ccy === fromCcy && r.dst_ccy === toCcy
            );
