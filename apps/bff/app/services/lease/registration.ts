...
export class LeaseRegistrationService {
  ...
  async upsertLease(
    ...
    const termMonths =
      (endDate.getFullYear() - commenceDate.getFullYear()) * 12 +
    ...
    if (!isShortTerm && !data.low_value_exempt) {
      await this.calculateOpeningMeasures(
        leaseId,
        userId,
        cashflows,
        data.discount_rate
   ...
   */
  async queryLeases(companyId: string, query: LeaseQueryType): Promise<any[]> {
    ...
    if (query.lessor) {
      whereConditions.push(
        sql`${lease.lessor} ILIKE ${'%' + query.lessor + '%'}`
      );
    }
