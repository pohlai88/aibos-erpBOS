...
export class LeaseOnerousRollService {
     ...
     */
    async createOnerousRoll(request: OnerousRollRequest): Promise<OnerousRollResponse> {
        try {
            ...
            for (const assessment of assessments) {
                const previousRoll = await this.getPreviousRoll(assessment.id, year, month);
            ...
            await this.dbInstance
                .insert(leaseOnerousRoll)
                .values(rollData.map(data => ({
                    id: ulid(),
            ...
            const journalResult = await this.postOnerousJournal(companyId, year, month, rollData, userId);
     ...
     */
    private async getPreviousRoll(assessmentId: string, year: number, month: number): Promise<OnerousRollData | null> {
     ...
     */
    private calculateCharge(assessment: any, year: number, month: number): number {
     ...
     */
    private calculateUnwind(assessment: any, year: number, month: number): number {
        // Implementation depends on business logic
        // For now, return a placeholder calculation
     ...
     */
    private calculateUtilization(assessment: any, year: number, month: number): number {
        // Implementation depends on business logic
        // For now, return a placeholder calculation
     ...
     */
    async getOnerousRollHistory(assessmentId: string): Promise<OnerousRollData[]> {
     ...
     */
    async getOnerousRollSummary(companyId: string, year: number, month: number): Promise<{
        totalOpening: number;
        totalCharge: number;
        totalUnwind: number;
        ...
        const rolls = await this.dbInstance
            ...
            .from(leaseOnerousRoll)
            .innerJoin(leaseOnerousAssessment, eq(leaseOnerousRoll.assessmentId, leaseOnerousAssessment.id))
            .where(and(
                eq(leaseOnerousAssessment.companyId, companyId),
                eq(leaseOnerousRoll.year, year),
                eq(leaseOnerousRoll.month, month)
        ...
        const summary = rolls.reduce((acc, roll) => {
            if (!roll) return acc;
            return {
    ...
    async buildRoll(
        ...
        try {
            ...
            const previousRoll = await this.getPreviousRoll(assessment_id, year, month);
            ...
            const rollData: OnerousRollData = {
                ...
                closingBalance: (previousRoll?.openingBalance || 0) + Number(assessment[0]?.provision || 0)
            };
