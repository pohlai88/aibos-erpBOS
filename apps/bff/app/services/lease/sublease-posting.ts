...
export class SubleasePostingService {
  ...
  async postMonthlyEntries(
    ...
    if (!data.dryRun) {
      await this.dbInstance.insert(subleasePostLock).values({
        id: ulid(),
        subleaseId: data.subleaseId,
        year: data.year,
        month: data.month,
  ...
  private async postFinanceSubleaseEntries(
    ...
    const template =
      this.journalTemplateService.getFinanceSubleaseMonthlyTemplate();
    ...
    const variables = {
      ...
      principal:
        Number(scheduleData.receipt) - Number(scheduleData.interestIncome),
    };
  ...
  private async postOperatingSubleaseEntries(
    ...
    const template =
      this.journalTemplateService.getOperatingSubleaseMonthlyTemplate();
