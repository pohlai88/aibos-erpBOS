...
export class LeaseRemeasureService {
  ...
  async applyEvent(
    ...
    if (artifact.outputs.delta_rou !== 0) {
      await this.dbInstance
        ...
        .set({
          initialRou: (
            Number(opening.initialRou) + artifact.outputs.delta_rou
          ).toString(),
          computedAt: new Date(),
          computedBy: userId,
        })
    ...
    if (
      artifact.outputs.new_discount_rate !== Number(leaseRecord.discountRate)
    ) {
  ...
  private async calculateRemeasurement(
    ...
    switch (event.kind) {
      ...
      case 'SCOPE':
        ...
        break;
      case 'TERMINATION':
        await this.calculateTerminationRemeasurement(
          inputs,
          calculations,
          outputs
        );
  ...
  private async calculateIndexRemeasurement(
    ...
    for (const cf of remainingCashflows) {
      ...
      const monthsFromEffective =
        (cfDate.getFullYear() - effectiveDateObj.getFullYear()) * 12 +
        (cfDate.getMonth() - effectiveDateObj.getMonth());

      const amount = Number(cf.amount);
      const presentValue =
        amount / Math.pow(1 + monthlyRate, monthsFromEffective);
    ...
    const rouAdjustment =
      (deltaLiability / inputs.current_liability) * inputs.current_rou;
  ...
  private async calculateScopeRemeasurement(
    ...
    if (!inputs.scope_change_pct) {
      throw new Error(
        'Scope change percentage is required for SCOPE remeasurement'
      );
    }
...
export class LeaseScheduleService {
  ...
  async rebuild(
    ...
    for (const cf of remainingCashflows) {
      const cfDate = new Date(cf.dueOn);
      const monthsFromRebuild =
        (cfDate.getFullYear() - rebuildDate.getFullYear()) * 12 +
        (cfDate.getMonth() - rebuildDate.getMonth());

      const amount = Number(cf.amount);
      const presentValue =
        amount / Math.pow(1 + monthlyRate, monthsFromRebuild);
      newPresentValue += presentValue;
    }
