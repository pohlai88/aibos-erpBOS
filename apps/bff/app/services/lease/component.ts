...
export class ComponentDesignService {
  ...
  async designFromAllocation(
    ...
    const leaseData = await this.dbInstance
      .select()
      .from(lease)
      .where(and(eq(lease.id, leaseId), eq(lease.companyId, companyId)))
      .limit(1);
    ...
    for (const split of splits) {
      ...
      if (split.pct_of_rou <= 0 || split.pct_of_rou > 1) {
        throw new Error(
          `Component ${split.code} percentage must be between 0 and 1, got ${split.pct_of_rou}`
        );
      }
      if (split.useful_life_months <= 0) {
        throw new Error(
          `Component ${split.code} useful life must be positive, got ${split.useful_life_months}`
        );
      }
    }
    const totalPct = splits.reduce(
      (sum: number, split: LeaseComponentDesignReqType['splits'][0]) =>
        sum + split.pct_of_rou,
      0
    );
    if (Math.abs(totalPct - 1.0) > 0.00001) {
      throw new Error(
        `Component percentages must sum to 1.00000, got ${totalPct.toFixed(5)}`
      );
    }
    ...
    for (const split of splits) {
      if (split.useful_life_months > leaseTermMonths) {
        throw new Error(
          `Component ${split.code} useful life (${split.useful_life_months} months) exceeds lease term (${leaseTermMonths} months)`
        );
      }
    }
  ...
  async updateComponentAllocation(
    ...
    if (updates.useful_life_months)
      updateData.usefulLifeMonths = updates.useful_life_months;
    if (updates.method) updateData.method = updates.method;
    if (updates.units_basis)
      updateData.unitsBasis = updates.units_basis.toString();
...
export class ComponentScheduleService {
  ...
  async buildSchedules(
    ...
    const totalLeaseAmort = leaseScheduleData.reduce(
      (sum: number, s: { rouAmort: string }) => sum + Number(s.rouAmort),
      0
    );
    ...
    return {
      ...
      reconciliation: {
        ...
        reconciliation_passed:
          Math.abs(totalComponentRou - totalLeaseRou) < 0.01,
      },
  ...
  private async buildComponentSchedule(
    ...
    const componentRou =
      Number(component.pctOfRou) * Number(opening.initialRou || 0);
   ...
   */
  private findPaymentForMonth(
    cashflows: Array<{ dueOn: string; amount: string }>,
    date: Date
  ): number {
  ...
  > {
    ...
    return await this.dbInstance
      ...
      .innerJoin(
        leaseComponent,
        eq(leaseComponentSched.leaseComponentId, leaseComponent.id)
      )
