...
export class ComponentDesignService {
    ...
    async designFromAllocation(
        ...
        // Validate percentage allocation sums to ~1.00000
        const totalPct = splits.reduce((sum: number, split: any) => sum + split.pct_of_rou, 0);
...
export class ComponentScheduleService {
    ...
    async buildSchedules(
        ...
        const totalLeaseAmort = leaseScheduleData.reduce((sum: number, s: any) => sum + Number(s.rouAmort), 0);
    ...
    private async buildComponentSchedule(
        ...
        const componentRou = Number(component.pctOfRou) * Number(leaseRecord.initialRou || 0);
