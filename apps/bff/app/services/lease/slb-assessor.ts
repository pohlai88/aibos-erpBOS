...
export class SlbAssessor {
  ...
  async createSlbTransaction(
    ...
    await this.dbInstance.insert(slbTxn).values({
      id: slbId,
      companyId,
    ...
    if (data.adjustments && data.adjustments.length > 0) {
      ...
      await this.dbInstance.insert(slbMeasure).values(adjustmentInserts);
    }
    ...
    try {
      await this.evidencePackService.createSlbEvidencePack(
        ...
        slbId,
        {
          ...
          leasebackAgreement: data.leasebackId
            ? `Leaseback agreement: ${data.leasebackId}`
            : 'No leaseback',
      ...
      console.warn(
        'Failed to create evidence pack for SLB transaction:',
        error
      );
  ...
  async getSlbDetail(
    ...
    const result = await this.dbInstance
      ...
      .from(slbTxn)
      .leftJoin(lease, eq(slbTxn.leasebackId, lease.id))
      .where(and(eq(slbTxn.id, slbId), eq(slbTxn.companyId, companyId)))
      .limit(1);
        ...
        allocation.length > 0
          ? {
              proportionTransferred: Number(
                allocation[0]!.proportionTransferred
              ),
