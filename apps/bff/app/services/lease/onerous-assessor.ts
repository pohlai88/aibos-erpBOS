...
export class OnerousAssessor {
  ...
  async upsertAssessment(
    ...
    const provision = Math.max(
      0,
      data.unavoidable_cost - data.expected_benefit
    );

    await this.dbInstance.insert(leaseOnerousAssessment).values({
      id: assessmentId,
      companyId,
   ...
   */
  async queryAssessments(companyId: string, query: OnerousAssessmentQueryType) {
    ...
    if (query.lease_component_id) {
      conditions.push(
        eq(leaseOnerousAssessment.leaseComponentId, query.lease_component_id)
      );
    }
    if (query.service_item) {
      conditions.push(
        sql`${leaseOnerousAssessment.serviceItem} ILIKE ${`%${query.service_item}%`}`
      );
    }
    if (query.status) {
    ...
    const assessments = await this.dbInstance
      ...
      .leftJoin(
        leaseComponent,
        eq(leaseOnerousAssessment.leaseComponentId, leaseComponent.id)
      )
      .leftJoin(lease, eq(leaseComponent.leaseId, lease.id))
      .where(and(...conditions))
      .orderBy(
        desc(leaseOnerousAssessment.asOfDate),
        desc(leaseOnerousAssessment.createdAt)
      )
      .limit(query.limit)
      .offset(query.offset);
  ...
  async updateAssessment(
    ...
    if (data.lease_component_id !== undefined)
      updateData.leaseComponentId = data.lease_component_id;
    if (data.service_item) updateData.serviceItem = data.service_item;
    if (data.term_months !== undefined)
      updateData.termMonths = data.term_months;
    if (data.unavoidable_cost !== undefined)
      updateData.unavoidableCost = data.unavoidable_cost;
    if (data.expected_benefit !== undefined)
      updateData.expectedBenefit = data.expected_benefit;

    // Recalculate provision if costs or benefits changed
    if (
      data.unavoidable_cost !== undefined ||
      data.expected_benefit !== undefined
    ) {
      const unavoidableCost =
        data.unavoidable_cost !== undefined
          ? data.unavoidable_cost
          : updateData.unavoidableCost;
      const expectedBenefit =
        data.expected_benefit !== undefined
          ? data.expected_benefit
          : updateData.expectedBenefit;
   ...
   */
  async releaseProvision(assessmentId: string, userId: string): Promise<void> {
    await this.dbInstance
      .update(leaseOnerousAssessment)
      .set({
        status: 'RELEASED',
   ...
   */
  async getAssessmentsSummary(companyId: string, asOfDate?: string) {
    const conditions = [eq(leaseOnerousAssessment.companyId, companyId)];
    ...
    const summary = await this.dbInstance
      .select({
        ...
        totalProvision:
          sql<number>`sum(${leaseOnerousAssessment.provision})`.as(
            'totalProvision'
          ),
        totalUnavoidableCost:
          sql<number>`sum(${leaseOnerousAssessment.unavoidableCost})`.as(
            'totalUnavoidableCost'
          ),
        totalExpectedBenefit:
          sql<number>`sum(${leaseOnerousAssessment.expectedBenefit})`.as(
            'totalExpectedBenefit'
          ),
      })
      .from(leaseOnerousAssessment)
