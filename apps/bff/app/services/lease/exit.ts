...
export class LeaseExitService {
    ...
    async upsertExit(
        ...
        await this.calculateExitAmounts(companyId, exitId, leaseRecord.id, componentId, data);
    ...
    private async calculateExitAmounts(
        ...
        const latestSchedule = await this.getLatestCarryingAmounts(leaseId, componentId);
        ...
        const gainLoss = derecogLiab - derecogRou + (data.settlement || 0) - (data.penalty || 0);
    ...
    private async storeFxInformation(
        ...
        const fxRates = await listRates(companyId, { year: new Date().getFullYear(), month: new Date().getMonth() + 1 });
    ...
    async postExit(
        ...
        const exitData = await this.dbInstance
            ...
            .from(leaseExit)
            .where(and(
                eq(leaseExit.id, exit_id),
                eq(leaseExit.companyId, companyId)
            ))
            .limit(1);
        ...
        // Check posting lock
        await this.checkPostingLock(companyId, exit.leaseId, exit.componentId, exit.eventDate);
        ...
        if (!dry_run) {
            try {
                // Set posting lock
                await this.setPostingLock(companyId, exit.leaseId, exit.componentId, exit.eventDate, 'POSTING');
                ...
                // Update posting lock
                await this.setPostingLock(companyId, exit.leaseId, exit.componentId, exit.eventDate, 'POSTED', journalId, userId);
                ...
                await this.setPostingLock(companyId, exit.leaseId, exit.componentId, exit.eventDate, 'ERROR', null, null, message);
            }
        } else {
     ...
     */
    private generateJournalEntries(exit: any, calc: any): Array<{
        account: string;
        debit: number;
        credit: number;
        memo: string;
