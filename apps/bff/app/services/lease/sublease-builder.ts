...
export class SubleaseBuilder {
  ...
  async createSublease(
    ...
    await this.dbInstance.insert(sublease).values({
      id: subleaseId,
      companyId,
    ...
    if (data.cashflows && data.cashflows.length > 0) {
      ...
      await this.dbInstance.insert(subleaseCf).values(cashflowInserts);
    }
    ...
    if (data.componentLinks && data.componentLinks.length > 0) {
      ...
      await this.dbInstance.insert(leaseComponentSublet).values(linkInserts);
    }
  ...
  async getSubleaseDetail(
    ...
    const result = await this.dbInstance
      ...
      .leftJoin(lease, eq(sublease.headLeaseId, lease.id))
      .where(
        and(eq(sublease.id, subleaseId), eq(sublease.companyId, companyId))
      )
      .limit(1);
    ...
    const componentLinks = await this.dbInstance
      ...
      .leftJoin(
        leaseComponent,
        eq(leaseComponentSublet.leaseComponentId, leaseComponent.id)
      )
  ...
  private async classifySublease(
    ...
    const subleaseTermMonths = this.getTotalMonths(
      new Date(data.startOn),
      new Date(data.endOn)
    );
    const headLeaseTermMonths = this.getTotalMonths(
      new Date(headLease.commenceOn),
      new Date(headLease.endOn)
    );
    ...
    const subleasePv = await this.calculatePresentValue(
      data.cashflows || [],
      data.discountRate || 0.05
    );
    ...
    if (
      substantiallyAllRisksRewards ||
      (majorPartOfEconomicLife && substantiallyAllFairValue)
    ) {
      return 'FINANCE';
    }
  ...
  private async calculateEffectiveRate(cashflows: any[]): Promise<number> {
    ...
    const totalAmount = cashflows.reduce(
    ...
    const annualRate =
      Math.pow(1 + totalAmount / 100000, 12 / avgTermMonths) - 1;
