...
export class LeaseRemeasureService {
    ...
    async generateDisclosure(
        ...
        const disclosureData = this.calculateDisclosureTotals(leases, components, schedules);

        // Insert disclosure record
        const disclosureId = ulid();
        await this.dbInstance
            .insert(leaseDisclosure)
            .values({
                id: disclosureId,
                companyId,
                year,
    ...
    private calculateImpairmentTotals(impairmentLines: any[]): {
        ...
        impairmentLines.forEach(line => {
            ...
            totals.method_breakdown[method] = (totals.method_breakdown[method] || 0) + loss;
        });
    ...
    async getComponentDisclosures(
        ...
        return {
            ...
            total_carrying_amount: componentDisclosures.reduce((sum, comp) => sum + comp.carrying_amount, 0),
            total_liability_amount: componentDisclosures.reduce((sum, comp) => sum + comp.liability_amount, 0),
            total_interest_expense: componentDisclosures.reduce((sum, comp) => sum + comp.interest_expense, 0),
            total_amortization_expense: componentDisclosures.reduce((sum, comp) => sum + comp.amortization_expense, 0),
     ...
     */
    async generateDisclosures(companyId: string, userId: string, data: LeaseDisclosureReqType): Promise<LeaseDisclosureResponseType> {
     ...
     */
    async getImpairmentDisclosures(companyId: string, userId: string, data: ImpairmentOnerousDisclosureReqType): Promise<ImpairmentOnerousDisclosureResponseType> {
     ...
     * Store disclosures
     */
    async storeDisclosures(companyId: string, userId: string, data: LeaseDisclosureReqType): Promise<string> {
     ...
     */
    async generateSubleaseSlbDisclosure(companyId: string, userId: string, data: any): Promise<any> {
        // Mock implementation
        return {
            sublease_summary: {},
            slb_summary: {},
     ...
     * Link evidence
     */
    async linkEvidence(companyId: string, userId: string, data: any): Promise<string> {
        // Mock implementation
        return ulid();
    }
}
