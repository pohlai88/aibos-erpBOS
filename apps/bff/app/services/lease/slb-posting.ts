...
export class SlbPostingService {
    ...
    async postInitialEntries(
        ...
        return {
            ...
            message: data.dryRun ? 'Dry run completed' : 'Initial SLB entries posted successfully'
        };
    }
    ...
    async postMonthlyEntries(
        ...
        return {
            ...
            journalId: data.dryRun ? null : journalId,
            status: data.dryRun ? 'DRY_RUN' : 'POSTED',
            message: data.dryRun ? 'Dry run completed' : 'Monthly SLB entries posted successfully'
        };
    }
    ...
    private async postSlbInitialJournal(
        ...
        const carryingAmount = Number(slbData.salePrice) - Number(allocationData.gainRecognized) - Number(allocationData.gainDeferred);
        journalLines.push({
            accountCode: 'PPE',
            dc: 'CR',
        ...
        if (slbData.leasebackId) {
            const leasebackLiability = await this.getLeasebackLiability(slbData.leasebackId);
    ...
    private async postSlbMonthlyJournal(
        ...
        const principal = Number(scheduleData.payment) - Number(scheduleData.interest);
        if (principal > 0) {
            journalLines.push({
                accountCode: 'LEASE_LIABILITY',
                dc: 'DR',
        ...
        const deferredGainUnwind = await this.calculateDeferredGainUnwind(allocationData, scheduleData);
     ...
     */
    private async calculateDeferredGainUnwind(allocationData: any, scheduleData: any): Promise<number> {
