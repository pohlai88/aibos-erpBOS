...
export class ImpairmentIndicatorService {
    ...
    async queryIndicators(
        ...
        if (query.lease_component_id) {
            conditions.push(eq(leaseImpIndicator.leaseComponentId, query.lease_component_id));
        }
        if (query.kind) {
        ...
        const indicators = await this.dbInstance
            ...
            .leftJoin(leaseComponent, eq(leaseImpIndicator.leaseComponentId, leaseComponent.id))
            .where(and(...conditions))
            .orderBy(desc(leaseImpIndicator.asOfDate), desc(leaseImpIndicator.createdAt))
            .limit(query.limit)
            .offset(query.offset);
    ...
    async collectIndicators(
        ...
        try {
            ...
            if (sources.includes('BUDGET_SYSTEM')) {
                const budgetIndicators = await this.collectBudgetIndicators(companyId, userId, asOfDate);
                collected += budgetIndicators;
            }

            // Collect vacancy indicators
            if (sources.includes('MARKET_DATA')) {
                const vacancyIndicators = await this.collectVacancyIndicators(companyId, userId, asOfDate);
                collected += vacancyIndicators;
            }

            // Collect market rent drop indicators
            if (sources.includes('MARKET_DATA')) {
                const marketIndicators = await this.collectMarketRentIndicators(companyId, userId, asOfDate);
                collected += marketIndicators;
            }

        } catch (error) {
            errors.push(`Error collecting indicators: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
