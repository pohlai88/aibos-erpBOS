...
export class ImpairmentPoster {
    ...
    async postImpairmentTest(
        ...
        if (!dryRun) {
            ...
            if (existingLock.length > 0) {
                throw new Error(`Impairment test ${testId} for period ${year}-${month.toString().padStart(2, '0')} is already posted`);
            }
        }
    ...
    async reverseImpairment(
        ...
        if (reversalAmount > Number(test[0]?.reversalCap || 0)) {
            throw new Error(`Reversal amount ${reversalAmount} exceeds reversal cap ${test[0]?.reversalCap}`);
        }
        ...
        for (const line of lines) {
            const lineReversalAmount = reversalAmount * (Number(line.loss || 0) / Number(test[0]?.loss || 1));
