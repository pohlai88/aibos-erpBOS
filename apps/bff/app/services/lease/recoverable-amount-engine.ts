...
import type { ImpairmentTestUpsertType } from '@aibos/contracts';
...
export class RecoverableAmountEngine {
  ...
  async createImpairmentTest(
    ...
    await this.createImpairmentLines(
      testId,
      userId,
      data.carrying_amount,
      loss
    );
  ...
  private async createImpairmentLines(
    ...
    for (const component of components) {
      ...
      const latestSchedule = await this.dbInstance
        ...
        .where(
          eq(leaseComponentSched.leaseComponentId, component.leaseComponentId)
        )
        .orderBy(
      ...
      if (latestSchedule.length > 0) {
        ...
        const componentCarryingAmount =
          Number(schedule.closingRou || 0) -
          Number(schedule.closingLiability || 0);
  ...
  async updateImpairmentTest(
    ...
    if (data.discount_rate !== undefined)
      updateData.discountRate = data.discount_rate;
    if (data.cashflows) updateData.cashflows = data.cashflows;
    if (data.carrying_amount !== undefined)
      updateData.carryingAmount = data.carrying_amount;
    if (data.reversal_cap !== undefined)
      updateData.reversalCap = data.reversal_cap;
    ...
    if (
      ...
      data.cashflows ||
      data.carrying_amount !== undefined
    ) {
      let recoverableAmount = 0;
