...
export class SubleaseScheduler {
    ...
    async buildSchedule(
        ...
        return {
            ...
            totalReceipts: scheduleRows.reduce((sum, row) => sum + Number(row.receipt), 0),
            totalInterestIncome: scheduleRows.reduce((sum, row) => sum + (Number(row.interestIncome) || 0), 0),
            totalLeaseIncome: scheduleRows.reduce((sum, row) => sum + (Number(row.leaseIncome) || 0), 0)
        };
    }
    ...
    private async buildFinanceSchedule(
        ...
        for (const cf of cashflows) {
            const monthsFromStart = this.getMonthsFromStart(startDate, new Date(cf.dueOn));
    ...
    private async buildOperatingSchedule(
        ...
        const totalReceipts = cashflows.reduce((sum, cf) => sum + Number(cf.amount), 0);
