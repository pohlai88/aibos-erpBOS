...
export class SlbMeasurer {
  ...
  async measureSlbTransaction(
    ...
    if (!slbData.controlTransferred) {
      throw new Error(
        'Cannot measure SLB transaction - control not transferred'
      );
    }
    ...
    await this.dbInstance.insert(slbAllocation).values({
      id: ulid(),
      slbId: data.slbId,
  ...
  private async calculateSlbAllocation(slbData: any): Promise<{
    ...
    const proportionTransferred =
      await this.calculateProportionTransferred(slbData);
    ...
    const adjustedGainRecognized =
      gainRecognized + fairValueAdjustment * proportionTransferred;
    const adjustedGainDeferred =
      gainDeferred + fairValueAdjustment * (1 - proportionTransferred);
  ...
  private async calculateProportionTransferred(slbData: any): Promise<number> {
    ...
    const timeProportion = Math.min(
      leasebackTermMonths / economicLifeMonths,
      1.0
    );

    // Adjust for leaseback payment terms
    const leasebackPayments = await this.getLeasebackPayments(
      slbData.leasebackId
    );
    const totalLeasebackPayments = leasebackPayments.reduce(
      (sum, payment) => sum + Number(payment.amount),
      0
    );
    const paymentProportion = Math.min(
      totalLeasebackPayments / Number(slbData.fmv),
      1.0
    );

    // Weighted average of time and payment proportions
    const proportionTransferred = Math.max(
      0,
      Math.min(1, timeProportion * 0.6 + paymentProportion * 0.4)
    );
