...
export class HeadLeaseAdjuster {
  ...
  async adjustHeadLease(
    ...
    for (const link of componentLinks) {
      const rouReduction =
        Number(link.componentPctOfRou) * Number(link.proportion);
    ...
    if (modLines.length > 0) {
      await this.dbInstance.insert(leaseModLine).values(modLines);
    }
    ...
    for (const link of componentLinks) {
      ...
      for (const scheduleRow of existingSchedule) {
        ...
        if (scheduleDate >= effectiveDateObj) {
          const rouReduction =
            Number(scheduleRow.rouAmort) * Number(link.proportion);
          const interestReduction =
            Number(scheduleRow.interest) * Number(link.proportion);
  ...
  async revertHeadLeaseAdjustment(
    ...
    if (reversalLines.length > 0) {
      await this.dbInstance.insert(leaseModLine).values(reversalLines);
    }
