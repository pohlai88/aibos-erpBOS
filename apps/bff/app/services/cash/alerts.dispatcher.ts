...
const MAX_COMPANIES_PER_BATCH = parseInt(process.env.MAX_COMPANIES_PER_BATCH || "50");
...
const transport = (SMTP_HOST && SMTP_PORT)
    ? createTransport({
        host: SMTP_HOST,
        port: SMTP_PORT,
...
async function withRetry<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`⚠️ Attempt ${attempt + 1} failed, retrying in ${delayMs}ms:`, error);
            await delay(delayMs);
        }
    }
...
export async function dispatchCashNotifications(
    ...
    if (transport && recips.length) {
        try {
            ...
            console.log(`📧 Email sent to ${recips.length} recipients for company ${companyId}`);
        } catch (error) {
            console.error(`❌ Email dispatch failed for company ${companyId} after ${MAX_RETRIES} retries:`, error);
        }
    }
    ...
    if (webhook) {
        try {
            ...
            console.error(`❌ Webhook dispatch failed for company ${companyId} after ${MAX_RETRIES} retries:`, error);
    ...
    if (mode === "noop") {
        console.log(`⚠️ No dispatch configured for company ${companyId} - ${breaches.length} breaches ignored`);
    }
...
export async function dispatchBatchCashNotifications(
    companies: Array<{ id: string; breaches: Breach[]; scenario?: string; delivery?: Delivery }>
): Promise<{ totalDispatched: number; results: Array<{ companyId: string; dispatched: number; mode: string }> }> {
    if (companies.length > MAX_COMPANIES_PER_BATCH) {
        console.warn(`⚠️ Batch size ${companies.length} exceeds limit ${MAX_COMPANIES_PER_BATCH}, processing in chunks`);
    }

    const results: Array<{ companyId: string; dispatched: number; mode: string }> = [];
    ...
    console.log(`📊 Batch dispatch complete: ${totalDispatched} notifications sent to ${companies.length} companies`);
