...
export interface ScheduleAuditEvent {
    type: "CashAlertScheduleCreated" | "CashAlertScheduleUpdated" | "CashAlertScheduleDeleted";
    company_id: string;
    actor: string;
    changes?: {
        enabled?: boolean;
...
export async function logScheduleAuditEvent(event: ScheduleAuditEvent): Promise<void> {
    try {
        await pool.query(
            `INSERT INTO outbox (topic, payload) VALUES ($1, $2)`,
            [event.type, JSON.stringify(event)]
        );
        console.log(`📝 Audit logged: ${event.type} for company ${event.company_id}`);
    } catch (error) {
