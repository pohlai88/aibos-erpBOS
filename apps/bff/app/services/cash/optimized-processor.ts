...
export class EnterpriseCashAlertsProcessor {
  ...
  constructor(
    private options: BatchProcessingOptions = ENTERPRISE_DEFAULT_OPTIONS
  ) {
  ...
  async processDailyAlerts(): Promise<{
    ...
    return await observability.traceOperation(
      ...
      async () => {
        ...
        console.log(
          `🚀 Starting enterprise cash alerts processing with batch size ${this.options.batchSize}`
        );
  ...
  private async processInBatches(
    ...
    console.log(
      `📦 Processing ${companies.length} companies in ${batches.length} batches`
    );
  ...
  private async processBatchWithConcurrency(
    ...
    const concurrentBatches = this.chunkArray(
      batch,
  ...
  private async processCompanyWithRetry(
    ...
    for (let attempt = 1; attempt <= this.options.retryAttempts; attempt++) {
      try {
        ...
        console.warn(
          `⚠️ Attempt ${attempt}/${this.options.retryAttempts} failed for company ${company.id}:`,
          error
        );
  ...
  private async processCompany(
    ...
    return await observability.traceOperation(
      ...
      async () => {
        ...
        if (company.schedule_enabled) {
          ...
          if (
            ...
            console.log(
              `⏰ Company ${company.id} scheduled for ${scheduledHour}:${scheduledMinute.toString().padStart(2, '0')} ${company.timezone}, current time: ${localTime.hour}:${localTime.minute.toString().padStart(2, '0')} - skipping`
            );
          ...
          if (company.schedule_scenario_code) {
            ...
            const evalResult = await this.evaluateAlertsOptimized(
              company.id,
              scenarioCode,
              period
            );
      ...
      },
      {
        ...
        timezone: company.timezone,
        scenario_code:
          company.schedule_scenario_code ||
          company.cash_version_code ||
          'auto-resolved',
        period: this.getLocalPeriod(company.timezone),
        schedule_enabled: company.schedule_enabled,
        scheduled_time: company.schedule_enabled
          ? `${company.hour_local}:${company.minute_local}`
          : 'default',
      }
    );
  }
  ...
  private async evaluateAlertsOptimized(
    ...
    if (this.options.useMaterializedView) {
      return await this.evaluateAlertsWithMaterializedView(
        companyId,
        scenarioCode,
        period
      );
    } else if (this.options.useCache) {
      return await this.evaluateAlertsWithCache(
        companyId,
        scenarioCode,
        period
      );
  ...
  private async evaluateAlertsWithCache(
    ...
    if (!cashMetrics) {
      throw new EvaluationError(
        `No cash metrics found for company ${companyId}`,
        companyId
      );
    }
    ...
    const breaches = alertRules
      ...
      .map(rule => ({
        ...
        current_value:
          rule.type === 'min_cash'
            ? cashMetrics.cumulative_balance
            : rule.type === 'max_burn'
              ? cashMetrics.avg_burn_rate_3m
  ...
  private async getLastApprovedCashVersion(
    companyId: string
  ): Promise<string | null> {
    return await this.errorHandler.executeWithErrorHandling(
      async () => {
