...
import {
  EventDrivenCashAlertsProcessor,
  CashAlertEvent,
...
export class HorizontalScalingManager {
  ...
  private async scaleToMinWorkers(): Promise<void> {
    ...
    if (neededWorkers > 0) {
      console.log(
        `📈 Scaling up to minimum workers: adding ${neededWorkers} workers`
      );
  ...
  private async checkAndScale(): Promise<void> {
    try {
      ...
      console.log(
        `📊 Scaling check: queue=${queueStats.queue_length}, workers=${currentWorkers}, healthy=${healthyWorkers}`
      );

      // Scale up if queue is too long
      if (
        queueStats.queue_length > this.config.scale_up_threshold &&
        currentWorkers < this.config.max_workers
      ) {
      ...
      if (
        queueStats.queue_length < this.config.scale_down_threshold &&
        currentWorkers > this.config.min_workers
      ) {
        const workersToRemove = Math.min(
          currentWorkers - this.config.min_workers,
          Math.ceil(
            (this.config.scale_down_threshold - queueStats.queue_length) / 5
          ) // Remove 1 worker per 5 events under threshold
        );
  ...
  private async getQueueStats(): Promise<{ queue_length: number }> {
    ...
    const totalProcessed = Array.from(this.workers.values()).reduce(
      (sum, worker) => sum + worker.processed_events,
      0
    );
  ...
  private weightedSelection(workers: WorkerNode[]): WorkerNode | undefined {
    ...
    const weights = workers.map(worker => {
      const totalEvents = worker.processed_events + worker.failed_events;
      const successRate =
        totalEvents > 0 ? worker.processed_events / totalEvents : 1;
...
export const scalingManager = new HorizontalScalingManager(
  {
    ...
    scale_check_interval_ms: parseInt(
      process.env.SCALE_CHECK_INTERVAL || '30000'
    ),
    worker_startup_time_ms: parseInt(process.env.WORKER_STARTUP_TIME || '5000'),
    worker_shutdown_time_ms: parseInt(
      process.env.WORKER_SHUTDOWN_TIME || '3000'
    ),
  },
  {
    strategy:
      (process.env.LOAD_BALANCER_STRATEGY as any) || 'least_connections',
