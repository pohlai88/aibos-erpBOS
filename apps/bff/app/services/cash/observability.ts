...
export class ComprehensiveObservability {
    ...
    private async collectBusinessMetrics(): Promise<BusinessMetrics> {
        const performanceMetrics = performanceMonitor.getMetrics().getPerformanceMetrics('cash_alerts', 300000); // Last 5 minutes
    ...
    private async checkAlertConditions(systemMetrics: SystemMetrics, businessMetrics: BusinessMetrics): Promise<void> {
        const alerts: string[] = [];

        // Check error rate
        if (businessMetrics.error_rate > this.config.alertThresholds.errorRate) {
            alerts.push(`High error rate: ${(businessMetrics.error_rate * 100).toFixed(2)}%`);
        }

        // Check response time
        if (businessMetrics.avg_processing_time_ms > this.config.alertThresholds.responseTimeP95) {
            alerts.push(`High response time: ${businessMetrics.avg_processing_time_ms.toFixed(2)}ms`);
        }

        // Check memory usage
        if (systemMetrics.memory_usage > this.config.alertThresholds.memoryUsage) {
            alerts.push(`High memory usage: ${systemMetrics.memory_usage.toFixed(2)}MB`);
        }
    ...
    private async sendAlert(alerts: string[], systemMetrics: SystemMetrics, businessMetrics: BusinessMetrics): Promise<void> {
        const alertMessage = {
            level: 'WARN',
            event: 'system_alert',
            timestamp: new Date().toISOString(),
    ...
    async getDetailedMetrics(): Promise<{
        ...
        const systemMetrics = await this.collectSystemMetrics();
        const businessMetrics = await this.collectBusinessMetrics();
        const performanceMetrics = performanceMonitor.getMetrics().getPerformanceMetrics('cash_alerts');
