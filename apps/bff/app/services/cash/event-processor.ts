...
export class EventDrivenCashAlertsProcessor {
    ...
    for (const company of companies) {
      try {
        const scenarioCode =
          ...
          (await this.getLastApprovedCashVersion(company.id));
        ...
        if (!event.scenario_code) {
          console.warn(
            `⚠️ No cash version configured for company ${company.id}, skipping`
          );
          failed++;
          continue;
        }
  ...
  private async runWorker(workerId: string): Promise<void> {
    ...
    while (this.isProcessing) {
      try {
        ...
        if (events.length > 0) {
          console.log(
            `📥 Worker ${workerId} processing ${events.length} events`
          );
  ...
  private async processEvent(
    ...
    try {
      console.log(
        `🔄 Worker ${workerId} processing event ${event.id} for company ${event.company_id}`
      );

      // Evaluate alerts
      const result = await this.errorHandler.executeWithErrorHandling(
        () =>
          evaluateCashAlerts(event.company_id, event.scenario_code, {
            year: event.year,
            month: event.month,
          }),
        'database',
  ...
  private async handleRetry(
    ...
    const delay =
      this.config.retry_delay_ms * Math.pow(2, event.retry_count - 1);

    console.log(
      `🔄 Retrying event ${event.id} (attempt ${event.retry_count}/${event.max_retries}) in ${delay}ms`
    );
  ...
  private async getLastApprovedCashVersion(
    ...
    try {
      ...
      console.error(
        `❌ Failed to get cash version for company ${companyId}:`,
        error
      );
      return null;
    }
