...
export class MetricsCollector {
    ...
    getPerformanceMetrics(operation?: string, timeWindowMs?: number): PerformanceMetrics {
        ...
        const durations = filteredMetrics.map(m => m.duration_ms).sort((a, b) => a - b);
    ...
    getCompanyMetrics(companyId: string, timeWindowMs?: number): PerformanceMetrics {
...
export class PerformanceMonitor {
    ...
    async monitor<T>(
        ...
        try {
            ...
            this.endTimer(operationId, operation, true, companyId, undefined, metadata);
            return result;
        } catch (error) {
...
export class HealthChecker {
    ...
    async checkHealth(): Promise<HealthStatus> {
        const checks = {
            database: (await this.checkDatabase() ? 'pass' : 'fail') as 'pass' | 'fail',
            memory: (this.checkMemory() ? 'pass' : 'fail') as 'pass' | 'fail',
            metrics: (this.checkMetrics() ? 'pass' : 'fail') as 'pass' | 'fail',
        };

        const metrics = this.metricsCollector.getPerformanceMetrics('cash_alerts', 300000); // Last 5 minutes

        const failedChecks = Object.values(checks).filter(status => status === 'fail').length;
    ...
    private checkMetrics(): boolean {
        const recentMetrics = this.metricsCollector.getPerformanceMetrics('cash_alerts', 300000);
