...
export async function evaluateCashAlerts(
  ...
  if (!skipIdempotency) {
    ...
    if (existing) {
      console.log(
        `🔒 Duplicate alert prevented for ${companyId} ${monthKey(period)} ${cashVersionCode}`
      );
      return {
  ...
  if (versionResult.rows.length === 0)
    return { breaches: [], versionMissing: true };
  ...
  const burnAvg = (cc?: string | null, project?: string | null) => {
    ...
    for (const p of months) {
      ...
      const net = arr
        .filter(
          it => (!cc || it.cc === cc) && (!project || it.project === project)
        )
  ...
  for (const r of rulesResult.rows) {
    ...
    if (r.type === 'min_cash' && bal < Number(r.threshold_num)) {
      breaches.push({
        ...
        threshold: Number(r.threshold_num),
      });
    }
    if (r.type === 'max_burn' && burn > Number(r.threshold_num)) {
      breaches.push({
        ...
        cc,
        project: pj,
        burn_rate: Number(burn.toFixed(2)),
        threshold: Number(r.threshold_num),
      });
    }
    if (r.type === 'runway_months' && runway < Number(r.threshold_num)) {
      breaches.push({
        ...
        cc,
        project: pj,
        runway_months: Number(runway.toFixed(2)),
        threshold: Number(r.threshold_num),
      });
    }
  }
