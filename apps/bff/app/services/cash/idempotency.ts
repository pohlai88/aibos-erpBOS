...
const DEDUPE_WINDOW_HOURS = parseInt(process.env.CASH_ALERTS_DEDUPE_WINDOW_HOURS || "24");
...
export async function recordIdempotency(
    ...
    try {
        const now = new Date();
        const expiresAt = new Date(now.getTime() + (DEDUPE_WINDOW_HOURS * 60 * 60 * 1000));
        ...
        console.log(`🔒 Idempotency recorded: ${key} (expires in ${DEDUPE_WINDOW_HOURS}h)`);
    } catch (error) {
