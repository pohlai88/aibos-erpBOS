...
export async function importRatesCsv(companyId: string, actor: string, fileText: string, mapping?: Partial<Record<string, string>>) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        try {
            ...
            await upsertRate(companyId, actor, { as_of_date, src_ccy, dst_ccy, rate });
            upserted++;
        } catch (e: any) {
