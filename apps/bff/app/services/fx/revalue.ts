...
export async function revalueMonetaryAccounts(opts: RevalOpts) {
  ...
  // Optional subset
  const items = opts.accounts?.length
    ? tb.filter((x: any) => opts.accounts!.includes(x.account_code))
    : tb;
  ...
  for (const [account, arr] of byAcc) {
    ...
    const memo =
      opts.memo ??
      `FX revaluation ${opts.year}-${String(opts.month).padStart(2, '0')} ${account}`;
