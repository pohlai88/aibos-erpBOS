...
describe('InsightsBenchmarksService', () => {
  ...
  it('should compute SELF benchmarks correctly', async () => {
    ...
    const daysToCloseBaseline = baselines.find(
      b => b.metric === 'DAYS_TO_CLOSE'
    );
    expect(daysToCloseBaseline).toBeDefined();
    expect(parseFloat(daysToCloseBaseline!.p50)).toBeGreaterThan(0);
    expect(parseFloat(daysToCloseBaseline!.p75)).toBeGreaterThan(
      parseFloat(daysToCloseBaseline!.p50)
    );
    expect(parseFloat(daysToCloseBaseline!.p90)).toBeGreaterThan(
      parseFloat(daysToCloseBaseline!.p75)
    );
  });
