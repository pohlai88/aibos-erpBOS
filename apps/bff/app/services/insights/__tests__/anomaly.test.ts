...
describe("InsightsAnomalyService", () => {
    ...
    it("should generate recommendations correctly", async () => {
        ...
        const taskReco = recommendations.find(r => r.recoCode === "TASK_SLA_OPTIMIZATION");
        ...
        const controlReco = recommendations.find(r => r.recoCode === "CONTROL_PRECHECK");
        expect(controlReco).toBeDefined();
        expect(controlReco?.title).toBe("Add Control Precheck");
        expect(controlReco?.effort).toBe("MEDIUM");

        const fluxReco = recommendations.find(r => r.recoCode === "FLUX_COMMENT_TEMPLATE");
