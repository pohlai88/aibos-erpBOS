...
export class InsightsAnomalyService {
    ...
    async scanAnomalies(
        ...
        for (const run of runs) {
            try {
                ...
                const controlAnomalies = await this.detectControlAnomalies(companyId, run.id);
                const fluxAnomalies = await this.detectFluxAnomalies(companyId, run.id);
                const durationAnomalies = await this.detectDurationAnomalies(companyId, run.id);

                anomaliesDetected += taskAnomalies.length + controlAnomalies.length +
                    fluxAnomalies.length + durationAnomalies.length;

                highSeverityCount += [...taskAnomalies, ...controlAnomalies, ...fluxAnomalies, ...durationAnomalies]
                    .filter(a => a.severity === "HIGH").length;

                // Generate recommendations based on anomalies
                const recommendations = await this.generateRecommendations(companyId, run.id, {
                    taskAnomalies,
                    controlAnomalies,
                    fluxAnomalies,
                    durationAnomalies
     ...
     */
    private async detectTaskAnomalies(companyId: string, runId: string): Promise<any[]> {
        const anomalies: any[] = [];
        ...
        const lateTasks = await db
            ...
            .from(insFactTask)
            .where(and(
                eq(insFactTask.runId, runId),
                eq(insFactTask.breached, true)
            ))
     ...
     */
    private async detectControlAnomalies(companyId: string, runId: string): Promise<any[]> {
        const anomalies: any[] = [];
        ...
        for (const failure of controlFailures) {
            const anomalyId = ulid();
            const score = this.calculateControlAnomalyScore(failure.count, failure.materialFailCount);
            const severity = this.determineSeverity(score);
     ...
     */
    private async detectFluxAnomalies(companyId: string, runId: string): Promise<any[]> {
        const anomalies: any[] = [];
        ...
        const fluxData = await db
            .select()
            .from(insFactFlux)
            .where(sql`flux_run_id IN (SELECT id FROM flux_run WHERE run_id = ${runId})`);

        for (const flux of fluxData) {
            const commentCoverage = flux.material > 0 ?
                (flux.material - flux.commentMissing) / flux.material : 1;

            if (commentCoverage < 0.5) { // Less than 50% coverage
                const anomalyId = ulid();
     ...
     */
    private async detectDurationAnomalies(companyId: string, runId: string): Promise<any[]> {
        const anomalies: any[] = [];
    ...
    private async generateRecommendations(
        ...
        for (const anomaly of anomalies.taskAnomalies) {
            ...
            await db.insert(insReco).values({
                ...
                detail: {
                    ...
                    recommendation: "Consider shifting JE cutoff task earlier by 12 hours",
        ...
        for (const anomaly of anomalies.fluxAnomalies) {
            ...
            await db.insert(insReco).values({
                ...
                detail: {
                    anomalyId: anomaly.id,
                    recommendation: "Create standardized comment templates for material variances",
    ...
    async actionRecommendation(
        ...
        await db
            ...
            .where(and(
                eq(insReco.id, data.id),
                eq(insReco.companyId, companyId)
            ));
    ...
    async getRecommendations(companyId: string, status?: string): Promise<any[]> {
        ...
        if (status) {
            whereConditions.push(eq(insReco.status, status as "OPEN" | "PLANNED" | "DONE"));
        }
    ...
    private calculateControlAnomalyScore(failureCount: number, materialFailCount: number): number {
    ...
    private calculateDurationAnomalyScore(currentDays: number, p90: number): number {
