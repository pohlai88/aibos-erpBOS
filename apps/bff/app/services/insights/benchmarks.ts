...
export class InsightsBenchmarksService {
  ...
  private async computeMetricBaseline(
    ...
    if (entityGroup === 'SELF') {
      values = await this.getSelfValues(
        companyId,
        metric,
        windowStart,
        windowEnd
      ...
      values = await this.getPeerValues(
        companyId,
        metric,
        windowStart,
        windowEnd
  ...
  async seedBenchmarks(
    ...
    const values = await this.getSelfValues(
      companyId,
      data.metric,
      windowStart,
      windowEnd
    ...
    if (values.length === 0) {
      throw new Error(
        `No values found for metric ${data.metric} in the specified window`
      );
    }
  ...
  async getBenchmarkDeltas(companyId: string): Promise<any[]> {
    ...
    return deltas.map(delta => ({
      ...delta,
      deltaToTarget: delta.target
        ? parseFloat(delta.currentValue) - parseFloat(delta.target)
        : null,
