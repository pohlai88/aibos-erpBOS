...
import type { InsightsHarvestResponseType } from '@aibos/contracts';
...
export class InsightsHarvestService {
   ...
   */
  private async harvestCloseFacts(
    companyId: string,
    runId: string
  ): Promise<void> {
    const run = await db
  ...
  private async harvestFluxFacts(runId: string): Promise<number> {
    ...
    for (const fluxRunData of fluxRuns) {
      ...
      const materialLines = await db
        ...
        .from(fluxLine)
        .where(
          and(eq(fluxLine.runId, fluxRunData.id), eq(fluxLine.material, true))
        );
  ...
  private async countMaterialExceptions(runId: string): Promise<number> {
    const exceptions = await db
      ...
      .from(ctrlException)
      .innerJoin(ctrlRun, eq(ctrlException.ctrlRunId, ctrlRun.id))
      .where(and(eq(ctrlRun.runId, runId), eq(ctrlException.material, true)));
