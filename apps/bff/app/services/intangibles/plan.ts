...
function generateSourceHash(companyId: string, payload: IntangiblePlanUpsertPayload): string {
...
export async function upsertIntangiblePlan(
    ...
    await pool.query(
      ...
      in_service, life_m, cost_center, project, source_hash, created_by
        ...
        [
            id, companyId, payload.class, payload.description, payload.amount,
            payload.currency, payload.present_ccy, payload.in_service, payload.life_m,
            payload.cost_center ?? null, payload.project ?? null,
            sourceHash, actor
        ]
    );
...
export async function listIntangiblePlans(companyId: string, limit: number = 100) {
    const result = await pool.query(
