    ...
    async createAssertion(
        ...
        try {
            ...
            if (existingResult.rows.length > 0) {
                throw new Error(`Assertion already exists for ${data.type} period ${data.period} by user ${userId}`);
            }
            ...
            const sha256Hex = crypto.createHash('sha256').update(payloadString).digest('hex');
            ...
            const insertResult = await client.query(
                `INSERT INTO sox_assertion 
                 ...
                 RETURNING id`,
                [
                    companyId, data.period, data.type, JSON.stringify(data.statement),
                    data.ebinder_id, userId, data.signed_role, sha256Hex
                ]
            );
            ...
            await this.emitAssertionSignedEvent(companyId, assertionId, data.type, data.period);
