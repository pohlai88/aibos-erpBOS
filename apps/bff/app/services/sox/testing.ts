    ...
    async createTestPlan(
        ...
        try {
            ...
            if (existingResult.rows.length > 0) {
                throw new Error(`Test plan already exists for control ${data.control_id} in period ${data.period}`);
            }
            ...
            const insertResult = await client.query(
                `INSERT INTO sox_test_plan 
                 ...
                 RETURNING id`,
                [
                    companyId, data.control_id, data.period, JSON.stringify(data.attributes),
                    data.sample_method, data.sample_size, userId
                ]
            );
        ...
        try {
            ...
            const insertResult = await client.query(
                `INSERT INTO sox_test_result 
                 ...
                 RETURNING id`,
                [
                    companyId, data.plan_id, data.sample_id, data.attribute,
                    data.outcome, data.note, data.evd_record_id, userId
                ]
            );
