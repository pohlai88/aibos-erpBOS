            ...
            if (existingResult.rows.length > 0) {
                ...
                await client.query(
                    `UPDATE sox_key_control SET 
                     ...
                     WHERE id = $9`,
                    [
                        data.name, data.process, data.risk_stmt, data.assertion,
                        data.frequency, data.automation, data.owner_id, data.active,
                        controlId
                    ]
                );
            } else {
                ...
                const insertResult = await client.query(
                    `INSERT INTO sox_key_control 
                     ...
                     RETURNING id`,
                    [
                        companyId, data.code, data.name, data.process, data.risk_stmt,
                        data.assertion, data.frequency, data.automation, data.owner_id,
                        data.active, userId
                    ]
                );
            ...
            if (existingResult.rows.length > 0) {
                ...
                const insertResult = await client.query(
                    `INSERT INTO sox_control_scope 
                     ...
                     VALUES ($1, $2, $3, $4, $5, $6)
                     RETURNING id`,
                    [companyId, data.control_id, data.period, data.in_scope, data.materiality, userId]
                );
