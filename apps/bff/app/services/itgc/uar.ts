...
export class ITGCUARService {
   ...
   */
  private async buildCampaignScope(
    companyId: string,
    data: CampaignOpenType
  ): Promise<any[]> {
    let whereConditions = [
      eq(itUser.companyId, companyId),
      eq(itUser.status, 'ACTIVE'),
    ];
    ...
    if (data.exclude_systems && data.exclude_systems.length > 0) {
      whereConditions.push(
        sql`${itUser.systemId} NOT IN (${data.exclude_systems.join(',')})`
      );
    }
   ...
   */
  private async createUARItem(
    campaignId: string,
    companyId: string,
    user: any
  ): Promise<void> {
    // Get user's grants snapshot
    const grants = await this.getUserGrantsSnapshot(
      companyId,
      user.id,
      user.systemId
    );
   ...
   * Get user's grants snapshot
   */
  private async getUserGrantsSnapshot(
    companyId: string,
    userId: string,
    systemId: string
  ): Promise<any> {
  ...
  async closeCampaign(
    ...
    if (pendingItems[0]!.count > 0) {
      throw new Error(
        `Cannot close campaign: ${pendingItems[0]!.count} items still pending`
      );
    }
    ...
    if (data.build_evidence_pack) {
      packId = await this.buildEvidencePack(
        companyId,
        userId,
        data.campaign_id
      );
