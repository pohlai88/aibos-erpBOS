...
export class ITGCSoDService {
   ...
   */
  async evaluateSoDRules(
    companyId: string
  ): Promise<{ violations_found: number; rules_evaluated: number }> {
  ...
  private async evaluateRule(
    ...
    if (type === 'allOf') {
      ...
      violations.push(
        ...(await this.findAllOfViolations(
          companyId,
          rule,
          entitlements,
          roles
      ...
      violations.push(
        ...(await this.findAnyOfViolations(
          companyId,
          rule,
          entitlements,
          roles
  ...
  private async findAllOfViolations(
    ...
    if (entitlements.length > 0) {
      ...
      for (const violation of entitlementViolations.rows) {
        ...
        const violationId = await this.createViolation(
          companyId,
          rule.id,
          violation,
          explanation
  ...
  async getSoDViolations(
    query: SoDQueryType
  ): Promise<SoDViolationResponseType[]> {
  ...
  async deleteSoDRule(companyId: string, ruleId: string): Promise<void> {
    await this.dbInstance
      .delete(itSodRule)
      .where(and(eq(itSodRule.id, ruleId), eq(itSodRule.companyId, companyId)));
  }
