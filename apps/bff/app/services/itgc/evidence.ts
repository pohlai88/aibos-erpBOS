...
export class ITGCEvidenceService {
  ...
  private async buildSnapshotData(
    ...
    switch (data.scope) {
      case 'USERS':
        snapshotData.data.users = await this.getUsersSnapshot(
          companyId,
          data.systems
        );
      ...
      case 'ROLES':
        snapshotData.data.roles = await this.getRolesSnapshot(
          companyId,
          data.systems
        );
      ...
      case 'GRANTS':
        snapshotData.data.grants = await this.getGrantsSnapshot(
          companyId,
          data.systems
        );
      ...
      case 'SOD':
        snapshotData.data.violations =
          await this.getSoDViolationsSnapshot(companyId);
        recordCount = snapshotData.data.violations.length;
        break;

      case 'BREAKGLASS':
        snapshotData.data.breakglass =
          await this.getBreakglassSnapshot(companyId);
   ...
   * Get roles snapshot
   */
  private async getRolesSnapshot(
    companyId: string,
    systems?: string[]
  ): Promise<any[]> {
   ...
   * Get grants snapshot
   */
  private async getGrantsSnapshot(
    companyId: string,
    systems?: string[]
  ): Promise<any[]> {
   ...
   * Get campaign items
   */
  private async getCampaignItems(
    companyId: string,
    campaignId: string
  ): Promise<any[]> {
    const results = await this.dbInstance
