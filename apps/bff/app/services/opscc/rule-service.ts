...
export class RuleService {
  ...
  async upsertRule(
    ...
    const existingRule = await db
      ...
      .from(opsRule)
      .where(
        and(eq(opsRule.company_id, companyId), eq(opsRule.code, data.code))
      )
      .limit(1);
  ...
  async toggleRule(
    ...
    const [updatedRule] = await db
      ...
      .where(and(eq(opsRule.company_id, companyId), eq(opsRule.code, ruleCode)))
      .returning();
   ...
   */
  async compileRuleFilters(companyId: string): Promise<
    Array<{
      ruleId: string;
      code: string;
      predicate: any;
...
export class PlaybookService {
  ...
  async getPlaybookVersions(
    ...
    const versions = await db
      ...
      .innerJoin(
        opsPlaybook,
        eq(opsPlaybookVersion.playbook_id, opsPlaybook.id)
      )
      .where(
        and(
          eq(opsPlaybook.company_id, companyId),
