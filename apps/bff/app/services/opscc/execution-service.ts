...
export class ExecutionService {
    ...
    async planRun(
        ...
        return {
            ...
            approvalReason: requiresDualControl ? 'Dual control required' : 'Live execution requires approval'
        };
    }
    ...
    private async executeStep(step: any, companyId: string): Promise<any> {
        ...
        try {
            ...
            const [updatedStep] = await db
                ...
                .set({
                    status: 'failed',
                    output_jsonb: { error: error instanceof Error ? error.message : 'Unknown error' },
                    duration_ms: duration
                })
                .where(eq(opsRunStep.id, step.id))
                .returning();
    ...
    private async checkOutcomes(runId: string, step: any): Promise<{ passed: boolean; reason?: string }> {
    ...
    private async rollbackRun(runId: string, executedSteps: any[]): Promise<void> {
    ...
    private async finalizeRun(runId: string, steps: any[]): Promise<any> {
        const metrics: RunMetricsM27_2 = {
            ...
            rollback_count: 0,
            p50_duration_ms: this.calculatePercentile(steps.map(s => s.duration_ms || 0), 50),
            p95_duration_ms: this.calculatePercentile(steps.map(s => s.duration_ms || 0), 95)
        };
    ...
    private computeCanaryScope(scope: any, samplePercent: number, minEntities: number): any {
    ...
    private resolveStepInput(stepInput: any, runScope: any): any {
        ...
        if (typeof stepInput === 'string') {
            return stepInput.replace(/\{\{scope\.(\w+)\}\}/g, (match: string, key: string) => {
    ...
    private async emitEvent(topic: string, key: string, payload: any): Promise<void> {
        await db
            .insert(opsOutbox)
            .values({
                id: crypto.randomUUID(),
