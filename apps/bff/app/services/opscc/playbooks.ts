...
export class PlaybooksService {
     ...
     */
    async getPlaybookAction(actionId: string): Promise<PlaybookActionResponse | null> {
        try {
        ...
        try {
            ...
            try {
                ...
                logLine({
                    ...
                    companyId, actionId: request.action_id, error: error instanceof Error ? error.message : String(error)
                });
            }

            // Log execution in outbox
            await this.logExecution(companyId, executionId, request, result, errorMessage);
    ...
    private async logExecution(
        ...
        try {
            ...
            logLine({
                ...
                companyId, executionId, error: error instanceof Error ? error.message : String(error)
            });
        }
    }
     ...
     */
    async getExecutionHistory(companyId: string, limit: number = 50): Promise<any[]> {
        try {
            const results = await this.dbInstance
                .select()
                .from(opsccOutbox)
