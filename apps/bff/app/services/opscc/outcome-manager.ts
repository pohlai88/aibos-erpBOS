...
export class OutcomeManager {
    ...
    async getAggregatedMetrics(
        ...
        const runStats = await db
            .select({
                ...
                count: count(),
                avgDuration: avg(sql`EXTRACT(EPOCH FROM (ended_at - started_at)) * 1000`),
        ...
        const successfulRuns = runStats.find(s => s.status === 'succeeded')?.count || 0;
        const failedRuns = runStats.find(s => s.status === 'failed')?.count || 0;
        const rolledBackRuns = runStats.find(s => s.status === 'rolled_back')?.count || 0;
    ...
    async checkMetricImprovement(
        ...
        const previousValue = await this.getMetricValue(companyId, metric, windowStart);

        const improvement = previousValue > 0 ? ((currentValue - previousValue) / previousValue) * 100 : 0;
        const improved = this.evaluateThreshold(metric, currentValue, previousValue, threshold);
    ...
    async checkBreachesZero(
        ...
        const currentBreaches = await this.getMetricValue(companyId, `breaches.${breachType}`, new Date());
    ...
    async checkErrorCountBelow(
        ...
        const currentErrors = await this.getMetricValue(companyId, `errors.${errorType}`, new Date());
    ...
    private evaluateThreshold(
        ...
        if (typeof threshold === 'object') {
            if (threshold.op === 'gt') {
                ...
                return currentValue >= threshold.value[0] && currentValue <= threshold.value[1];
            }
        }
