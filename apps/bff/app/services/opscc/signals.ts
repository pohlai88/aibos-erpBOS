...
export class OpsSignalService {
        ...
        try {
            ...
            for (const signal of batch.signals) {
                ...
                await this.dbInstance.insert(opsSignal).values({
                    ...
                    hash,
                    dedup_until: signal.ts ? new Date(new Date(signal.ts).getTime() + 3600000) : null, // 1 hour dedup window
    ...
    async querySignals(
        ...
        try {
            ...
            if (query.source) {
                whereConditions = and(whereConditions, eq(opsSignal.source, query.source));
            }
            if (query.kind) {
            ...
            if (query.severity) {
                whereConditions = and(whereConditions, eq(opsSignal.severity, query.severity));
            }
            if (query.from_ts) {
                whereConditions = and(whereConditions, gte(opsSignal.ts, new Date(query.from_ts)));
            }
            if (query.to_ts) {
                whereConditions = and(whereConditions, lte(opsSignal.ts, new Date(query.to_ts)));
            }
            if (query.tags && query.tags.length > 0) {
                whereConditions = and(whereConditions, sql`${opsSignal.tags} && ${query.tags}`);
            }
    ...
    async getWindowedSignals(
        ...
        try {
            ...
            if (filters.source) {
                whereConditions = and(whereConditions, eq(opsSignal.source, filters.source));
            }
            if (filters.kind) {
                whereConditions = and(whereConditions, eq(opsSignal.kind, filters.kind));
            }
            if (filters.kpi) {
                whereConditions = and(whereConditions, eq(opsSignal.kpi, filters.kpi));
            }
            if (filters.tags && filters.tags.length > 0) {
                whereConditions = and(whereConditions, sql`${opsSignal.tags} && ${filters.tags}`);
            }
     ...
     */
    private determineSeverity(signal: SignalIngest): "LOW" | "MEDIUM" | "HIGH" | "CRITICAL" {
        ...
        if (signal.value !== undefined) {
            if (signal.kpi === 'PTP_BROKEN_RATE' && signal.value > 10) return 'CRITICAL';
