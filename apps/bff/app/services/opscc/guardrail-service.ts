...
export class GuardrailService {
    ...
    async getEffectiveGuards(
        ...
        if (playbookCode) {
            ...
            if (playbookPolicy.length > 0) {
                effectivePolicy = this.mergePolicies(effectivePolicy, playbookPolicy[0]);
            }
        }
    ...
    async evaluateBlastRadius(
        ...
        const guards = await this.getEffectiveGuards(companyId, playbookCode, specGuards);
        ...
        if (guards.blast_radius?.maxEntities && entityCount > guards.blast_radius.maxEntities) {
            return {
                allowed: false,
        ...
        if (guards.blast_radius?.maxPercent && percentage > guards.blast_radius.maxPercent) {
            return {
                allowed: false,
    ...
    private mergeSpecGuards(policy: any, specGuards: any): any {
        return {
            ...policy,
            requiresDualControl: specGuards.requiresDualControl ?? policy.requires_dual_control,
