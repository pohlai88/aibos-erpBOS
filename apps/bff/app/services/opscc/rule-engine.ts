...
export class OpsRuleEngine {
     ...
     */
    async evaluateRules(companyId: string): Promise<{ fired: number; suppressed: number }> {
        try {
    ...
    async testRule(
        ...
        try {
            ...
            const windowFrom = new Date(Date.now() - testPeriodHours * 60 * 60 * 1000);
            ...
            while (currentWindowStart < windowTo) {
                const currentWindowEnd = new Date(currentWindowStart.getTime() + testRequest.window_sec * 1000);
                if (currentWindowEnd > windowTo) break;

                const windowSignals = signals.filter(signal => {
                    const signalTime = new Date(signal.ts);
                    return signalTime >= currentWindowStart && signalTime <= currentWindowEnd;
                });
                ...
                currentWindowStart = new Date(currentWindowStart.getTime() + testRequest.window_sec * 1000);
            }
    ...
    private async evaluateRule(rule: any): Promise<boolean> {
        ...
        return await this.evaluateExpression(rule.when_expr, signals, rule.threshold);
    }
     ...
     */
    private evaluateCondition(condition: any, signals: SignalResponse[], threshold: any): boolean {
        if (condition.kpi) {
            const kpiSignals = signals.filter(s => s.kpi === condition.kpi);
            if (kpiSignals.length === 0) return false;

            const values = kpiSignals.map(s => s.value).filter(v => v !== null) as number[];
            ...
            return this.compareValues(aggregated, condition.op || '>', thresholdValue);
        }
    ...
    private compareValues(left: number, op: string, right: number): boolean {
        switch (op) {
            ...
            case '==': return left === right;
            case '!=': return left !== right;
            case 'abs_gt': return Math.abs(left) > right;
            case 'delta_gt': return Math.abs(left - right) > right;
     ...
     */
    private extractFiltersFromExpression(whenExpr: any): { source?: string; kind?: string; kpi?: string; tags?: string[] } {
        const filters: any = {};
