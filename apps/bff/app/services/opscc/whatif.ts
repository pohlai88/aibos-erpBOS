...
import { whatifScenario, kpiSnapshot } from '@aibos/db-adapter/schema';
import type {
  WhatIfRunReq,
  WhatIfRunResponse,
  WhatIfScenario,
...
export class WhatIfService {
  ...
  async runSimulation(
    ...
    try {
      ...
      if (request.save_scenario) {
        scenarioId = await this.saveScenario(
          companyId,
          request,
          baseline,
          diff
  ...
  private async runArUpliftSimulation(
    ...
    const simulation: Record<string, any> = {
      ...baseline,
      PTP_KEPT_RATE:
        baseline.PTP_KEPT_RATE?.value * (1 + ptp_uplift_pct / 100) || 0,
      DUNNING_HIT_RATE:
        baseline.DUNNING_HIT_RATE?.value * (1 + dunning_wave_boost_pct / 100) ||
        0,
      DSO_DPO_CCC:
        baseline.DSO_DPO_CCC?.value * (1 - ptp_uplift_pct / 200) || 0, // DSO improves
      LIQUIDITY_RUNWAY_W:
        baseline.LIQUIDITY_RUNWAY_W?.value * (1 + ptp_uplift_pct / 100) || 0,
    };
    ...
    for (const [kpi, value] of Object.entries(simulation)) {
      if (typeof value === 'number' && baseline[kpi]?.value) {
        diff[kpi] = {
          ...
          delta_pct:
            ((value - baseline[kpi].value) / baseline[kpi].value) * 100,
        };
      }
    }
  ...
  private async runApDiscountBudgetSimulation(
    ...
    const simulation: Record<string, any> = {
      ...baseline,
      DISCOUNT_CAPTURE_RATE: Math.min(
        100,
        baseline.DISCOUNT_CAPTURE_RATE?.value * 1.2 || 0
      ),
  ...
  private async saveScenario(
    ...
    try {
      ...
      await this.dbInstance.insert(whatifScenario).values({
