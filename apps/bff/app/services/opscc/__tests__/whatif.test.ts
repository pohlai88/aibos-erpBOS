...
describe('What-If Service', () => {
  ...
  describe('Scenario Management', () => {
    ...
    it('should get specific scenario', async () => {
      ...
      const simulationResult = await service.runSimulation(
        ids.companyId,
        request
      );
    ...
    it('should return null for non-existent scenario', async () => {
      const scenario = await service.getScenario(
        ids.companyId,
        'non-existent-id'
      );
  ...
  describe('Baseline Data', () => {
    ...
    it('should get baseline data for TREASURY board', async () => {
      const baseline = await service['getBaselineData'](
        ids.companyId,
        'TREASURY'
      );
      expect(baseline).toBeDefined();
  ...
  describe('Simulation Calculations', () => {
    it('should run AR uplift simulation calculation', async () => {
      ...
      expect(simulation.PTP_KEPT_RATE).toBeGreaterThan(
        baseline.PTP_KEPT_RATE.value
      );
    ...
    it('should run AP discount budget simulation calculation', async () => {
      ...
      const { simulation, diff } = await service[
        'runApDiscountBudgetSimulation'
      ...
      expect(simulation.DISCOUNT_CAPTURE_RATE).toBeGreaterThan(
        baseline.DISCOUNT_CAPTURE_RATE.value
      );
      expect(diff.DISCOUNT_CAPTURE_RATE?.delta).toBeGreaterThan(0);

      // Check that liquidity runway improved
      expect(simulation.LIQUIDITY_RUNWAY_W).toBeGreaterThan(
        baseline.LIQUIDITY_RUNWAY_W.value
      );
    ...
    it('should run FX shock simulation calculation', async () => {
      ...
      expect(simulation.FX_EXPOSURE_BY_CCY).toBeGreaterThan(
        baseline.FX_EXPOSURE_BY_CCY.value
      );
      expect(diff.FX_EXPOSURE_BY_CCY?.delta).toBeGreaterThan(0);

      // Check that liquidity runway decreased slightly
      expect(simulation.LIQUIDITY_RUNWAY_W).toBeLessThan(
        baseline.LIQUIDITY_RUNWAY_W.value
      );
  ...
  describe('Scenario Saving', () => {
    it('should save scenario with correct data', async () => {
      ...
      const scenarioId = await service['saveScenario'](
        ids.companyId,
        request,
        baseline,
        diff
