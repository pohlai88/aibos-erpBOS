...
describe('M27.2: Playbook Studio + Guarded Autonomy', () => {
    ...
    afterEach(async () => {
        // Clean up test data
        await db.delete(opsRunStep).where(sql`run_id IN (SELECT id FROM ops_run WHERE company_id = ${companyId})`);
        await db.delete(opsRun).where(eq(opsRun.company_id, companyId));
        await db.delete(opsPlaybookVersion).where(sql`playbook_id IN (SELECT id FROM ops_playbook WHERE company_id = ${companyId})`);
        ...
        await db.delete(opsGuardPolicy).where(eq(opsGuardPolicy.company_id, companyId));
        await db.delete(opsOutbox).where(sql`payload_jsonb->>'company_id' = ${companyId}`);
    });
    ...
    describe('Rule Service', () => {
        ...
        it('should toggle rule enabled status', async () => {
            ...
            const disabledRule = await ruleService.toggleRule(companyId, 'toggle-test', false, userId);
            expect(disabledRule.enabled).toBe(false);

            const enabledRule = await ruleService.toggleRule(companyId, 'toggle-test', true, userId);
    ...
    describe('Playbook Service', () => {
        it('should create and validate playbooks', async () => {
            ...
            const playbook = await playbookService.upsertPlaybook(companyId, userId, playbookData);
        ...
        it('should reject invalid playbook specs', async () => {
            ...
            expect(validation.errors).toContain('Playbook must have at least one step');
        });
        ...
        it('should archive playbooks', async () => {
            const playbook = await playbookService.upsertPlaybook(companyId, userId, {
                ...
                name: 'Archive Test',
                status: 'active',
                spec: { steps: [{ id: 'step-1', action: 'payments.run.select', input: {} }] }
            });

            const archivedPlaybook = await playbookService.archivePlaybook(companyId, 'archive-test', userId);
    ...
    describe('Guardrail Service', () => {
        it('should create and manage guard policies', async () => {
            ...
            const policy = await guardrailService.upsertGuardPolicy(companyId, userId, policyData);
        ...
        it('should check concurrency limits', async () => {
            ...
            const concurrencyCheck = await guardrailService.checkConcurrency(companyId, 'test-playbook');
        ...
        it('should check dual control requirements', async () => {
            ...
            const requiresDualControl = await guardrailService.requiresDualControl(companyId, 'test-playbook');
        ...
        it('should evaluate canary requirements', async () => {
            ...
            const canaryEval = await guardrailService.evaluateCanary(companyId, 'test-playbook');
    ...
    describe('Action Registry', () => {
        ...
        it('should get inverse actions for rollback', () => {
            const inverseAction = actionRegistry.getInverseAction('payments.run.dispatch', {
    ...
    describe('Execution Service', () => {
        ...
        it('should enforce dual control approval', async () => {
            ...
            const run = await executionService.requestApproval(companyId, userId, plan);
            ...
            const approvedRun = await executionService.approveRun(companyId, 'different-user', {
                run_id: run.id,
                decision: 'approve',
    ...
    describe('Outcome Manager', () => {
        ...
        it('should calculate run metrics', () => {
            const steps = [
                ...
                { status: 'failed', duration_ms: 500, output_jsonb: { error: 'test error' } }
            ];
    ...
    describe('Integration Tests', () => {
        ...
        it('should enforce idempotency for duplicate runs', async () => {
            ...
            const run1 = await executionService.requestApproval(companyId, userId, plan1);
