...
describe('PlaybookStudioService', () => {
  ...
  describe('Playbook Versioning', () => {
    it('should create playbook version with correct version number', async () => {
      ...
      const result = await service.createPlaybookVersion(
        companyId,
        userId,
        data
      );
    ...
    it('should get playbook version history', async () => {
      ...
      const result = await service.getPlaybookVersionHistory(
        companyId,
        playbookId,
        query
      );
  ...
  describe('Visual Editor', () => {
    it('should save playbook from visual editor', async () => {
      ...
      const createPlaybookVersionSpy = vi
        .spyOn(service, 'createPlaybookVersion')
        .mockResolvedValue({
          id: 'version-123',
          company_id: companyId,
          playbook_id: data.playbook_id!,
      ...
      const result = await service.saveFromVisualEditor(
        companyId,
        userId,
        data
      );
  ...
  describe('Error Handling', () => {
    it('should handle database errors gracefully', async () => {
      ...
      await expect(
        service.createPlaybookVersion(companyId, userId, data)
    ...
    it('should handle missing playbook version', async () => {
      ...
      await expect(
        service.loadForVisualEditor(companyId, query)
