...
export class AlertsService {
     ...
     */
    async getAlertRulesByBoard(companyId: string, board: BoardType): Promise<AlertRuleResponse[]> {
        try {
            const results = await this.dbInstance
                .select()
                .from(alertRule)
     ...
     * Upsert alert rule
     */
    async upsertAlertRule(companyId: string, rule: AlertRuleUpsert): Promise<AlertRuleResponse> {
        try {
            ...
            logLine({
                ...
                companyId, rule, error: error instanceof Error ? error.message : String(error)
            });
            throw error;
        }
    }
    ...
    async deleteAlertRule(companyId: string, ruleId: string): Promise<void> {
        try {
            ...
            logLine({
                ...
                companyId, ruleId, error: error instanceof Error ? error.message : String(error)
            });
            throw error;
        }
    }
     ...
     */
    async getAlertEventsByStatus(companyId: string, status: AlertStatus): Promise<AlertEventResponse[]> {
        try {
            const results = await this.dbInstance
                .select()
                .from(alertEvent)
            ...
            logLine({
                ...
                companyId, status, error: error instanceof Error ? error.message : String(error)
            });
            throw error;
        }
    }
    ...
    async acknowledgeAlert(companyId: string, eventId: string): Promise<void> {
        try {
            await this.dbInstance
                ...
                .where(and(
                    eq(alertEvent.company_id, companyId),
                    eq(alertEvent.id, eventId)
                ));
        } catch (error) {
            logLine({
                msg: "AlertsService.acknowledgeAlert error",
                companyId, eventId, error: error instanceof Error ? error.message : String(error)
            });
            throw error;
        }
    }
     ...
     */
    private async evaluateAndFireAlert(companyId: string, rule: AlertRuleResponse): Promise<void> {
        try {
            // Check throttle
            ...
            // Evaluate expression
            const shouldFire = await this.evaluateExpression(rule.expr, parseFloat(kpi.value.toString()));
            ...
            logLine({
                ...
                companyId, rule: rule.rule_id, error: error instanceof Error ? error.message : String(error)
            });
        }
    }
     ...
     */
    private async evaluateExpression(expr: string, value: number): Promise<boolean> {
        try {
            ...
            logLine({
                ...
                expr, value, error: error instanceof Error ? error.message : String(error)
            });
            return false;
        }
    }
