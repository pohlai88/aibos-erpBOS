...
export class PlaybookStudioService {
    ...
    ): Promise<{ execution_id: string; steps: any[]; total_duration_ms: number }> {
        const executionId = crypto.randomUUID();
        const startTime = Date.now();

        // Get playbook version
        const playbookQuery = versionNo
            ? db.select().from(opsPlaybookVersion)
                .where(
                    and(
                        eq(opsPlaybookVersion.company_id, companyId),
                        eq(opsPlaybookVersion.playbook_id, playbookId),
    ...
    async verifyActionOutcome(
        ...
        switch (data.verification_type) {
            case "OUTCOME_CHECK":
                ...
                verificationResult = this.evaluateOutcome(actualOutcome, data.expected_outcome);
                break;
    ...
    async getExecutionMetrics(
        ...
        if (query.playbook_id) {
            whereConditions.push(eq(opsExecutionMetrics.playbook_id, query.playbook_id));
        }

        if (query.from_date) {
            whereConditions.push(gte(opsExecutionMetrics.execution_date, new Date(query.from_date)));
        }

        if (query.to_date) {
            whereConditions.push(lte(opsExecutionMetrics.execution_date, new Date(query.to_date)));
        }
    ...
    async getBlastRadius(
        ...
        if (query.playbook_id) {
            whereConditions.push(eq(opsBlastRadiusLog.playbook_id, query.playbook_id));
        }

        if (query.entity_type) {
            whereConditions.push(eq(opsBlastRadiusLog.entity_type, query.entity_type));
        }

        if (query.from_date) {
            whereConditions.push(gte(opsBlastRadiusLog.created_at, new Date(query.from_date)));
        }

        if (query.to_date) {
            whereConditions.push(lte(opsBlastRadiusLog.created_at, new Date(query.to_date)));
        }
        ...
        return logs.map((l: any) => ({
            ...
            blast_radius_percentage: l.blast_radius_percentage ? Number(l.blast_radius_percentage) : null,
    ...
    private async checkGuardrails(
        ...
        if (data.verification_rules.some((rule: any) => rule.rule_type === "blast_radius")) {
