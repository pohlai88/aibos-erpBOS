  ...
  async capture(params: {
    ...
    try {
      ...
      const fee = charges.data[0]?.balance_transaction
        ? await this.getStripeFee(charges.data[0].balance_transaction as string)
        : undefined;
      ...
      throw new Error(
        `Failed to capture Stripe payment: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async getStripeFee(balanceTransactionId: string): Promise<number> {
    try {
      const balanceTransaction =
        await this.stripe.balanceTransactions.retrieve(balanceTransactionId);
      return (
