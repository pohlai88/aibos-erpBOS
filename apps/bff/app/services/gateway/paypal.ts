...
export class PayPalGateway implements Gateway {
  ...
  constructor() {
    ...
    const environment = new PayPalApi.Environment.Sandbox(
  ...
  async capture(params: {
    ...
    try {
      ...
      return {
        ...
        fee: amount * 0.029 + 0.3, // PayPal fee structure
      };
    } catch (error) {
  ...
  async refund(params: {
    ...
    try {
      const request = new PayPalApi.Payments.CapturesRefundRequest(
        params.extRef
      );
      request.requestBody({
        amount: {
  ...
  parseWebhook(rawBody: string): GatewayWebhookEvent {
    try {
      ...
      throw new Error(
        `Failed to parse PayPal webhook: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
}
