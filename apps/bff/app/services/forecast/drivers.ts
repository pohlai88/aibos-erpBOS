...
export async function generateForecastFromBudget(
  ...
  for (const budgetLine of budgetLines) {
    ...
    if (!formula) {
      ...
      const driverAmount = evaluateDriverFormula(
        formula,
        baseAmount,
        simulationParams
      );
...
function evaluateDriverFormula(
  ...
  if (formula.includes('price * volume')) {
    // Revenue calculation
    const priceMultiplier = simulationParams?.priceDelta
      ? 1 + simulationParams.priceDelta / 100
      : 1;
    const volumeMultiplier = simulationParams?.volumeDelta
      ? 1 + simulationParams.volumeDelta / 100
      : 1;
