...
export async function upsertAllocRule(
  ...
  `,
    [
      ...
      validatedInput.driver_code,
      validatedInput.rate_per_unit,
      validatedInput.src_account,
      validatedInput.src_cc_like,
      validatedInput.src_project,
      validatedInput.eff_from,
      validatedInput.eff_to,
      validatedInput.order_no,
      actor,
    ]
  );
  ...
  if (validatedInput.method === 'PERCENT' && validatedInput.targets) {
    ...
    await pool.query('DELETE FROM alloc_rule_target WHERE rule_id = $1', [
      ruleId,
    ]);
...
export async function getAllocRuleTargets(
  ruleId: string
): Promise<AllocRuleTarget[]> {
  const { rows } = await pool.query(
    `
