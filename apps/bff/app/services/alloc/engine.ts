...
export async function runAllocation(
  ...
  for (const rule of filteredRules) {
    ...
    totalAmountAllocated += ruleLines.reduce(
...
async function processAllocRule(
  ...
  const sourcePool = trialBalance.filter(row => {
    if (rule.srcAccount && !row.accountCode.includes(rule.srcAccount))
      return false;
    if (
      rule.srcCcLike &&
      row.costCenter &&
      !row.costCenter.includes(rule.srcCcLike)
    )
      return false;
  ...
  switch (rule.method) {
    case 'PERCENT':
      lines.push(
        ...(await processPercentRule(rule, totalPool, companyId, year, month))
      );
      break;
    case 'RATE_PER_UNIT':
      lines.push(
        ...(await processRatePerUnitRule(
          rule,
          totalPool,
          companyId,
          year,
...
async function processDriverShareRule(
  ...
  const totalDriverValue = driverValues.reduce(
