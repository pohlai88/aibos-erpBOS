...
async function dispatchNotifications(breaches: AlertBreach[]): Promise<void> {
  ...
  for (const breach of breaches) {
    ...
    console.log(
      `Breach: ${breach.account} - ${breach.variancePct.toFixed(2)}% variance`
    );
  }
}
...
export async function evaluateAlerts(
  ...
  for (const rule of rulesResult.rows) {
    ...
    const dataset = await fetchVarianceDataset(
      companyId,
      period,
      rule.period_scope
    );
    ...
    const filteredDataset = dataset.filter(row => {
      if (rule.account_code && row.account_code !== rule.account_code)
        return false;
      if (rule.cost_center && row.cost_center !== rule.cost_center)
        return false;
