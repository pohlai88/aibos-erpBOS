...
export class FluxEngineService {
    ...
    private async generateFluxLines(
        ...
        return mockLines.map(line => {
            ...
            if (rule) {
                ...
                line.material = Math.abs(Number(line.delta)) >= absThreshold ||
     ...
     * Query flux runs
     */
    async queryFluxRuns(companyId: string, query: FluxQueryType): Promise<FluxRunResponseType[]> {
     ...
     * Query flux lines
     */
    async queryFluxLines(companyId: string, query: FluxQueryType): Promise<FluxLineResponseType[]> {
        let whereConditions = [];
        ...
        if (query.requires_comment !== undefined) {
            whereConditions.push(eq(fluxLine.requiresComment, query.requires_comment));
        }
     ...
     * Query flux comments
     */
    async queryFluxComments(companyId: string, query: FluxQueryType): Promise<FluxCommentResponseType[]> {
        let whereConditions = [];
     ...
     * Query flux rules
     */
    async queryFluxRules(companyId: string, query: FluxRuleQueryType): Promise<FluxRuleResponseType[]> {
     ...
     */
    async getTopMovers(runId: string, limit: number = 10): Promise<FluxLineResponseType[]> {
        const lines = await db
            .select()
            .from(fluxLine)
            .where(and(
                eq(fluxLine.runId, runId),
