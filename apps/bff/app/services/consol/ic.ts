...
export async function createIcLink(
  ...
  `, [id, companyId, data.entity_code, data.co_entity_cp, data.source_type, data.source_id, data.ext_ref, data.amount_base]);
...
export async function createIcMatch(
  ...
  `, [data.link_ids, companyId]);
    ...
    const totalAmount = linkRows.reduce((sum, link) => sum + Number(link.amount_base), 0);
