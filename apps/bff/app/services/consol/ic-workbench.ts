...
export async function upsertIcElimRule(
    ...
    `, [companyId, data.rule_code, data.src_account_like, data.cp_account_like, data.note, data.active, updatedBy]);
...
export async function generateAutoMatchProposals(
    ...
    const icLinks = await getIcLinksForPeriod(companyId, data.group_code, data.year, data.month);
    ...
    for (const proposal of proposals) {
        ...
        await pool.query(`
            ...
            VALUES ($1, $2, $3, $4, $5, $6)
        `, [proposalId, companyId, data.group_code, data.year, data.month, proposal.score]);
...
function findMatchingPairs(
    ...
    for (let i = 0; i < links.length; i++) {
        ...
        for (let j = i + 1; j < links.length; j++) {
            ...
            if ((linkA.entity_code === linkB.co_entity_cp && linkA.co_entity_cp === linkB.entity_code) ||
                (linkA.entity_code === linkB.entity_code && linkA.co_entity_cp === linkB.co_entity_cp)) {

                const matchScore = calculatePairScore(linkA, linkB, elimRules, tolerance, windowDays);
                if (matchScore.score > 0) {
                    candidates.push({ ...linkB, score: matchScore.score, hint: matchScore.hint });
                }
            }
        }
...
function calculatePairScore(
    ...
    const amountDiff = Math.abs(Number(linkA.amount_base) + Number(linkB.amount_base));
    ...
    if (linkA.ext_ref && linkB.ext_ref &&
    ...
    const dateDiff = Math.abs(new Date(linkA.posted_at).getTime() - new Date(linkB.posted_at).getTime());
...
async function createIcMatchFromProposal(
    ...
    await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7)
    `, [matchId, companyId, proposal.group_code, proposal.year, proposal.month, 0.01, createdBy]);
