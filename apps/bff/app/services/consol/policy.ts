...
export async function getRatePolicies(companyId: string): Promise<ConsolRatePolicy[]> {
    const result = await pool.query(`
        SELECT class, method, updated_at, updated_by
        FROM consol_rate_policy
        WHERE company_id = $1
...
export async function getRateOverrides(companyId: string): Promise<ConsolRateOverride[]> {
    const result = await pool.query(`
        SELECT account, method, note, updated_at, updated_by
        FROM consol_rate_override
        WHERE company_id = $1
...
export async function getCtaPolicy(companyId: string): Promise<ConsolCtaPolicy | null> {
    const result = await pool.query(`
        SELECT cta_account, re_account, updated_at, updated_by
        FROM consol_cta_policy
        WHERE company_id = $1
...
export async function getNciMap(companyId: string): Promise<ConsolNciMap | null> {
    const result = await pool.query(`
        SELECT nci_equity_account, nci_ni_account, updated_at, updated_by
        FROM consol_nci_map
        WHERE company_id = $1
...
export async function upsertLedgerOption(
    ...
    `, [companyId, data.enabled, data.ledger_entity, data.summary_account, updatedBy]);
...
export async function getLedgerOption(companyId: string): Promise<ConsolLedgerOption | null> {
    const result = await pool.query(`
        SELECT enabled, ledger_entity, summary_account, updated_at, updated_by
        FROM consol_ledger_option
        WHERE company_id = $1
...
export async function getDefaultRatePolicies(): Promise<ConsolRatePolicy[]> {
    return [
        { companyId: '', class: 'ASSET', method: 'CLOSING', updatedAt: '', updatedBy: '' },
        { companyId: '', class: 'LIAB', method: 'CLOSING', updatedAt: '', updatedBy: '' },
        { companyId: '', class: 'EQUITY', method: 'HISTORICAL', updatedAt: '', updatedBy: '' },
        { companyId: '', class: 'REVENUE', method: 'AVERAGE', updatedAt: '', updatedBy: '' },
        { companyId: '', class: 'EXPENSE', method: 'AVERAGE', updatedAt: '', updatedBy: '' }
    ];
}
