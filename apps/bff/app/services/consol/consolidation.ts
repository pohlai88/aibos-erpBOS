...
export async function runConsolidation(
  ...
  if (!data.dry_run) {
    ...
    if (lockRows.length > 0) {
      throw new Error(
        `Consolidation already committed for ${data.group_code} ${data.year}-${data.month}`
      );
    }
  }
  ...
  const ownershipTree = await getOwnershipTree(
    companyId,
    data.group_code,
    periodEndDate
  );
  ...
  const trialBalances = await getEntityTrialBalances(
    companyId,
    data.group_code,
    data.year,
    data.month
...
async function translateToPresentationCurrency(
  ...
  for (const balance of trialBalances) {
    ...
    const translationMethod = await resolveTranslationMethod(
      companyId,
      balance.accountCode
    );

    // Get appropriate rate based on translation method
    const rate = getRateByMethod(
      fxRates,
      balance.currency,
      presentCcy,
      translationMethod,
    ...
    if (!rate) {
      throw new Error(
        `FX rate not found for ${balance.currency} to ${presentCcy} using ${translationMethod} method`
      );
    }
...
function generateConsolidatedPl(
  balances: TrialBalanceRow[]
  ...
  for (const balance of balances) {
    ...
    if (balance.accountCode.match(/^[45]\d{3}/)) {
      accountTotals.set(
        balance.accountCode,
        (accountTotals.get(balance.accountCode) || 0) + balance.balance
      );
    }
  }
...
function generateConsolidatedBs(
  balances: TrialBalanceRow[]
): Array<{ account: string; amount: number }> {
  const accountTotals = new Map<string, number>();
...
async function getFxRatesForPeriod(
  ...
  Map<string, { closing: number; average: number; historical: number }>
> {
  ...
  const { rows: averageRows } = await pool.query(
    ...
    `,
    [companyId, periodEnd]
  ); // Using closing as fallback for average

  const fxRates = new Map<
    string,
    { closing: number; average: number; historical: number }
  >();
    ...
    if (Math.abs(summaryItem.amount) > 0.01) {
      ...
      await pool.query(
        ...
        [
          lineId,
          journalId,
          summaryItem.component === 'TRANSLATION'
            ? 'CTA'
            : summaryItem.component === 'IC_ELIM'
              ? '9890'
              : '3990',
