...
describe('IC Elimination Engine', () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query('DELETE FROM ic_elim_run WHERE company_id = $1', [
      companyId,
    ]);
    // Delete ic_match_line records that reference ic_link records for this company
    await pool.query(
      'DELETE FROM ic_match_line WHERE ic_link_id IN (SELECT id FROM ic_link WHERE company_id = $1)',
      [companyId]
    );
  ...
  it('should be idempotent on second dry-run', async () => {
    ...
    expect(result1.summary.totalEliminations).toBe(
      result2.summary.totalEliminations
    );
  });
