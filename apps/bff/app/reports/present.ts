...
export async function getAdminRateOr1(p: GetRateParams): Promise<number> {
    ...
    const y = p.asOf.getUTCFullYear(), m = p.asOf.getUTCMonth();
...
export async function convertMatrix(companyId: string, asOf: Date, dst: string, baseCcy: string, matrix: any) {
    ...
    const rows = matrix.rows.map((r: any) => ({
        ...r,
        values: Object.fromEntries(Object.entries(r.values).map(([k, v]) => [k, num(v) * rate])),
        total: matrix.grand_total != null && r.total != null ? num(r.total) * rate : r.total
    }));

    const grand_total = matrix.grand_total != null ? num(matrix.grand_total) * rate : undefined;
