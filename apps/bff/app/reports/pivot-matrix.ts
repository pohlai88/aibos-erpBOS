...
export function buildPivotMatrix(
    ...
    const { pivotKey, nullLabel, precision, grandTotal, valueKey, pivotValueSelector } = opts;
    ...
    function normPivot(v: any): string {
        ...
        const s = (raw === null || raw === undefined || raw === "") ? nullLabel : String(raw);
        if (!seen.has(s)) { seen.add(s); pivotOrder.push(s); }
        return s;
    }
    ...
    if (grandTotal) {
        const grand = Object.values(totals_by_pivot).reduce((a, b) => a + (b as number), 0);
