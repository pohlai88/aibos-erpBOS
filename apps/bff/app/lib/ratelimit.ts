...
export function rateLimitCheck(keyId: string, now = Date.now(), rate = DEFAULT_RATE): boolean {
...
export function retryAfterSecs(keyId: string, now = Date.now(), rate = DEFAULT_RATE): number {
