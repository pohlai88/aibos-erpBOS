...
export function buildMatrix(
    rows: MatrixRowIn[],
    opts: { pivotNullLabel: string; precision: number; includeGrandTotal: boolean }
): MatrixOut {
    const { pivotNullLabel, precision, includeGrandTotal } = opts;
    const norm = (v: string | null) => (v && v.trim().length ? v : pivotNullLabel);
    ...
    // Group by account
    const byAcct = new Map<string, { account_name: string; by_pivot: Record<string, number> }>();
    for (const r of rows) {
        const key = r.account_code;
        const col = norm(r.pivot_key);
        const entry = byAcct.get(key) ?? { account_name: r.account_name, by_pivot: {} };
