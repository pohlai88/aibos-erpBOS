...
export async function requireAuth(req: Request): Promise<AuthCtx | Response> {
  ...
  if (!rateLimitCheck(id)) {
    ...
    const headers = new Headers({ "Retry-After": String(ra), "Content-Type": "application/json" });
    return new Response(JSON.stringify({ ok: false, message: "rate limited" }), { status: 429, headers });
  }
  ...
  pool.query(`update api_key set last_used_at = now() where id = $1`, [id]).catch(() => { });
...
export function requireCapability(auth: AuthCtx, cap: Capability): Response | void {
  if (!auth.scopes.includes(cap)) {
    return new Response(JSON.stringify({ ok: false, message: `forbidden: missing scope ${cap}` }), { status: 403, headers: { "content-type": "application/json" } });
  }
}
