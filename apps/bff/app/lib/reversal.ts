...
export async function reverseJournal(journalId: string, postingISO: string) {
    ...
    // Load original
    const jres = await pool.query(`select id, company_id, currency from journal where id=$1`, [journalId]);
    ...
    const jid = await tx.run(async (t: any) => {
       ...
       values ($1,$2,$3,$4,'Reversal',$5,$6,'true',$7) returning id`,
            [crypto.randomUUID(), j.company_id, postingISO, j.currency, journalId, key, journalId]
        );
        ...
        await pool.query(
            ...
            [crypto.randomUUID(), "JournalReversed", JSON.stringify({ original_id: journalId, reversal_id: newId })]
        );
