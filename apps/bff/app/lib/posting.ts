...
export async function postByRule(
  ...
  for (const line of lines) {
    ...
    await ensureDimsMeetAccountPolicy(line.account_code, company_id, {
      cc,
      pr,
    });
  ...
  const baseAmounts = await computeBaseAmounts(
    company_id,
    docDate,
    lines.map(l => ({
  ...
  lines.forEach((line, index) => {
    line.base_amount = {
      amount: (
        baseAmounts.baseAmounts[index] ?? parseFloat(line.amount.amount)
      ).toFixed(2),
      currency: baseAmounts.baseCurrency,
    };
