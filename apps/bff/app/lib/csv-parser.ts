...
export async function parseCsvStream(
    ...
    const requiredColumns = [config.mapping.account_code, config.mapping.month, config.mapping.amount];
    const missingColumns = requiredColumns.filter(col => !headers.includes(col));

    if (missingColumns.length > 0) {
        return {
            rows: [],
            errors: [{ row: 1, issues: [`Missing required columns: ${missingColumns.join(', ')}`] }],
            meta: { rows_total: 0, rows_valid: 0, rows_invalid: 0 }
        };
    }
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const accountCode = getColumnValue(values, headers, config.mapping.account_code);
        ...
        const costCenter = config.mapping.cost_center ? getColumnValue(values, headers, config.mapping.cost_center) : undefined;
        const project = config.mapping.project ? getColumnValue(values, headers, config.mapping.project) : undefined;
...
function getColumnValue(values: string[], headers: string[], columnName: string): string | undefined {
