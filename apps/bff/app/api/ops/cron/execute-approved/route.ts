...
/**
 * M27.2: Cron Execute Approved API Route
 * 
 * POST /api/ops/cron/execute-approved - Execute approved runs respecting concurrency & cooldown
 * Runs every 2 minutes to execute approved runs
 */
...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json(
            { error: "Failed to execute approved runs" },
            { status: 500 }
        );
    }
}
