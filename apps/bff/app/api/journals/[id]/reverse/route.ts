...
export const POST = withRouteErrors(
  async (req: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    const capCheck = requireCapability(auth, 'reversal:create');
    if (isResponse(capCheck)) return capCheck;

    const params = await context.params;
    const url = new URL(req.url);
    const posting_date =
      url.searchParams.get('posting_date') ?? new Date().toISOString();
    ...
    const chk = await pool.query(`select company_id from journal where id=$1`, [
      params.id,
    ]);
    if (!chk.rows.length) return new Response('Not found', { status: 404 });
    if (chk.rows[0].company_id !== auth.company_id)
