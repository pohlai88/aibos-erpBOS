...
import { ComponentDesignService, ComponentScheduleService } from "@/services/lease/component";
import { requireAuth, requireCapability } from "@/lib/auth";
import { ok, badRequest, serverError } from "@/api/_lib/http";
import { LeaseComponentDesignReq, LeaseComponentScheduleReq } from "@aibos/contracts";
...
export async function GET(request: NextRequest, { params }: { params: { leaseId: string } }) {
    try {
        const auth = await requireAuth(request);
        if (auth instanceof Response) return auth;
