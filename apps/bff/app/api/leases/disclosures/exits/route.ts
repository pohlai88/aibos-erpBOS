...
function generateExitNarrative(
  ...
  narrative +=
    'All exits were processed in accordance with MFRS 16 requirements.';
...
export const GET = withRouteErrors(async (request: NextRequest) => {
  try {
    ...
    const partialDerecognition = {
      ...
      amount: partialExits.reduce(
        (sum, exit) => sum + Number(exit.settlement),
        0
      ),
      avg_share_pct:
        partialExits.length > 0
          ? partialExits.reduce(
              (sum, exit) => sum + Number(exit.share_pct || 100),
              0
            ) / partialExits.length
          : 0,
    };
    ...
    const restorationRollForward =
      await restorationService.getRestorationRollForward(
        auth.company_id,
        validatedQuery.year,
    ...
    // Generate narrative
    const narrative = generateExitNarrative(
      terminations,
      partialDerecognition,
      buyoutStats,
      restorationRollForward
    );
