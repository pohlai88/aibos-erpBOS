...
import { LeaseRestorationUpsert, LeaseRestorationQuery } from "@aibos/contracts";
...
export const GET = withRouteErrors(async (request: NextRequest) => { try {
        ...
        const query = {
            ...
            posted: url.searchParams.get('posted') ? url.searchParams.get('posted') === 'true' : undefined,
        ...
        const result = await restorationService.queryRestorations(auth.company_id, validatedQuery);
        return ok(result);
    } catch (error) {
        if (error instanceof Error && error.name === 'ZodError') {
