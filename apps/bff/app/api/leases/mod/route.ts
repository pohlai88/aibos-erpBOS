...
import { ConcessionService, ScopeTermService } from "../../../services/lease/m28-4-services";
...
export const POST = withRouteErrors(async (request: NextRequest) => { try {
        ...
        switch (validatedData.kind) {
            case 'CONCESSION':
                result = await concessionService.createConcession(companyId, userId, validatedData);
                break;
            case 'SCOPE':
                result = await scopeTermService.createScopeModification(companyId, userId, validatedData);
                break;
            case 'TERM':
                result = await scopeTermService.createTermModification(companyId, userId, validatedData);
                break;
            default:
