...
import { LeaseModApplyReq } from '@aibos/contracts';
import { z } from 'zod';
...
export const POST = withRouteErrors(
  async (request: NextRequest, { params }: { params: { modId: string } }) => {
    try {
      const body = await request.json();
      const validatedData = LeaseModApplyReq.parse({
        ...body,
        mod_id: params.modId,
      });
      ...
      const result = await remeasurementEngine.applyRemeasurement(
        companyId,
        userId,
        validatedData
      );
