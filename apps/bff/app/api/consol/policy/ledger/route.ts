...
export async function GET(req: NextRequest) {
    try {
        ...
        return Response.json({ error: 'Failed to fetch ledger option' }, { status: 500 });
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        const option = await upsertLedgerOption(auth.company_id, data, auth.user_id);
        ...
        return Response.json({ error: 'Failed to upsert ledger option' }, { status: 500 });
    }
}
