...
describe('Cash Alert Schedule Admin API', () => {
  it('validates timezone and scenario', async () => {
    ...
    try {
      new Intl.DateTimeFormat('en-CA', { timeZone: validTimezone }).format(
        new Date()
      );
    ...
    try {
      new Intl.DateTimeFormat('en-CA', { timeZone: invalidTimezone }).format(
        new Date()
      );
  ...
  it('validates scenario code format', () => {
    const validScenarioCodes = ['CFY26-01', 'CFY25-BL', 'CFY24-WIP', 'FY25-V2'];
