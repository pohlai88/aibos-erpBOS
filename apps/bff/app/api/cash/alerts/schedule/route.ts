...
export const PUT = withRouteErrors(async (req: NextRequest) => {
    try {
        ...
        const exists = await ensureScenarioExists(auth.company_id, parsed.scenario_code);
        if (!exists) {
            return badRequest(`Unknown cash version code: ${parsed.scenario_code}. Check /api/cash/versions for available scenarios`);
        }

        // Quick TZ sanity: let Intl throw if invalid
        try {
            new Intl.DateTimeFormat("en-CA", { timeZone: parsed.timezone }).format(new Date());
        } catch {
            return badRequest(`Invalid timezone: ${parsed.timezone}. Use IANA timezone identifiers like 'Asia/Ho_Chi_Minh' or 'America/New_York'`);
        }
        ...
        return badRequest("Failed to update cash alert schedule: " + (error instanceof Error ? error.message : 'Unknown error'));
    }
});
...
export const DELETE = withRouteErrors(async (req: NextRequest) => {
    try {
        ...
        const { deleteSchedule } = await import("../../../../services/cash/schedule");
        const result = await deleteSchedule(auth.company_id, auth.user_id || "unknown");
