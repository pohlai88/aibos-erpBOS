...
import {
  evaluateCashAlerts,
  dispatchCashNotifications,
} from '../../../../services/cash/alerts';
...
export const POST = withRouteErrors(async (req: Request) => {
  try {
    ...
    const result = await evaluateCashAlerts(auth.company_id, code, {
      year,
      month,
    });
    const dispatch = await dispatchCashNotifications(
      auth.company_id,
      result.breaches
    );
