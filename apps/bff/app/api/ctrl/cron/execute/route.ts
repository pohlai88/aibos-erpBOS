...
export const POST = withRouteErrors(async (request: NextRequest) => {
    ...
    try {
        ...
        switch (trigger) {
            case "scheduled":
                result = await executeScheduledControls(authCtx.company_id, authCtx.user_id, runnerService);
                break;
            case "daily_escalation":
                result = await escalateExceptions(authCtx.company_id, authCtx.user_id, exceptionsService);
                break;
            case "close_transition":
                result = await executeTransitionControls(authCtx.company_id, authCtx.user_id, runnerService);
                break;
            case "weekly_summary":
...
async function executeScheduledControls(companyId: string, userId: string, runnerService: ControlsRunnerService) {
    ...
    for (const assignment of dueAssignments.rows) {
        try {
            ...
            console.error(`Failed to execute control for assignment ${assignment.id}:`, error);
        }
    }
...
async function escalateExceptions(companyId: string, userId: string, exceptionsService: ControlsExceptionsService) {
...
async function executeTransitionControls(companyId: string, userId: string, runnerService: ControlsRunnerService) {
    ...
    for (const run of transitionedRuns.rows) {
        try {
            ...
            console.error(`Failed to execute transition control for run ${run.id}:`, error);
        }
    }
...
async function generateSummaryReport(companyId: string, period: "week" | "month") {
