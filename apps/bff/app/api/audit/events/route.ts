...
export const GET = withRouteErrors(async (req: Request) => {
    ...
    if (topic) { where.push(`topic = $${++p}`); params.push(topic); }
    if (from) { where.push(`created_at >= $${++p}`); params.push(from); }
    if (to) { where.push(`created_at <= $${++p}`); params.push(to); }
    if (afterTs && afterId) {
        where.push(`(created_at < $${++p} or (created_at = $${p} and id < $${++p}))`);
  ...
  `;
    ...
    const next = hasMore && slice.length
        ? Buffer.from(`${slice[slice.length - 1].created_at}|${slice[slice.length - 1].id}`).toString("base64url")
        : null;
