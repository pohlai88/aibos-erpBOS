...
export const GET = withRouteErrors(async (req: Request) => {
  ...
  if (afterTs && afterId) {
    where.push(
      `(j.posting_date < $${++p} OR (j.posting_date = $${p} AND j.id < $${++p}))`
    );
  ...
  const next =
    hasMore && slice.length
      ? Buffer.from(
          `${slice[slice.length - 1].posting_date}|${slice[slice.length - 1].id}`
        ).toString('base64url')
      : null;
