...
export async function POST(req: Request) {
    try {
        ...
        try {
            ...
            const exist = await client.query("select journal_id from payment where id=$1 and company_id=$2 and kind='AP'", [id, b.company_id]);
            if (exist.rows.length) {
                await client.query("COMMIT");
                return ok({ payment_id: id, journal_id: exist.rows[0].journal_id }, {
                    "X-Idempotent-Replay": "true",
                    "Location": `/api/payments/supplier?id=${id}&company_id=${b.company_id}`
                });
            }
         ...
         values ($1,$2,'AP','Supplier',$3,$4,$5,$6)`,
            ...
            const journalId = await postByRule("SupplierPayment", id, b.currency, b.company_id, {
            ...
            await client.query("COMMIT");
            return created({ payment_id: id, journal_id: journalId }, `/api/payments/supplier?id=${id}&company_id=${b.company_id}`);
        } catch (e) {
            await client.query("ROLLBACK");
            throw e;
        } finally {
