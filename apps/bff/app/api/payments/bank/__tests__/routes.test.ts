...
import {
  GET as getProfile,
  POST as postProfile,
} from '@/api/payments/bank/profile/route';
...
describe('Bank Connectivity API Routes', () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query('DELETE FROM ap_pay_line WHERE run_id = $1', [
      'test-run-123',
    ]);
    await pool.query('DELETE FROM ap_pay_run WHERE company_id = $1', [
      testCompanyId,
    ]);
  ...
  describe('Bank Profile Routes', () => {
    it('should create bank profile via POST', async () => {
      ...
      const req = new NextRequest(
        'http://localhost/api/payments/bank/profile',
        {
          method: 'POST',
    ...
    it('should retrieve bank profile via GET', async () => {
      ...
      await pool.query(
        ...
        [
          ...
          testBankCode,
          'SFTP',
          JSON.stringify({ host: 'sftp.hsbc.my' }),
          true,
          testUserId,
        ]
      );
      ...
      const req = new NextRequest(
        `http://localhost/api/payments/bank/profile?bank_code=${testBankCode}`
      );
    ...
    it('should return 404 for non-existent profile', async () => {
      const req = new NextRequest(
        'http://localhost/api/payments/bank/profile?bank_code=NONEXISTENT'
      );
    ...
    it('should list all bank profiles via GET', async () => {
      ...
      await pool.query(
        ...
        [
          ...
          'DBS-SG',
          'API',
          JSON.stringify({ api_base: 'https://api.dbs.sg' }),
          true,
          testUserId,
        ]
      );
  ...
  describe('Dispatch Routes', () => {
    ...
    it('should dispatch payment run successfully', async () => {
      ...
      const req = new NextRequest(
        'http://localhost/api/payments/bank/dispatch',
        {
          method: 'POST',
    ...
    beforeEach(async () => {
      ...
      await pool.query(
        ...
        [
          ...
          'DBS-SG',
          'SFTP',
          JSON.stringify({ host: 'sftp.dbs.sg' }),
          true,
          testUserId,
        ]
      );
  ...
  describe('Job Log Routes', () => {
    ...
    it('should retrieve job logs', async () => {
      const req = new NextRequest(
        `http://localhost/api/payments/bank/jobs?bank_code=${testBankCode}`
      );
    ...
    it('should filter job logs by kind', async () => {
      const req = new NextRequest(
        `http://localhost/api/payments/bank/jobs?bank_code=${testBankCode}&kind=DISPATCH`
      );
