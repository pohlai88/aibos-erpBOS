...
export async function POST(req: NextRequest) {
    try {
        ...
        return Response.json({
            message: data.decision === 'approve' ? 'Payment run second approved successfully' : 'Payment run rejected'
        }, {
            status: 200,
        ...
        return Response.json({ error: 'Failed to second approve payment run' }, { status: 500 });
    }
}
