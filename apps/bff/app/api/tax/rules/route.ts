...
export const POST = withRouteErrors(async (req: Request) => {
  ...
  const b = await req.json() as { company_id: string, items: { tax_code_id: string, effective_from: string, effective_to?: string, override_rate?: number }[] };
  ...
  for (const it of b.items) {
    await pool.query(
      ...
      [crypto.randomUUID(), b.company_id, it.tax_code_id, it.effective_from, it.effective_to ?? null, it.override_rate ?? null]
    );
  }
