...
import { TaxExportProfileUpsert, TaxExportProfileUpsertType } from "@aibos/contracts";
...
export const POST = withRouteErrors(async (req: NextRequest) => {
    ...
    await pool.query(
        ...
        [auth.company_id, input.partner_code, input.version, input.format, input.is_default, auth.user_id]
    );
