...
export const GET = withRouteErrors(async (req: Request) => {
  ...
  const out = await pool.query(
    `select coalesce(meta->>'tax_code_id','') as tax_code_id, sum(case when jl.dc='CR' then jl.base_amount else -jl.base_amount end) as amt
  ...
  const inn = await pool.query(
    `select coalesce(meta->>'tax_code_id','') as tax_code_id, sum(case when jl.dc='DR' then jl.base_amount else -jl.base_amount end) as amt
       from journal_line jl
       join journal j on j.id = jl.journal_id
      where j.company_id=$1 and j.posting_date >= $2 and j.posting_date < ($3::date + 1)
        and jl.account_code = any($4)
  ...
  const outRows = out.rows.map((r:any)=> ({ tax_code_id: r.tax_code_id, amount: Number(r.amt || 0) }));
  const inRows  = inn.rows.map((r:any)=> ({ tax_code_id: r.tax_code_id, amount: Number(r.amt || 0) }));
