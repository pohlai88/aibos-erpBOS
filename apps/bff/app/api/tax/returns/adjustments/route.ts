...
import { upsertTaxAdjustment, getTaxAdjustments } from "@/services/tax_return/templates";
...
export const GET = withRouteErrors(async (req: NextRequest) => {
    ...
    const adjustments = await getTaxAdjustments(auth.company_id, partnerCode, parseInt(year), parseInt(month));
...
export const POST = withRouteErrors(async (req: NextRequest) => {
    ...
    const result = await upsertTaxAdjustment(auth.company_id, input, auth.user_id);
    return ok(result);
});
