...
import { upsertTaxBoxMap, getTaxBoxMap } from '@/services/tax_return/templates';
import { TaxBoxMapUpsert } from '@aibos/contracts';

// GET /api/tax/box-map - Get tax box mapping
export const GET = withRouteErrors(async (req: NextRequest) => {
  ...
  const boxMap = await getTaxBoxMap(auth.company_id, partnerCode, version);
  return ok(boxMap);
});

// POST /api/tax/box-map - Create or update tax box mapping
export const POST = withRouteErrors(async (req: NextRequest) => {
  ...
  const input = TaxBoxMapUpsert.parse(json);

  const result = await upsertTaxBoxMap(auth.company_id, input, auth.user_id);
  return ok(result);
});
