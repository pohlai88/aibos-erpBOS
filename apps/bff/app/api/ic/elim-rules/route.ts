...
import { IcElimRuleUpsert, IcAutoMatchRequest, IcProposalDecision } from "@aibos/contracts";
import {
    upsertIcElimRule,
    getIcElimRules,
...
export async function GET(req: NextRequest) {
    try {
        ...
        return Response.json({ error: 'Failed to fetch IC elimination rules' }, { status: 500 });
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        return Response.json({ error: 'Failed to upsert IC elimination rule' }, { status: 500 });
    }
}
