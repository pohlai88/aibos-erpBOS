...
import { runIcElimination, getIcElimRuns } from '@/services/consol/elimination';
import { IcElimRunRequest } from '@aibos/contracts';

// GET /api/ic/runs - List IC elimination runs
export const GET = withRouteErrors(async (req: NextRequest) => {
  ...
  const runs = await getIcElimRuns(
    auth.company_id,
    groupCode || undefined,
...
// POST /api/ic/eliminate - Run IC elimination
export const POST = withRouteErrors(async (req: NextRequest) => {
  ...
  const input = IcElimRunRequest.parse(json);

  const result = await runIcElimination(auth.company_id, input, auth.user_id);
  return ok(result);
});
