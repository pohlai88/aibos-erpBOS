...
export function badRequest(message: string, details?: unknown) {
    return NextResponse.json({ ok: false, error: "BadRequest", message, details }, { status: 400 });
}
export function unprocessable(message: string, details?: unknown) {
    return NextResponse.json({ ok: false, error: "UnprocessableEntity", message, details }, { status: 422 });
}
