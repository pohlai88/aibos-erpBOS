...
export const GET = withRouteErrors(async (request: NextRequest) => {
    ...
    if (type === "rules") {
        if (board) {
            ...
            const rules = await service.getAlertRulesByBoard(authCtx.company_id, boardType);
            return ok({ rules });
        } else {
        ...
        if (status) {
            ...
            const events = await service.getAlertEventsByStatus(authCtx.company_id, statusType);
            return ok({ events });
        } else {
...
export const POST = withRouteErrors(async (request: NextRequest) => {
    ...
    const service = new AlertsService();
    const rule = await service.upsertAlertRule(
        authCtx.company_id,
        validatedData
    );
...
export const PATCH = withRouteErrors(async (request: NextRequest) => {
    ...
    const action = z.enum(["acknowledge", "resolve"]).parse(searchParams.get("action"));
