...
import {
  createImpairment,
  listImpairments,
} from '../../../services/assets/impairments';
...
export const POST = withRouteErrors(async (req: NextRequest) => {
  ...
  try {
    ...
    const result = await createImpairment(
      auth.company_id,
...
export const GET = withRouteErrors(async (req: NextRequest) => {
  ...
  try {
    ...
    const planKind = url.searchParams.get('plan_kind') as
      | 'capex'
      | 'intangible'
      | undefined;
    const planId = url.searchParams.get('plan_id') || undefined;

    const impairments = await listImpairments(
      auth.company_id,
      planKind,
      planId
    );
