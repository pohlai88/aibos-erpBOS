...
export const GET = withRouteErrors(async (req: NextRequest) => { const auth = await requireAuth(req);
    ...
    try {
        ...
        if (kind && year && month) {
            // Get specific draft
            const draft = await getDraft(auth.company_id, kind, Number(year), Number(month));
            if (!draft) {
                return ok(null);
            }
            return ok(draft);
...
export const DELETE = withRouteErrors(async (req: NextRequest) => { const auth = await requireAuth(req);
    if (auth instanceof Response) return auth;
    ...
    try {
        ...
        const deleted = await deleteDraft(auth.company_id, kind, Number(year), Number(month));
        return ok({ deleted });
    } catch (error) {
        if (error instanceof Error) {
