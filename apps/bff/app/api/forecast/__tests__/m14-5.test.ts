...
import {
  ...
  it,
  expect,
} from '../../../services/cash/__tests__/test-utils';
...
describe('M14.5 Driver-Based Rolling Forecast', () => {
  ...
  describe('Forecast Generation', () => {
    ...
    it('should apply seasonality to monthly forecasts', () => {
      ...
      if (!janFactor || !decFactor)
        throw new Error('Seasonality factors not found');
  ...
  describe('What-If Simulation', () => {
    ...
    it('should handle combined simulation parameters', () => {
      ...
      const finalAmount =
        baseRevenue * priceMultiplier * volumeMultiplier * fxMultiplier;
  ...
  describe('Report Integration', () => {
    ...
    it('should generate forecast matrix with pivot support', () => {
      ...
      const pivotByCC = forecastLines.reduce(
        (acc, line) => {
    ...
    it('should generate forecast in under 5 seconds for 50k lines', () => {
      ...
      lines.forEach(line => {
        const seasonalFactor =
          mockDriverProfile.seasonality_json[
            line.month.toString() as keyof typeof mockDriverProfile.seasonality_json
          ] || 100;
...
console.log(
  '✅ M14.5 Driver-Based Rolling Forecast tests completed successfully!'
);
