...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!scope || !['INDIRECT', 'DIRECT13'].includes(scope)) {
      return Response.json(
        { error: 'Scope must be INDIRECT or DIRECT13' },
        { status: 400 }
      );
    }
    ...
    const result = await runConsolidatedCashFlow(
      auth.company_id,
      groupCode,
      data,
      auth.user_id
    ...
    return Response.json(
      {
        ...
        message: data.dry_run
          ? 'Consolidated dry run completed successfully'
          : 'Consolidated cash flow run completed successfully',
      },
      {
        status: 200,
    ...
    return Response.json(
      { error: 'Failed to run consolidated cash flow' },
      { status: 500 }
    );
  }
}
