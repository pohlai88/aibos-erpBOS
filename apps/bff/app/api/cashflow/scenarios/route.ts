...
export async function GET(req: NextRequest) {
    try {
        ...
        return Response.json({ error: 'Failed to fetch cash flow scenarios' }, { status: 500 });
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        const scenario = await upsertCfScenario(auth.company_id, data, auth.user_id);
        ...
        return Response.json({ error: 'Failed to upsert cash flow scenario' }, { status: 500 });
    }
}
