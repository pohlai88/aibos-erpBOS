...
import { parseCsvStream, normalizeMonth, normalizeAmount } from "../../../lib/csv-parser";
...
export const POST = withRouteErrors(async (req: Request) => {
    ...
    try {
        ...
        if (!parsed.mapping.account_code || !parsed.mapping.month || !parsed.mapping.amount) {
            throw new Error("Missing required mapping fields: account_code, month, amount");
        }
    ...
    const dimRes = await resolveDimensionsForRows(auth.company_id, parseResult.rows.map(r => ({
    ...
    if (totalErrors > 0) {
        return badRequest("Cannot commit with validation errors", parseResult.errors);
    }
    ...
    const res = await insertBudgetLinesTxn(
        ...
        parseResult.rows.map(r => ({
            ...
            month: parseInt(normalizeMonth(r.month, payload.defaults.year).split('-')[1] || '1'),
