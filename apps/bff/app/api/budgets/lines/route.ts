...
export const POST = withRouteErrors(async (req: Request) => {
    ...
    if (budgetCheck.rows[0].locked) {
        return unprocessable(`Budget ${b.budget_id} is locked and cannot be modified`);
    }

    let upserted = 0;

    for (const item of b.items) {
        if (!item.period_month || !item.account_code || typeof item.amount_base !== "number") {
            return unprocessable("Each item must have period_month, account_code, and amount_base");
        }
