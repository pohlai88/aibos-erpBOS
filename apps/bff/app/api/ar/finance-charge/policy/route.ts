...
import { FinanceChargePolicyUpsert, StatementRunReq, StatementEmailReq } from "@aibos/contracts";
...
export const GET = withRouteErrors(async (request: NextRequest) => { try {
        ...
        const policy = await statementService.getFinanceChargePolicy(auth.company_id);

        if (!policy) {
            return ok({ enabled: false, annual_pct: 0, min_fee: 0, grace_days: 0, comp_method: "simple" });
        }
