...
export const GET = withRouteErrors(async (req: Request) => {
    try {
        ...
        console.log(`🚀 Starting revenue recognition monthly cron job at ${new Date().toISOString()}`);
        ...
        return badRequest("Revenue recognition monthly cron failed: " + (error instanceof Error ? error.message : 'Unknown error'));
    }
});
...
async function processMonthlyRevenueRecognition() {
    ...
    try {
        ...
        console.log(`Processed ${eventResults.processed} events, ${eventResults.errors} errors`);

        // 2. Run recognition for previous month (dry run first)
        console.log(`💰 Running recognition for ${year}-${month.toString().padStart(2, '0')}`);
        ...
        console.log(`Dry run completed: ${dryRunResult.lines_created} lines, ${dryRunResult.total_amount} total amount`);
        ...
        if (dryRunResult.lines_created > 0) {
            ...
            console.log(`Recognition completed: ${actualResult.lines_created} lines posted`);
            processedCount++;
        }
        ...
        return badRequest(`Monthly revenue recognition failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
