...
export const GET = withRouteErrors(async (req: Request) => {
    try {
        ...
        console.log(`🚀 Starting revenue recognition daily cron job at ${new Date().toISOString()}`);
        ...
        return badRequest("Revenue recognition daily cron failed: " + (error instanceof Error ? error.message : 'Unknown error'));
    }
});
...
async function processDailyRevenueRecognition() {
    ...
    try {
        ...
        return badRequest(`Daily revenue recognition failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}