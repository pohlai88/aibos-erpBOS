...
export const POST = withRouteErrors(async (request: NextRequest) => {
    ...
    try {
        ...
        switch (trigger) {
            case "connector_pull":
                result = await executeConnectorPull(authCtx.company_id, authCtx.user_id);
                break;
            case "sod_scan":
            ...
            case "breakglass_expiry":
                result = await executeBreakglassExpiry(authCtx.company_id, authCtx.user_id);
                break;
            case "evidence_snapshots":
                result = await executeEvidenceSnapshots(authCtx.company_id, authCtx.user_id);
                break;
            default:
