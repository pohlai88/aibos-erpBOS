...
export const POST = withRouteErrors(async (req: Request) => {
  ...
  // Load event (outbox)
  const ev = await pool.query(`select id, topic, payload, created_at from outbox where id=$1`, [b.event_id]);
  ...
  const hooks = await pool.query(`select id, url, secret, topics from webhook where company_id=$1 and enabled=true`, [auth.company_id]);
