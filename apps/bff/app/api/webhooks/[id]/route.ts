...
export const PATCH = withRouteErrors(async (req: Request, context: { params: Promise<{ id: string }> }) => {
  ...
  const b = await req.json() as { enabled?: boolean; rotate_secret?: boolean; url?: string; topics?: string[] };
  const updates: string[] = [];
  const vals: any[] = [];
  let p = 0;

  if (typeof b.enabled === "boolean") { updates.push(`enabled=$${++p}`); vals.push(b.enabled); }
  if (b.url) { updates.push(`url=$${++p}`); vals.push(b.url); }
  if (Array.isArray(b.topics)) { updates.push(`topics=$${++p}`); vals.push(b.topics); }
