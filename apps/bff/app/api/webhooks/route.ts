...
export const POST = withRouteErrors(async (req: Request) => {
  ...
  const b = await req.json() as { url: string; topics: string[]; id?: string };
  if (!b?.url || !Array.isArray(b.topics) || b.topics.length === 0) return unprocessable("url and topics required");
  ...
  // show secret once
  return created({ id, url: b.url, topics: b.topics, secret }, `/api/webhooks/${id}`);
});
