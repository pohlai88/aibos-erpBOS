...
import { attestTask, attestCampaign, closeItem, outbox } from "@aibos/db-adapter/schema";
...
export const POST = withRouteErrors(async (request: NextRequest) => {
    ...
    try {
        ...
        console.error(`Attestations cron job failed for trigger ${trigger}:`, error);
        return ok({
                        success: false,
                        trigger,
...
async function tickSlaForAllCampaigns(companyId: string, slaService: AttestSlaService) {
...
async function sendDueSoonNotifications(companyId: string) {
    ...
    for (const task of dueSoonTasks.rows) {
        try {
            ...
            console.error(`Failed to send due soon notification for task ${task.id}:`, error);
        }
    }
...
async function syncWithCloseBoard(companyId: string, boardService: CloseBoardService) {
    ...
    for (const task of tasksNeedingBoardItems.rows) {
        try {
            ...
            console.error(`Failed to create Close Board item for task ${task.id}:`, error);
        }
    }
    ...
    for (const item of existingBoardItems.rows) {
        try {
            const newStatus = item.state === "APPROVED" ? "DONE" :
