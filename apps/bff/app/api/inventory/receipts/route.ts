...
export const POST = withRouteErrors(async (req: Request) => {
    ...
    try {
        ...
        if (exist.rows.length) {
            ...
            return ok(
                { ok: true, data: { move_id: id, company_id: b.company_id, item_id: b.item_id, qty: b.qty, unit_cost: b.unit_cost, currency: b.currency, total_cost: Number(b.qty) * Number(b.unit_cost), created_at: exist.rows[0].created_at }, replay: true },
                { "X-Idempotent-Replay": "true", "Location": `/api/inventory/receipts?company_id=${b.company_id}&id=${id}` }
            );
        }
       ...
       set on_hand_qty = excluded.on_hand_qty,
        ...
        await client.query("COMMIT");
        return created(
            { ok: true, data: { move_id: id, journal_id: null, company_id: b.company_id, item_id: b.item_id, qty, unit_cost: ucost, total_cost: in_total.toFixed(2), currency: b.currency, on_hand_after: q1.toFixed(6), mac_after: mac1.toFixed(6) } },
