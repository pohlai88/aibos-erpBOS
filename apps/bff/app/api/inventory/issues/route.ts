...
export const POST = withRouteErrors(async (req: Request) => {
  ...
  try {
    ...
    if (exist.rows.length) {
      ...
      return ok(
        { ok: true, data: { move_id: id }, replay: true },
        {
          'X-Idempotent-Replay': 'true',
          Location: `/api/inventory/issues?company_id=${auth.company_id}&id=${id}`,
        }
      );
    }
    ...
    return created(
      {
        ...
        data: {
          ...
          journal_id: jid,
          qty: b.qty,
          unit_cost: mac0.toFixed(6),
          total_cost: out_total.toFixed(2),
