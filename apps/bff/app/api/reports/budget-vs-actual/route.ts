...
export const GET = withRouteErrors(async (req: Request) => {
  ...
  if (!scenarioVersionId) {
    return badRequest(
      `Scenario '${scenario}' not found or no budget version available`
    );
  }
  ...
  for (const g of groupKeys)
    if (!supported.has(g)) return badRequest(`unsupported group key: ${g}`);
  ...
  const shaped = rows.map((r: Row) => {
    ...
    const cost_center_id =
      r.cost_center_id === '' ? null : (r.cost_center_id ?? null);
    ...
    const includeCostCenter =
      groupByCols.includes('cost_center_id') || pivot === 'cost_center';
    const includeProject =
      groupByCols.includes('project_id') || pivot === 'project';
  ...
  const totals_pct =
    totals.budget === 0 ? null : totals.variance / Math.abs(totals.budget);
  ...
  if (pivot === 'cost_center' || pivot === 'project') {
    ...
    for (const r of shaped) {
      ...
      if (accountData) {
        const cell = accountData[k ?? pivotNullLabel] ?? {
          budget: 0,
          actual: 0,
        };
    ...
    // build matrix rows
    const matrixRows = Object.entries(byAccount).map(
      ([account_code, axisAgg]) => {
        ...
        for (const k of pivotKeys) {
          ...
          const variance = actual - budget;
          const variance_pct =
            budget === 0 ? null : variance / Math.abs(budget);
        ...
        row.total = {
          ...
          variance_pct:
            rowBudget === 0
              ? null
              : (rowActual - rowBudget) / Math.abs(rowBudget),
        };
        return row;
      }
    );
    ...
    for (const k of pivotKeys) {
      ...
      totalsByPivot[keyStr] = {
        ...
        actual: a,
        variance: v,
        variance_pct: b === 0 ? null : v / Math.abs(b),
      };
    }
