...
export const GET = withRouteErrors(async (req: Request) => {
  ...
  const pivotNullLabel =
    url.searchParams.get('pivot_null_label') ?? 'Unassigned';
  const precision = Number.parseInt(
    url.searchParams.get('precision') ?? '2',
    10
  );
  ...
  for (const s of policy.bs) {
    if ('formula' in s) {
      const expr = s.formula.replace(
        /\b([A-Za-z ][A-Za-z ]+)\b/g,
        (_m: string, name: string) => (values[name] ?? 0).toString()
      );
  ...
  if (present && present !== currency) {
    ...
    if (quotes.length) {
      ...
      for (const [key, value] of Object.entries(values)) {
        convertedValues[key] =
          convertToPresent(value, currency, present, quotes, asOf) ?? value;
      }
    }
  }
  ...
  const equationOK =
    Math.abs(
      (convertedValues['Assets'] ?? 0) -
        (convertedValues['Liabilities'] ?? 0) -
        (convertedValues['Equity'] ?? 0)
    ) < 0.005;
