import { NextRequest, NextResponse } from 'next/server';
import { requireAuth, AuthCtx } from '@/lib/auth';
import { requireCapability } from '@/lib/rbac';
import { withRouteErrors } from '@/lib/route-utils';
import { CloseBoardService } from '@/services/close/board';
import { CloseIngestRequestSchema } from '@aibos/contracts';
import { ok } from '@/api/_kit';

export const POST = withRouteErrors(async (request: NextRequest) => {
  const auth = await requireAuth(request);
  await requireCapability(auth, 'close:board:manage');

  const authCtx = auth as AuthCtx;
  const body = await request.json();

  // Validate request body
  const validatedData = CloseIngestRequestSchema.parse(body);

  const boardService = new CloseBoardService();
  const result = await boardService.bulkCreateFromSources(
    authCtx.company_id,
    authCtx.user_id,
    validatedData.period,
    validatedData.sources
  );

  return ok(result);
});
