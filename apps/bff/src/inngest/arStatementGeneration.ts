...
export const arStatementGenerationMonthly = inngest.createFunction(
    ...
    async ({ event, step }: { event: any; step: any }) => {
        console.log(`🚀 Starting AR statement generation monthly job at ${new Date().toISOString()}`);
        ...
        try {
            ...
            for (const company of companies) {
                try {
                    ...
                    const asOfDate = new Date(lastMonth.getFullYear(), lastMonth.getMonth() + 1, 0); // Last day of previous month

                    const result = await step.run(`generate-statements-${company.company_id}`, async () => {
                        // Import the service dynamically to avoid circular dependencies
                        const { ArStatementService } = await import("../../app/services/ar/statements");
                    ...
                    console.log(`✅ Generated statements for company ${company.company_id}: ${result.customers_processed} customers, ${result.artifacts_generated} artifacts`);

                } catch (error) {
                    totalErrors++;
                    console.error(`❌ Failed to generate statements for company ${company.company_id}:`, error);
                }
            }
...
export const arStatementEmailRetryHourly = inngest.createFunction(
    ...
    { cron: "0 * * * *" }, // Every hour
    async ({ event, step }: { event: any; step: any }) => {
        console.log(`🚀 Starting AR statement email retry hourly job at ${new Date().toISOString()}`);
        ...
        try {
            ...
            for (const company of companies) {
                try {
                    const result = await step.run(`retry-emails-${company.company_id}`, async () => {
                        ...
                        for (const run of failedRuns) {
                            try {
                                const emailResult = await statementService.sendStatementEmails(
                                    company.company_id,
                                    {
                                        run_id: run.id,
                                        resend_failed: true
                                ...
                                companyFailed++;
                                console.error(`Failed to retry emails for run ${run.id}:`, error);
                            }
                        }
                    ...
                    console.error(`❌ Failed to retry emails for company ${company.company_id}:`, error);
                }
            }
