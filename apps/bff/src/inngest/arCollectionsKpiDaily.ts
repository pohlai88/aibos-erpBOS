...
export const arCollectionsKpiDaily = inngest.createFunction(
    ...
    async ({ event, step }: { event: any; step: any }) => {
        console.log(`🚀 Starting AR collections KPI daily refresh at ${new Date().toISOString()}`);
        ...
        try {
            ...
            for (const company of companies) {
                try {
                    const result = await step.run(`kpi-snapshot-${company.company_id}`, async () => {
                        // Generate KPI snapshot
                        const response = await fetch(`${process.env.API_BASE_URL}/api/ar/collect/kpi`, {
                            method: 'GET',
                            headers: {
                        ...
                        if (!response.ok) {
                            throw new Error(`KPI snapshot failed for company ${company.company_id}: ${response.statusText}`);
                        }
                    ...
                    totalSnapshots++;
                    console.log(`✅ Company ${company.company_id}: KPI snapshot generated with ${result.snapshot.customers.length} customers`);

                } catch (error) {
                    console.error(`❌ Error generating KPI snapshot for company ${company.company_id}:`, error);
                    totalErrors++;
                }
            }
