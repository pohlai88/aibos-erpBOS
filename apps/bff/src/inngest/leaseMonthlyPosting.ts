...
export const leaseMonthlyPosting = inngest.createFunction(
  ...
  async ({ event, step }: { event: any; step: any }) => {
    ...
    console.log(
      `Starting monthly lease posting for ${previousYear}-${previousMonth.toString().padStart(2, '0')}`
    );
      ...
      try {
        ...
        for (const leaseRecord of leases) {
          try {
            ...
            console.error(
              `Failed to build schedule for lease ${leaseRecord.id}:`,
              error
            );
          }
        }
      ...
      try {
        ...
        for (const companyId of companies) {
          try {
            ...
            console.error(
              `Failed to generate disclosures for company ${companyId}:`,
              error
            );
            disclosureResults.push({
              company_id: companyId,
    ...
    await step.run('send-notification', async () => {
      try {
        ...
        console.log(
          `Monthly lease posting completed for ${previousYear}-${previousMonth.toString().padStart(2, '0')}`
        );
  ...
  async ({ event, step }: { event: any; step: any }) => {
    ...
    await step.run('find-leases-with-events', async () => {
      try {
        ...
        return {
          leases_found: leasesWithEvents.length,
          leases: leasesWithEvents,
        };
      } catch (error) {
