...
export const arCreditEvaluationDaily = inngest.createFunction(
    ...
    async ({ event, step }: { event: any; step: any }) => {
        console.log(`🚀 Starting AR credit evaluation daily job at ${new Date().toISOString()}`);
        ...
        try {
            ...
            for (const company of companies) {
                try {
                    const result = await step.run(`evaluate-company-${company.company_id}`, async () => {
                        // Call the credit evaluation API
                        const response = await fetch(`${process.env.API_BASE_URL}/api/ar/credit/evaluate`, {
                            method: 'POST',
                            headers: {
                        ...
                        if (!response.ok) {
                            throw new Error(`Credit evaluation failed for company ${company.company_id}: ${response.statusText}`);
                        }
                    ...
                    console.log(`✅ Company ${company.company_id}: ${result.result.customers_evaluated} customers evaluated, ${result.result.holds_triggered} holds, ${result.result.releases_triggered} releases`);

                } catch (error) {
                    console.error(`❌ Error evaluating company ${company.company_id}:`, error);
                    totalErrors++;
                }
            }
