...
describe('transform patterns', () => {
    ...
    `;
        ...
        expect(updated).toMatch(/const mapping = JSON\.parse\(data\.mapping as string\)/);
    });
    ...
    `;
        ...
        expect(updated).toMatch(/const validation = await validateFileUpload\(req, \[\]\)/);
...
export const POST = withRouteErrors(async (req: NextRequest) => {
  ...
  const file = formData.get('file') as File;
        ...
        const out = injectGuards(content, { injectLimit: true, injectAuditAttempt: true });
        expect(out).toMatch(/rateLimit\(\{/);
        expect(out).toMatch(/tooManyRequests\(/);
        expect(out).toMatch(/logAuditAttempt\(\{/);
        // key scope looks right
        expect(out).toMatch(/key:\s*`upload:\${auth\.company_id}:\${auth\.user_id}`/);
    });
...
describe('wrapper and nonstandard checks', () => {
    ...
    it('detects nonstandard routes', () => {
        expect(isNonstandard('// @api:nonstandard\n export const POST = () => {}')).toBe(true);
