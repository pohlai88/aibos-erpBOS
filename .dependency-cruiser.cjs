module.exports = {
  forbidden: [
    ...
    // BFF can only import from services, contracts, adapter, ports, policies, posting-rules
    {
      name: 'bff-boundaries',
      from: { path: '^apps/bff' },
      to: { pathNot: '^(packages/(services|contracts|adapters|ports|policies|posting-rules)|apps/bff)' }
    },
    ...
    // Services can only import from contracts, ports, policies, posting-rules
    {
      ...
      to: { pathNot: '^packages/(services|contracts|ports|policies|posting-rules)' }
    },
