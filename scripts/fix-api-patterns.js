...
const ROOT = node_path_1.default.resolve(process.cwd(), "apps/bff/app/api");
...
// Stats tracking
let updatedCount = 0, skippedRisky = 0, metaOnly = 0, totalScanned = 0;
const project = new ts_morph_1.Project({
    tsConfigFilePath: node_path_1.default.resolve(process.cwd(), "tsconfig.json"),
});
function ensureKitImport(sf) {
    ...
    const kitImports = sf.getImportDeclarations().filter(imp => imp.getModuleSpecifierValue() === KIT_MODULE);
    if (kitImports.length > 1) {
        const allNames = new Set();
        kitImports.forEach(imp => imp.getNamedImports().forEach(n => allNames.add(n.getName())));
        ...
        [...allNames].forEach(n => { if (!existing.has(n))
            primary.addNamedImport(n); });
    }
    const existing = new Set();
    let kitDecl = sf.getImportDeclarations().find(imp => imp.getModuleSpecifierValue() === KIT_MODULE);
    ...
    const needs = new Set();
    if (text.includes("withRouteErrors("))
        needs.add("withRouteErrors");
    if (/\bisResponse\(/.test(text))
        needs.add("isResponse");
    if (/\bok\(/.test(text))
        needs.add("ok");
    if (/\bbadRequest\(/.test(text))
        needs.add("badRequest");
    const toAdd = [...needs].filter(n => !existing.has(n));
    if (toAdd.length === 0)
        return false;
    if (!kitDecl) {
        sf.addImportDeclaration({ moduleSpecifier: KIT_MODULE, namedImports: toAdd });
    }
    else {
        const names = new Set(kitDecl.getNamedImports().map(n => n.getName()));
        toAdd.forEach(n => { if (!names.has(n))
            kitDecl.addNamedImport(n); });
    }
    return true;
}
function wrapWithRouteErrors(sf) {
    const exports = sf.getExportedDeclarations();
    let hasChanges = false;
    // If file already has const exports wrapped, skip those
    const wrappedConst = new Set(sf.getVariableStatements()
        .filter(vs => vs.getDeclarationKind() === ts_morph_1.VariableDeclarationKind.Const && /export/.test(vs.getText()))
        ...
        for (const d of decls) {
            if (d.getKind() === ts_morph_1.SyntaxKind.FunctionDeclaration) {
                ...
                if (REMOVE_OPTIONS && methodName === "OPTIONS") {
                    ...
                    const harmless = /new\s+Response\(null,\s*\{\s*status:\s*204[\s\S]*?\}\s*\)/.test(body);
                    if (!harmless) {
                        console.warn("⚠️  OPTIONS has custom logic, skip removal:", sf.getFilePath());
                    }
                    else {
                        fn.remove();
                        hasChanges = true;
                ...
                if (!["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"].includes(methodName))
                    continue;
                if (wrappedConst.has(methodName))
                    continue; // already exported as wrapped const
                // Skip if already wrapped
                const bodyText = fn.getBodyText() ?? "";
                if (bodyText.includes("withRouteErrors"))
                    continue;
    ...
    sf.forEachDescendant((n) => {
        if (n.getKind() === ts_morph_1.SyntaxKind.CallExpression) {
            ...
            if (text === "Response.json" || text === "NextResponse.json") {
                ...
                const second = args[1];
                if (second) {
                    const num = second.getFirstDescendantByKind?.(ts_morph_1.SyntaxKind.NumericLiteral)?.getText();
                    if (num && /^\d{3}$/.test(num))
                        status = num;
                    // { status: 201, headers: ... }
                    const statusProp = second.getFirstDescendant?.((d) => d.getKind() === ts_morph_1.SyntaxKind.PropertyAssignment &&
                        d.getFirstChild()?.getText() === "status");
                    const val = statusProp?.getLastChild()?.getText();
                    if (!status && val && /^\d{3}$/.test(val))
                        status = val;
                    // If headers present, warn (prefer manual migration / global CORS)
                    if (second.getText().includes("headers") || second.getText().includes("cookies") ||
                        second.getText().includes("content-type") || second.getText().includes("cache")) {
                        console.warn("⚠️  Complex Response.json options detected; consider manual check:", sf.getFilePath());
                    }
                }
                const replacement = status ? `ok(${dataArg}, ${status})` : `ok(${dataArg})`;
...
function fixAuthInstanceof(sf) {
    if (!USE_IS_RESPONSE)
        return false; // keep current idiom by default
    const originalText = sf.getFullText();
    // Replace ANY <ident> instanceof Response  -> isResponse(<ident>)
    const replaced = originalText.replace(/\b([A-Za-z_]\w*)\s+instanceof\s+Response\b/g, "isResponse($1)");
...
function processFile(filePath) {
    try {
        ...
        const onlyMeta = /\bexport\s+const\s+(runtime|preferredRegion|dynamic|revalidate)\b/.test(text0)
            && !/\bexport\s+(async\s+function|const)\s+(GET|POST|PUT|PATCH|DELETE|OPTIONS)\b/.test(text0);
        if (onlyMeta) {
            if (DRY_RUN)
        ...
        if (!FORCE) {
            ...
            const risky = t.includes("new Response(") ||
                t.includes("ReadableStream") ||
                t.includes("NextResponse.redirect");
            if (risky) {
                if (DRY_RUN)
                    console.log("⏭️  Would skip risky file (use --force to include):", filePath);
                else
...
if (REMOVE_OPTIONS)
    console.log("🗑️  Will remove per-file OPTIONS handlers");
if (!USE_IS_RESPONSE)
    console.log("🔒 Keeping `instanceof Response` (add --use-isresponse to convert)");
if (!FORCE)
    console.log("🛡️ Safe mode ON (skips streams/redirects/new Response) — add --force to override");
walk(ROOT);
if (!DRY_RUN)
    project.saveSync();
console.log(DRY_RUN ? "🧪 Dry run completed." : "✅ API pattern refactoring completed!");
console.log("💡 Run 'pnpm api:wrap:check' to verify all routes are properly wrapped");
console.log(`📊 Summary: updated=${updatedCount} skipped_risky=${skippedRisky} meta_only=${metaOnly} total_scanned=${totalScanned}`);
