...
function detectCategory(txt) {
    if (/new\s+Response\s*\(/.test(txt))
        return "new-response";
    if (/\bReadableStream\b/.test(txt))
        return "stream";
    if (/NextResponse\.redirect/.test(txt))
        return "redirect";
    if (/headers\s*:\s*\{/.test(txt))
...
function methodHits(txt) {
    ...
    for (const m of METHODS) {
        if (new RegExp(`\\bexport\\s+(?:const|async\\s+function)\\s+${m}\\b`).test(txt))
            hits.push(m);
    }
    return hits;
}
...
const hardFailures = (groups["missing-wrapper"]?.length ?? 0) +
