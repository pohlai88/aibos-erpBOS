...
function detectUploadPattern(content) {
  if (
    /formData\.get\(['"]file['"]\)/.test(content) &&
    /JSON\.parse\(formData\.get\(['"]mapping['"]\)/.test(content)
  ) {
    return 'multipart-with-mapping';
  }
  if (
    /await\s+req\.json\(\)/.test(content) &&
    /BankFileImport\.parse/.test(content)
  ) {
    return 'json-import';
  }
...
function analyzeHardeningStatus(content) {
  const status = {
    ...
    hasFileUploadImports: /from\s*["']@\/api\/_lib\/file-upload["']/.test(
      content
    ),
  ...
  status.isFullyHardened =
    status.hasRuntime &&
    status.hasFileUploadResponse &&
    status.hasValidateFileUpload;
...
function insertImportsAtTop(content, importLines) {
  ...
  const result = [...beforeImports, ...importLines, ...afterImports].join('\n');
...
function manageImports(content) {
  ...
  const needsValidateFileUpload =
    !imports.fileUpload.includes('validateFileUpload');
  ...
  const needsAuditAttempt =
    INJECT_AUDIT_ATTEMPT && !imports.audit.includes('logAuditAttempt');
  const needsWithRouteErrors =
    INJECT_WRAP &&
  ...
  if (needsFileUploadResponse || needsServerError) {
    ...
    newImports.push(
      `import { ${Array.from(new Set(httpImports)).join(', ')} } from "@/api/_lib/http";`
    );
  }
  ...
  if (INJECT_LIMIT) {
    if (needsRate)
      newImports.push(`import { rateLimit } from "${RATE_MODULE}";`);
    if (needsTooMany) {
      if (!imports.kit.length) {
        ...
        updated = updated.replace(
          new RegExp(
            `import\\s*\\{([\\s\\S]*?)\\}\\s*from\\s*["']${KIT_MODULE}["']`
          ),
          (_m, g1) => {
            const set = new Set(
              g1
...
function transformUploadPattern(content, pattern) {
  ...
  switch (pattern) {
    ...
    case 'csv-direct':
      ...
      const csvPattern =
        /const\s+csvFile\s*=\s*formData\.get\(['"]csv['"]\)\s+as\s+File;/;
...
function convertResponsePatterns(content) {
  ...
  const successPattern =
    /return\s+Response\.json\(\{\s*result,\s*message:\s*result\.success\s*\?\s*['"'][^'"]*['"]\s*:\s*['"'][^'"]*['"]\s*\},\s*\{\s*status:\s*result\.success\s*\?\s*200\s*:\s*400,\s*headers:\s*\{\s*['"]Access-Control-Allow-Origin['"]:\s*['"]\*['"]\s*\}\s*\}\);/;
  ...
  errorPatterns.forEach(pattern => {
    updated = updated.replace(
      pattern,
      'return serverError("Failed to process file upload");'
    );
  });
...
function injectGuards(content, options = {}) {
  ...
  if (verbose)
    console.log('   ✅ Capability check pattern found, injecting guards');
  ...
  if (INJECT_AUDIT_ATTEMPT && !/logAuditAttempt\(/.test(updated)) {
    const altFilePattern =
      /(const\s*\{\s*file\b[^=]*=\s*validation\s*;[^\n]*\n)/;
...
function injectWrapper(content) {
  ...
  for (const m of methods) {
    ...
    if (pattern.test(updated)) {
      updated = updated.replace(
        pattern,
        `export const ${m} = withRouteErrors(async (`
      );
    }
  }
  return updated;
...
function processFile(filePath) {
  try {
    ...
    if (!FORCE) {
      ...
      if (risky) {
        console.log(
          `⏭️  Skipping risky file (use --force to include): ${filePath}`
        );
        skippedRisky++;
        return;
      }
    ...
    if (isNonstandard(content) && !FORCE) {
      if (VERBOSE)
        console.log(
          '   ⏭️  @api:nonstandard, skipping (use --force to override)'
        );
      skippedCount++;
      return;
    }
    ...
    if (!hasWrapper(content) && !FORCE && !INJECT_WRAP) {
      if (VERBOSE)
        console.log(
          '   ⏭️  Not standardized (withRouteErrors). Use --wrap or --force.'
        );
      skippedCount++;
      return;
    }
    ...
    if (
      !status.hasFileUploadResponse ||
      status.usesResponseJson ||
      status.usesNextResponseJson
    ) {
    ...
    if (hasChanges) {
      ...
      if (VERBOSE)
        console.log(`   Pattern: ${pattern}, Changes: ${hasChanges}`);
      updatedCount++;
    } else {
...
console.log('\n' + '='.repeat(60));
console.log(
  DRY_RUN ? '🧪 Dry run completed.' : '✅ API security hardening completed!'
);
console.log(`⏱️  Execution time: ${endTime - startTime}ms`);
console.log(
  `📊 Summary: updated=${updatedCount} skipped=${skippedCount} skipped_risky=${skippedRisky} meta_only=${metaOnly} errors=${errorCount} total_scanned=${totalScanned}`
);
console.log(
...
console.log(
  '💡 Use --limit and --audit-attempt flags to inject security guards'
);
