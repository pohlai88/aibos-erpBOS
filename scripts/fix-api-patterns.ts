...
function ensureKitImport(sf: import("ts-morph").SourceFile) {
  ...
  if (kitImports.length > 1) {
    ...
    [...allNames].forEach(n => { if (!existing.has(n)) primary.addNamedImport(n); });
  }
  ...
  if (!kitDecl) {
    ...
    toAdd.forEach(n => { if (!names.has(n)) kitDecl!.addNamedImport(n); });
  }
  return true;
}
...
function wrapWithRouteErrors(sf: import("ts-morph").SourceFile) {
  ...
  const wrappedConst = new Set(
    sf.getVariableStatements()
      .filter(vs => vs.getDeclarationKind() === VariableDeclarationKind.Const && /export/.test(vs.getText()))
  ...
  sf.forEachDescendant((n: any) => {
    if (n.getKind() === SyntaxKind.CallExpression) {
      ...
      if (text === "Response.json" || text === "NextResponse.json") {
        ...
        const second = args[1];
        if (second) {
          const num = second.getFirstDescendantByKind?.(SyntaxKind.NumericLiteral)?.getText();
          if (num && /^\d{3}$/.test(num)) status = num;
          // { status: 201, headers: ... }
          const statusProp = second.getFirstDescendant?.((d: any) =>
...
function processFile(filePath: string) {
  try {
    ...
    if (!FORCE) {
      ...
      if (risky) {
        if (DRY_RUN) console.log("⏭️  Would skip risky file (use --force to include):", filePath);
        else console.log("⏭️  Skipping risky file (use --force to include):", filePath);
        skippedRisky++;
        return;
      }
    }
...
if (!USE_IS_RESPONSE) console.log("🔒 Keeping `instanceof Response` (add --use-isresponse to convert)");
if (!FORCE) console.log("🛡️ Safe mode ON (skips streams/redirects/new Response) — add --force to override");
