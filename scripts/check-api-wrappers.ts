...
type Finding = {
  file: string;
  reason: "missing-wrapper" | "new-response" | "custom-headers" | "stream" | "redirect" | "meta-only" | "approved-exception";
  methodHits: string[];
};
...
function isMetaOnly(txt: string) {
  const hasMeta = /\bexport\s+const\s+(runtime|preferredRegion|dynamic|revalidate)\b/.test(txt);
  const hasHandlers = new RegExp(`\\bexport\\s+(async\\s+function|const)\\s+(${METHODS.join("|")})\\b`).test(txt);
...
function hasWrapper(txt: string) {
  ...
  return /\bwithRouteErrors\s*\(/.test(txt) && /\b(GET|POST|PUT|PATCH|DELETE|OPTIONS)\b/.test(txt);
}
...
function detectCategory(txt: string): Omit<Finding, "file" | "methodHits">["reason"] | null {
...
function methodHits(txt: string) {
  ...
  for (const m of METHODS) {
    if (new RegExp(`\\bexport\\s+(?:const|async\\s+function)\\s+${m}\\b`).test(txt)) hits.push(m);
  }
  return hits;
}
...
function scan(file: string) {
  ...
  if (cat) {
    ...
    if (approvedException(txt)) {
      findings.push({ file, reason: "approved-exception", methodHits: methods });
    } else {
...
const groups = findings.reduce<Record<string, Finding[]>>((acc, f) => {
  (acc[f.reason] ||= []).push(f); return acc;
}, {});
...
function printGroup(key: string, title: string) {
  ...
  for (const f of items) {
    console.log(`- ${f.file}${f.methodHits.length ? ` [${f.methodHits.join(",")}]` : ""}`);
  }
}
