...
export const soxKeyControl = pgTable(
  'sox_key_control',
  {
  ...
  table => ({
    companyCodeIdx: index('idx_sox_key_control_company_code').on(
      table.companyId,
      table.code
    ),
    companyProcessIdx: index('idx_sox_key_control_company_process').on(
      table.companyId,
      table.process
    ),
    companyOwnerIdx: index('idx_sox_key_control_company_owner').on(
      table.companyId,
      table.ownerId
    ),
    companyActiveIdx: index('idx_sox_key_control_company_active').on(
      table.companyId,
      table.active
    ),
  })
...
export const soxControlScope = pgTable(
  'sox_control_scope',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    companyId: text('company_id').notNull(),
    controlId: uuid('control_id')
      .notNull()
      .references(() => soxKeyControl.id, { onDelete: 'cascade' }),
  ...
  table => ({
    companyPeriodIdx: index('idx_sox_control_scope_company_period').on(
      table.companyId,
      table.period
    ),
    controlPeriodIdx: index('idx_sox_control_scope_control_period').on(
      table.controlId,
      table.period
    ),
  })
...
// Testing Tables
export const soxTestPlan = pgTable(
  'sox_test_plan',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    companyId: text('company_id').notNull(),
  ...
  table => ({
    companyPeriodIdx: index('idx_sox_test_plan_company_period').on(
      table.companyId,
      table.period
    ),
    controlPeriodIdx: index('idx_sox_test_plan_control_period').on(
      table.controlId,
      table.period
    ),
    statusIdx: index('idx_sox_test_plan_status').on(
      table.status,
      table.preparedAt
    ),
  })
...
export const soxTestSample = pgTable(
  'sox_test_sample',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    companyId: text('company_id').notNull(),
    planId: uuid('plan_id')
      .notNull()
      .references(() => soxTestPlan.id, { onDelete: 'cascade' }),
...
export const soxTestResult = pgTable(
  'sox_test_result',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    companyId: text('company_id').notNull(),
    planId: uuid('plan_id')
      .notNull()
      .references(() => soxTestPlan.id, { onDelete: 'cascade' }),
    sampleId: uuid('sample_id').references(() => soxTestSample.id, {
      onDelete: 'set null',
    }),
    attribute: text('attribute').notNull(),
    outcome: text('outcome').notNull(), // PASS|FAIL|N/A
    note: text('note'),
    evdRecordId: uuid('evd_record_id').references(() => evdRecord.id, {
      onDelete: 'set null',
    }),
  ...
  table => ({
    companyPlanIdx: index('idx_sox_test_result_company_plan').on(
      table.companyId,
      table.planId
    ),
    planTestedAtIdx: index('idx_sox_test_result_plan_tested_at').on(
      table.planId,
      table.testedAt
    ),
    outcomeIdx: index('idx_sox_test_result_outcome').on(
      table.outcome,
      table.testedAt
    ),
...
// Deficiency Tables
export const soxDeficiency: any = pgTable(
  'sox_deficiency',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    companyId: text('company_id').notNull(),
    controlId: uuid('control_id').references(() => soxKeyControl.id, {
      onDelete: 'set null',
    }),
    ...
    aggregationId: uuid('aggregation_id').references(
      (): any => soxDeficiency.id,
      { onDelete: 'set null' }
    ),
  ...
  table => ({
    companyStatusIdx: index('idx_sox_deficiency_company_status').on(
      table.companyId,
      table.status
    ),
    companySeverityIdx: index('idx_sox_deficiency_company_severity').on(
      table.companyId,
      table.severity
    ),
    statusSeverityIdx: index('idx_sox_deficiency_status_severity').on(
      table.status,
      table.severity
    ),
    discoveredInIdx: index('idx_sox_deficiency_discovered_in').on(
      table.discoveredIn
    ),
...
export const soxDeficiencyLink = pgTable(
  'sox_deficiency_link',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    deficiencyId: uuid('deficiency_id')
      .notNull()
      .references(() => soxDeficiency.id, { onDelete: 'cascade' }),
    source: text('source').notNull(), // TEST_RESULT|INCIDENT|AUDIT
    sourceId: text('source_id').notNull(),
  },
  table => ({
    deficiencyIdx: index('idx_sox_deficiency_link_deficiency').on(
      table.deficiencyId
    ),
    sourceIdx: index('idx_sox_deficiency_link_source').on(
      table.source,
      table.sourceId
    ),
  })
...
// Assertion Tables
export const soxAssertion = pgTable(
  'sox_assertion',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    companyId: text('company_id').notNull(),
    ...
    ebinderId: uuid('ebinder_id').references(() => evdBinder.id, {
      onDelete: 'set null',
    }),
  ...
  table => ({
    companyTypeIdx: index('idx_sox_assertion_company_type').on(
      table.companyId,
      table.type
    ),
    companyPeriodIdx: index('idx_sox_assertion_company_period').on(
      table.companyId,
      table.period
    ),
    typePeriodIdx: index('idx_sox_assertion_type_period').on(
      table.type,
      table.period
    ),
...
export const soxControlScopeRelations = relations(
  soxControlScope,
  ({ one }) => ({
...
export const soxTestSampleRelations = relations(
  soxTestSample,
  ({ one, many }) => ({
...
export const soxDeficiencyRelations = relations(
  soxDeficiency,
  ({ one, many }) => ({
    control: one(soxKeyControl, {
...
export const soxDeficiencyLinkRelations = relations(
  soxDeficiencyLink,
  ({ one }) => ({
