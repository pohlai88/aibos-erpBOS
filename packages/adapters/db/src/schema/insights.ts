...
export const insFactClose = pgTable(
  'ins_fact_close',
  {
  ...
  table => ({
    companyRunIdx: index('ins_fact_close_company_run_idx').on(
      table.companyId,
      table.runId
    ),
    periodIdx: index('ins_fact_close_period_idx').on(
      table.companyId,
      table.year,
      table.month
    ),
...
export const insFactTask = pgTable(
  'ins_fact_task',
  {
    id: text('id').primaryKey(),
    runId: text('run_id')
  ...
  table => ({
    ...
    ownerStatusIdx: index('ins_fact_task_owner_status_idx').on(
      table.owner,
      table.status
    ),
    breachedIdx: index('ins_fact_task_breached_idx').on(
      table.breached,
      table.ageHours
    ),
...
export const insFactCtrl = pgTable(
  'ins_fact_ctrl',
  {
    id: text('id').primaryKey(),
  ...
  table => ({
    ...
    statusSeverityIdx: index('ins_fact_ctrl_status_severity_idx').on(
      table.status,
      table.severity
    ),
    materialFailIdx: index('ins_fact_ctrl_material_fail_idx').on(
      table.materialFail,
      table.exceptionsCount
    ),
  })
);

// Insights Flux Facts Table
export const insFactFlux = pgTable(
  'ins_fact_flux',
  {
    id: text('id').primaryKey(),
    fluxRunId: text('flux_run_id')
      .notNull()
  ...
  table => ({
    ...
    deltaIdx: index('ins_fact_flux_delta_idx').on(
      table.topDeltaAbs,
      table.topDeltaPct
    ),
  })
...
export const insFactCert = pgTable(
  'ins_fact_cert',
  {
    id: text('id').primaryKey(),
    runId: text('run_id')
  ...
  table => ({
    ...
    levelRoleIdx: index('ins_fact_cert_level_role_idx').on(
      table.level,
      table.signerRole
    ),
  })
...
export const insBenchBaseline = pgTable(
  'ins_bench_baseline',
  {
    id: text('id').primaryKey(),
    companyId: text('company_id').notNull(),
    entityGroup: text('entity_group', {
      enum: ['SELF', 'PEER', 'GLOBAL'],
    }).notNull(),
  ...
  table => ({
    companyMetricIdx: index('ins_bench_baseline_company_metric_idx').on(
      table.companyId,
      table.metric,
      table.entityGroup
    ),
    windowIdx: index('ins_bench_baseline_window_idx').on(
      table.windowStart,
      table.windowEnd
    ),
  })
...
export const insBenchTarget = pgTable(
  'ins_bench_target',
  {
    id: text('id').primaryKey(),
    companyId: text('company_id').notNull(),
  ...
  table => ({
    companyMetricIdx: index('ins_bench_target_company_metric_idx').on(
      table.companyId,
      table.metric,
      table.effectiveFrom
    ),
...
export const insAnomaly = pgTable(
  'ins_anomaly',
  {
    id: text('id').primaryKey(),
    companyId: text('company_id').notNull(),
    ...
    kind: text('kind', {
      enum: ['TASK', 'CONTROL', 'FLUX', 'DURATION'],
    }).notNull(),
  ...
  table => ({
    companyRunIdx: index('ins_anomaly_company_run_idx').on(
      table.companyId,
      table.runId
    ),
    scoreSeverityIdx: index('ins_anomaly_score_severity_idx').on(
      table.score,
      table.severity
    ),
...
export const insReco = pgTable(
  ...
  table => ({
    companyRunIdx: index('ins_reco_company_run_idx').on(
      table.companyId,
      table.runId
    ),
    statusIdx: index('ins_reco_status_idx').on(table.status, table.createdAt),
    effortImpactIdx: index('ins_reco_effort_impact_idx').on(
      table.effort,
