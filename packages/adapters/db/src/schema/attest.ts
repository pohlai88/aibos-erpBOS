...
export const attestCampaign = pgTable("attest_campaign", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull(),
    programId: uuid("program_id").notNull().references(() => attestProgram.id, { onDelete: "restrict" }),
    templateId: uuid("template_id").notNull().references(() => attestTemplate.id, { onDelete: "restrict" }),
...
export const attestTask = pgTable("attest_task", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull(),
    campaignId: uuid("campaign_id").notNull().references(() => attestCampaign.id, { onDelete: "cascade" }),
...
export const attestResponse = pgTable("attest_response", {
    id: uuid("id").primaryKey().defaultRandom(),
    taskId: uuid("task_id").notNull().references(() => attestTask.id, { onDelete: "cascade" }),
...
export const attestPack = pgTable("attest_pack", {
    id: uuid("id").primaryKey().defaultRandom(),
    taskId: uuid("task_id").notNull().unique().references(() => attestTask.id, { onDelete: "cascade" }),
...
export const attestAssignment = pgTable("attest_assignment", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull(),
    programId: uuid("program_id").notNull().references(() => attestProgram.id, { onDelete: "cascade" }),
