...
export const itSystem = pgTable("it_system", {
    ...
    companyCodeIdx: index("idx_it_system_company_code").on(table.companyId, table.code),
    companyActiveIdx: index("idx_it_system_company_active").on(table.companyId, table.isActive),
}));

// Connector profiles for data ingestion
export const itConnectorProfile = pgTable("it_connector_profile", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
...
export const itUser = pgTable("it_user", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
...
export const itRole = pgTable("it_role", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
    code: text("code").notNull(),
...
export const itEntitlement = pgTable("it_entitlement", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
    ...
    companySystemKindCodeIdx: index("idx_it_entitlement_company_system_kind_code").on(table.companyId, table.systemId, table.kind, table.code),
    companySystemIdx: index("idx_it_entitlement_company_system").on(table.companyId, table.systemId),
    kindIdx: index("idx_it_entitlement_kind").on(table.kind),
}));

// User grants (assignments of entitlements to users)
export const itGrant = pgTable("it_grant", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
    userId: uuid("user_id").notNull().references(() => itUser.id, { onDelete: "cascade" }),
    entitlementId: uuid("entitlement_id").notNull().references(() => itEntitlement.id, { onDelete: "cascade" }),
    ...
    companySystemUserEntitlementIdx: index("idx_it_grant_company_system_user_entitlement").on(table.companyId, table.systemId, table.userId, table.entitlementId),
...
export const itSodRule = pgTable("it_sod_rule", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    code: text("code").notNull(),
    name: text("name").notNull(),
    ...
    companyCodeIdx: index("idx_it_sod_rule_company_code").on(table.companyId, table.code),
    companyActiveIdx: index("idx_it_sod_rule_company_active").on(table.companyId, table.active),
    severityIdx: index("idx_it_sod_rule_severity").on(table.severity),
}));

// SoD violations detected by the engine
export const itSodViolation = pgTable("it_sod_violation", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    ruleId: uuid("rule_id").notNull().references(() => itSodRule.id, { onDelete: "cascade" }),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
    userId: uuid("user_id").notNull().references(() => itUser.id, { onDelete: "cascade" }),
    ...
    ruleUserDetectedIdx: index("idx_it_sod_violation_rule_user_detected").on(table.ruleId, table.userId, table.detectedAt),
    companyStatusIdx: index("idx_it_sod_violation_company_status").on(table.companyId, table.status),
...
export const uarCampaign = pgTable("uar_campaign", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    code: text("code").notNull(),
    name: text("name").notNull(),
    ...
    companyCodeIdx: index("idx_uar_campaign_company_code").on(table.companyId, table.code),
    companyStatusIdx: index("idx_uar_campaign_company_status").on(table.companyId, table.status),
    dueAtIdx: index("idx_uar_campaign_due_at").on(table.dueAt),
    periodIdx: index("idx_uar_campaign_period").on(table.periodStart, table.periodEnd),
}));

// Individual UAR items (users to be reviewed)
export const uarItem = pgTable("uar_item", {
    id: uuid("id").primaryKey().defaultRandom(),
    campaignId: uuid("campaign_id").notNull().references(() => uarCampaign.id, { onDelete: "cascade" }),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
    userId: uuid("user_id").notNull().references(() => itUser.id, { onDelete: "cascade" }),
    ownerUserId: text("owner_user_id").notNull(),
    ...
    campaignUserSystemIdx: index("idx_uar_item_campaign_user_system").on(table.campaignId, table.userId, table.systemId),
...
export const itBreakglass = pgTable("it_breakglass", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    systemId: uuid("system_id").notNull().references(() => itSystem.id, { onDelete: "cascade" }),
    userId: uuid("user_id").notNull().references(() => itUser.id, { onDelete: "cascade" }),
...
export const itSnapshot = pgTable("it_snapshot", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: text("company_id").notNull(),
    ...
    companyScopeTakenIdx: index("idx_it_snapshot_company_scope_taken").on(table.companyId, table.scope, table.takenAt),
    companyScopeIdx: index("idx_it_snapshot_company_scope").on(table.companyId, table.scope),
...
export const uarPack = pgTable("uar_pack", {
    id: uuid("id").primaryKey().defaultRandom(),
    campaignId: uuid("campaign_id").notNull().references(() => uarCampaign.id, { onDelete: "cascade" }),
    sha256: text("sha256").notNull(),
...
export const itConnectorProfileRelations = relations(itConnectorProfile, ({ one }) => ({
    system: one(itSystem, {
...
export const itEntitlementRelations = relations(itEntitlement, ({ one, many }) => ({
    system: one(itSystem, {
