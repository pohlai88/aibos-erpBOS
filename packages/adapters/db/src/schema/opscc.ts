...
export const boardConfig = pgTable('board_config', {
  ...
  default_present_ccy: char('default_present_ccy', { length: 3 })
    .notNull()
    .default('USD'),
  layout: jsonb('layout'),
  acl: jsonb('acl'),
...
export const opsPlaybookLegacy = pgTable('ops_playbook_legacy', {
  ...
  require_dual_control: boolean('require_dual_control')
    .notNull()
    .default(false),
...
export const opsFireStep = pgTable('ops_fire_step', {
  ...
  status: text('status')
    .notNull()
    .$type<'PENDING' | 'OK' | 'FAILED' | 'RETRIED' | 'SKIPPED'>()
    .default('PENDING'),
  duration_ms: integer('duration_ms'),
  error_message: text('error_message'),
  result: jsonb('result'),
...
export const opsPlaybookVersion = pgTable(
  'ops_playbook_version',
  {
    id: text('id').primaryKey(),
    company_id: text('company_id').notNull(),
    playbook_id: text('playbook_id')
      .notNull()
      .references(() => opsPlaybook.id, { onDelete: 'cascade' }),
  ...
  table => ({
    uniquePlaybookVersion: unique('unique_playbook_version').on(
      table.company_id,
      table.playbook_id,
      table.version_no
    ),
...
export const opsGuardPolicy = pgTable('ops_guard_policy', {
  ...
  requires_dual_control: boolean('requires_dual_control')
    .notNull()
    .default(false),
...
export const opsRun = pgTable('ops_run', {
  ...
  playbook_version_id: text('playbook_version_id')
    .notNull()
    .references(() => opsPlaybookVersion.id),
  trigger: text('trigger')
    .notNull()
    .$type<'cron' | 'signal' | 'manual' | 'canary'>(),
  status: text('status')
    .notNull()
    .$type<
      | 'queued'
...
export const opsRunStep = pgTable('ops_run_step', {
  ...
  run_id: text('run_id')
    .notNull()
    .references(() => opsRun.id, { onDelete: 'cascade' }),
  ...
  status: text('status')
    .notNull()
    .$type<'pending' | 'running' | 'succeeded' | 'failed' | 'rolled_back'>()
    .default('pending'),
  duration_ms: integer('duration_ms'),
...
export const opsRollbackStep = pgTable('ops_rollback_step', {
  id: text('id').primaryKey(),
  run_step_id: text('run_step_id')
    .notNull()
    .references(() => opsRunStep.id, { onDelete: 'cascade' }),
  action_code: text('action_code').notNull(),
  input_jsonb: jsonb('input_jsonb').notNull(),
  status: text('status')
    .notNull()
...
export const opsCap = pgTable(
  'ops_cap',
  {
    ...
    capability: text('capability')
      .notNull()
      .$type<
        'ops:playbook:approve' | 'ops:playbook:execute' | 'ops:run:read'
      >(),
  ...
  table => ({
    uniqueCapability: unique('unique_capability').on(
      table.company_id,
      table.playbook_code,
      table.capability,
      table.role
...
export const opsApprovalRequest = pgTable('ops_approval_request', {
  ...
  approval_type: text('approval_type')
    .notNull()
    .$type<'DUAL_CONTROL' | 'BLAST_RADIUS' | 'CANARY_PROMOTION'>(),
...
export const opsBlastRadiusLog = pgTable('ops_blast_radius_log', {
  ...
  blast_radius_percentage: numeric('blast_radius_percentage', {
    precision: 5,
    scale: 2,
  }),
  created_at: timestamp('created_at', { withTimezone: true })
