...
export const ctrlAssignment = pgTable("ctrl_assignment", {
    id: text("id").primaryKey(),
    controlId: text("control_id").notNull().references(() => ctrlControl.id, { onDelete: "cascade" }),
    runId: text("run_id").references(() => closeRun.id, { onDelete: "cascade" }),
    taskId: text("task_id").references(() => closeTask.id, { onDelete: "cascade" }),
...
export const ctrlRun = pgTable("ctrl_run", {
    ...
    companyId: text("company_id").notNull(),
    controlId: text("control_id").notNull().references(() => ctrlControl.id, { onDelete: "cascade" }),
    assignmentId: text("assignment_id").references(() => ctrlAssignment.id, { onDelete: "cascade" }),
    runId: text("run_id").references(() => closeRun.id, { onDelete: "cascade" }),
...
export const certSignoff = pgTable("cert_signoff", {
    ...
    statementId: text("statement_id").notNull().references(() => certStatement.id, { onDelete: "restrict" }),
