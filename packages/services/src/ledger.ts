  ...
  async insertJournal(
    ...
    if (this.ledgerRepo && this.txManager) {
      ...
      return await this.txManager.run(async (tx: Tx) => {
        ...
        const exists = await this.ledgerRepo!.existsByKey(
          journal.idempotency_key,
          tx
        );
        if (exists) {
          throw new Error(
            `Journal with idempotency key ${journal.idempotency_key} already exists`
          );
        }
  ...
  async trialBalance(
    ...
    if (this.ledgerRepo && this.txManager) {
      ...
      return await this.txManager.run(async (tx: Tx) => {
        ...
        const rows = await this.ledgerRepo.trialBalance(
          companyId,
          currency,
          tx
        );
