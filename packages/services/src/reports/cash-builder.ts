...
function makeMoney(currency: string, amount: number): CashMoney { return { currency, amount }; }

export async function buildCashReport(args: { companyId: string; year: number; month?: number }) {
    ...
    const lines: CashReportLine[] = Array.from(map.entries()).map(([id, rec]) => {
        ...
        return { id, name: rec.name, values: rec.values, total: makeMoney(rec.currency, totalAmt) };
    });
...
async function fetchLines(companyId: string, year: number, month?: number): Promise<RawLine[]> {
