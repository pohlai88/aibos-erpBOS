...
export async function getFxQuotesForDateOrBefore(pool: Pool, from: string, to: string, onISO: string, daysBack = 30) {
...
export async function computeBaseAmounts(
    ...
    for (const line of lines) {
        if (line.currency === baseCurrency) {
            ...
            const quotes = await getFxQuotesForDateOrBefore(pool, line.currency, baseCurrency, docDate);
