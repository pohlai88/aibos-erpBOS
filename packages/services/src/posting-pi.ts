...
function mapLines(
  ...
  return rule[kind].map((l: any) => {
    const money = get(pi, l.amountField) as any;
    ...
    if (l.party?.type && l.party.field) {
      const partyId = get(pi, l.party.field) as string;
      if (partyId) {
...
export async function postPurchaseInvoice(
  ...
  let fxData = {
    baseCurrency: pi.currency,
    rateUsed: 1.0,
    baseAmounts: lines.map(l => parseFloat(l.amount.amount)),
  };
  ...
  if (deps.pool) {
    try {
      fxData = await computeBaseAmounts(
        ...
        pi.doc_date,
        lines.map(l => ({
          amount: parseFloat(l.amount.amount),
          currency: l.currency,
        }))
      );
    } catch (error) {
  ...
  lines.forEach((line, index) => {
    const baseAmount =
      fxData.baseAmounts[index] ?? parseFloat(line.amount.amount);
    line.base_amount = {
  ...
  if (
    ...
    try {
      ...
      const taxRule = await deps.resolveTaxRule(
        deps.pool,
        pi.company_id,
        taxCodeId,
        pi.doc_date
      ...
      if (taxRule) {
        ...
        const netAmount = lines.reduce((sum, line) => {
          ...
          return (
            sum + parseFloat(line.base_amount?.amount ?? line.amount.amount)
          );
        }, 0);
        ...
        const inputAccount = await deps.mapTaxAccount(
          deps.pool,
          pi.company_id,
          taxCodeId,
          'input'
        ...
        if (inputAccount) {
          const taxLine: JournalLine = {
            ...
            amount: {
              amount: tax.tax_amount.toFixed(2),
              currency: fxData.baseCurrency,
            },
            currency: fxData.baseCurrency,
            base_amount: {
