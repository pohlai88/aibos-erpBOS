...
// Enums
export const CloseItemKindSchema = z.enum(["TASK", "AUTO_CTRL", "SOX_TEST", "DEFICIENCY", "FLUX", "CERT"]);
export const CloseItemStatusSchema = z.enum(["OPEN", "IN_PROGRESS", "BLOCKED", "DONE", "DEFERRED"]);
export const CloseItemSeveritySchema = z.enum(["LOW", "NORMAL", "HIGH", "CRITICAL"]);
export const CloseItemSlaStateSchema = z.enum(["OK", "DUE_SOON", "LATE", "ESCALATED"]);
export const CloseItemActionSchema = z.enum(["ACK", "REASSIGN", "DEFER", "COMPLETE", "REOPEN"]);
export const CloseProcessSchema = z.enum(["R2R", "P2P", "O2C", "Treasury", "Tax"]);
...
export const CloseIngestRequestSchema = z.object({
    period: z.string().regex(/^\d{4}-\d{2}$/),
    sources: z.array(z.enum(["controls", "sox", "deficiencies", "flux", "certs", "tasks"])).optional(),
});
...
export type CloseCommentResponseType = z.infer<typeof CloseCommentResponseSchema>;
...
export type CloseEvdLinkResponseType = z.infer<typeof CloseEvdLinkResponseSchema>;
