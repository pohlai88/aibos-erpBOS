...
// Enums
export const AuditGrantScopeSchema = z.enum(["ATTEST_PACK", "CTRL_RUN", "EVIDENCE", "REPORT", "EXTRACT"]);
export const AuditAuditorStatusSchema = z.enum(["ACTIVE", "SUSPENDED", "REVOKED"]);
export const AuditRequestStateSchema = z.enum(["OPEN", "RESPONDED", "CLOSED"]);
export const AuditRequestMsgAuthorKindSchema = z.enum(["AUDITOR", "OWNER", "SYSTEM"]);
export const AuditAccessLogActionSchema = z.enum(["VIEW", "DOWNLOAD", "DENY", "EXPIRED"]);
...
export const WatermarkPolicyUpsert = z.object({
    text_template: z.string().min(1).default("CONFIDENTIAL • {company} • {auditor_email} • {ts}"),
    diagonal: z.boolean().default(true),
    opacity: z.number().min(0).max(1).default(0.15),
    font_size: z.number().min(1).max(72).default(24),
    font_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/).default("#FF0000")
});
