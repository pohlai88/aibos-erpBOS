...
export const ControlUpsert = z.object({
    ...
    domain: z.enum(["CLOSE", "AP", "AR", "REV", "FX", "BANK", "INV", "FIXEDASSET"]),
...
export const ControlQuery = z.object({
    domain: z.enum(["CLOSE", "AP", "AR", "REV", "FX", "BANK", "INV", "FIXEDASSET"]).optional(),
    frequency: z.enum(["PER_RUN", "MONTHLY", "QUARTERLY", "ANNUAL", "ADHOC"]).optional(),
...
export const AssignmentUpsert = z.object({
    ...
    (data) => data.run_id || data.task_id || data.entity_id,
    {
        message: "At least one assignment target (run_id, task_id, or entity_id) must be provided"
    }
);
...
export const ControlRunRequest = z.object({
    control_id: z.string().optional(),
    assignment_id: z.string().optional(),
    run_id: z.string().optional(),
    scheduled_at: z.string().optional() // ISO 8601, defaults to now
}).refine(
    (data) => data.control_id || data.assignment_id,
    {
...
export const ExceptionQuery = z.object({
    ...
    remediation_state: z.enum(["OPEN", "IN_PROGRESS", "RESOLVED", "WAIVED"]).optional(),
